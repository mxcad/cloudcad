...
async function runSetupWizard() {
  console.log('');
  log('cyan', '╔════════════════════════════════════════╗');
  log('cyan', '║        CloudCAD 首次启动               ║');
  log('cyan', '╚════════════════════════════════════════╝');
  console.log('');
  log('yellow', '检测到尚未配置环境，请选择：');
  console.log('');
  log('cyan', '  [1] 进入引导配置（推荐）');
  log('cyan', '  [2] 跳过配置（使用默认值）');
  console.log('');
  ...
  if (answer === '1') {
    await startSetupService();
  } else {
    // 复制 .env.example 到 .env
    const examplePath = path.join(PROJECT_ROOT, 'packages', 'backend', '.env.example');
    const envPath = path.join(PROJECT_ROOT, 'packages', 'backend', '.env');
    ...
    if (fs.existsSync(examplePath)) {
      ...
      log('green', '[✓] 已创建默认配置文件');
      console.log('');
      log('cyan', '配置文件位置: packages/backend/.env');
      log('cyan', '如需修改配置，可以：');
      log('cyan', '  1. 直接编辑上述文件，然后重启服务');
      log('cyan', '  2. 运行 setup.bat / setup.sh 重新配置');
    } else {
      log('yellow', '[!] 未找到 .env.example 文件');
    }
  }
}
 ...
 * 启动配置引导服务
 */
async function startSetupService() {
  console.log('');
  log('cyan', '╔════════════════════════════════════════╗');
  log('cyan', '║        引导配置方式                    ║');
  log('cyan', '╚════════════════════════════════════════╝');
  ...
  if (IS_WINDOWS) {
    log('cyan', '  [1] 网页引导（推荐）- 将自动打开浏览器');
    log('cyan', '  [2] 命令行引导');
  } else {
    log('cyan', '  [1] 命令行引导（推荐）');
    log('cyan', '  [2] 网页引导 - 输出 URL 供远程访问');
  }
  console.log('');
  ...
  const defaultChoice = IS_WINDOWS ? '1' : '1';
  const answer = await new Promise((resolve) => {
    rl.question(`${colors.bright}请输入选项 [${defaultChoice}]: ${colors.reset}`, (ans) => {
      rl.close();
      resolve(ans.trim() || defaultChoice);
    });
  });
...
async function startWebSetup() {
  log('yellow', '[!] 网页引导功能开发中，暂时使用命令行引导');
  await startCliSetup();
}
...
async function startCliSetup() {
  const setupWizardPath = path.join(RUNTIME_DIR, 'scripts', 'setup-cli-wizard.js');
  
  if (!fs.existsSync(setupWizardPath)) {
    log('red', '[错误] 未找到命令行引导脚本');
    log('cyan', '请手动配置 packages/backend/.env 文件');
    return;
  }
  ...
  // 运行配置向导脚本
  const result = spawnSync(NODE_EXE, [setupWizardPath], {
    cwd: PROJECT_ROOT,
    stdio: 'inherit',
    shell: IS_WINDOWS,
    env: {
...
async function bootstrap() {
  ...
  // 检测是否需要引导配置（仅在非部署模式下）
  if (!isDeployMode && args.length === 0) {
    if (await checkSetupRequired()) {
      await runSetupWizard();
      // 配置完成后继续显示菜单
      console.log('');
      log('green', '[✓] 配置完成，即将启动服务...');
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
  }
