...
```
MxcadChunkModule (packages/backend/src/mxcad/chunk/)
├── mxcad-chunk.module.ts       # 模块定义
├── chunk-upload.service.ts     # 分片上传核心服务
└── file-check.service.ts       # 文件存在性检查

MxcadUploadModule (packages/backend/src/mxcad/upload/)
...
UploadOrchestrator (packages/backend/src/mxcad/facade/)
...
```
packages/backend/src/mxcad/chunk/mxcad-chunk.module.ts
packages/backend/src/mxcad/chunk/chunk-upload.service.ts
packages/backend/src/mxcad/chunk/file-check.service.ts
```

#### 2. MxcadUploadModule 相关
```
packages/backend/src/mxcad/upload/mxcad-upload.module.ts
packages/backend/src/mxcad/upload/chunk-upload-manager.service.ts
```

#### 3. 相关 DTO 和类型
```
packages/backend/src/mxcad/dto/chunk-exist-response.dto.ts
packages/backend/src/mxcad/dto/check-chunk-exist.dto.ts
packages/backend/src/mxcad/services/file-upload-manager.types.ts
```

#### 4. 上传控制器（需重构）
```
packages/backend/src/mxcad/mxcad.controller.ts  # 需移除分片上传相关端点
```
...
**文件**: `packages/backend/src/mxcad/mxcad.module.ts`
...
**文件**: `packages/backend/src/mxcad/core/mxcad-core.module.ts`
...
**文件**: `packages/backend/src/mxcad/facade/mxcad-facade.module.ts`
...
**文件**: `packages/backend/src/mxcad/upload/file-merge.service.ts`
...
**文件**: `packages/backend/src/mxcad/upload/file-conversion-upload.service.ts`
...
**文件**: `packages/backend/src/mxcad/upload/upload-utility.service.ts`
...
**文件**: `packages/backend/src/mxcad/tus/tus.module.ts`
...
**文件**: `packages/backend/src/mxcad/tus/tus.service.ts`
...
**文件**: `packages/backend/src/mxcad/tus/tus-event-handler.service.ts`
...
**文件**: `packages/backend/src/mxcad/tus/tus.controller.ts`
...
**文件**: `packages/backend/src/mxcad/tus/tus.config.ts`
...
| 接入点 | 文件路径 | 描述 |
|--------|----------|------|
| TusModule 注册 | `packages/backend/src/mxcad/mxcad.module.ts` | 添加 TusModule 到 imports |
| TusController 路由 | `packages/backend/src/mxcad/tus/tus.controller.ts` | 注册 tus 协议端点 |
| Finish 事件处理 | `packages/backend/src/mxcad/tus/tus-event-handler.service.ts` | 处理上传完成后的业务逻辑 |
| 秒传检查 | `packages/backend/src/mxcad/mxcad.controller.ts` | 保留现有检查接口 |
