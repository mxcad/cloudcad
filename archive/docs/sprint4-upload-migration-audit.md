...
**位置**: `packages/backend/src/mxcad/tus/`
...
**参考**: 现有认证实现位于 `packages/backend/src/auth/`
...
**参考现有实现**: 见 `packages/frontend/src/utils/mxcadUploadUtils.ts`
...
代码搜索 `webuploader` 在 `packages/frontend/src/` 中**无匹配结果**。
...
| 文件 | 位置 | 功能 |
|------|------|------|
| `MxCadUploader.tsx` | `packages/frontend/src/components/` | 上传 UI 组件 |
| `useMxCadUploadNative.ts` | `packages/frontend/src/hooks/` | 上传逻辑 Hook |
| `mxcadUploadUtils.ts` | `packages/frontend/src/utils/` | 分片上传工具函数 |
| `hashUtils.ts` | `packages/frontend/src/utils/` | MD5 哈希计算（使用 SparkMD5） |
| `mxcadApi.ts` | `packages/frontend/src/services/` | MxCAD API 封装 |
| `mxcadManager.ts` | `packages/frontend/src/services/` | 上传后文件打开逻辑 |
...
```typescript
// packages/frontend/src/utils/uppyConfig.ts
...
**优先级 P0**:
1. `packages/frontend/src/components/MxCadUploader.tsx` → 替换为 Uppy 版本
2. `packages/frontend/src/hooks/useMxCadUploadNative.ts` → 改为 useUppyUpload Hook

**优先级 P1**:
3. `packages/frontend/src/utils/mxcadUploadUtils.ts` → 保留但标记为 deprecated
4. `packages/frontend/src/utils/uppyConfig.ts` → 新增，Uppy 配置文件
5. `packages/frontend/src/services/mxcadManager.ts` → 调整 openUploadedFile 调用
...
- `packages/frontend/src/utils/mxcadUploadUtils.ts` (保留作为参考)
- `packages/frontend/src/hooks/useMxCadUploadNative.ts` (保留作为参考)
