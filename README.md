...
│   ├── frontend/          # React 19 前端应用 (端口 3000)
│   ├── frontend_mobile/   # 移动端 H5 应用 (Vue 3)
│   ├── config-service/    # 部署配置中心 (端口 3002)
│   ├── storage-service/   # 统一文件管理层 (端口 3200)
│   ├── conversion-service/ # 转换引擎托管运行时 (端口 3100)
│   ├── api-sdk/           # 自动生成 API 客户端 (@cloudcad/api-sdk)
│   ├── db/                # Prisma schema 单一源 + 共享 Client (@cloudcad/db)
│   ├── contracts/         # 跨包 DI token + 接口契约 (@cloudcad/contracts)
│   ├── impl-mx/           # 私有 MX 官方实现包 (IMPL 动态加载)
│   └── mxVersionTool/     # MX 版本控制工具
├── docker/             # Docker 部署配置 (docker/docker-compose.yml)
...
```powershell
cd packages\backend
pnpm db:migrate    # 执行数据库迁移（schema 单一源在 packages/db，禁止仅 db push）
...
```powershell
cd packages/backend
pnpm db:migrate    # 运行迁移（schema 单一源在 packages/db，见 ADR-0027）
...
- [文档导航](docs/README.md) - 权威文档与归档目录索引
