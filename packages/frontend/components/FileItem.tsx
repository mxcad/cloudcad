  ...
  if (viewMode === 'grid') {
    return (
      ...
      >
        {/* 选择指示器 */}
        <div
          className={`absolute top-3 left-3 w-5 h-5 rounded-full border-2 transition-all duration-200 z-10
            ${isSelected
        ...
        <div
          className={`h-1 rounded-b-xl transition-colors duration-200 ${
            isSelected ? 'bg-indigo-500' : 'bg-transparent group-hover:bg-indigo-100'
          }`}
        />
      </div>
    );
