...
export const FileSystemContent: React.FC<FileSystemContentProps> = ({
  ...
  const canCreate = projectPermissions['FILE_CREATE'] !== false;
  const canUpload = projectPermissions['FILE_UPLOAD'] !== false;
  const canDeleteFile = projectPermissions['FILE_DELETE'] !== false;
  const canMoveFile = projectPermissions['FILE_MOVE'] !== false;
  const canCopyFile = projectPermissions['FILE_COPY'] !== false;
  const canRestoreFile = projectPermissions['FILE_TRASH_MANAGE'] !== false;
  const canPaste = clipboardMode === 'cut' ? canMoveFile : canCopyFile;
            ...
            (() => {
              ...
              const actionProps = {
                ...
                isCadFile:
                  !node.isFolder &&
                  !node.isRoot &&
                  ['.dwg', '.dxf', '.mxweb'].includes(
                    node.extension?.toLowerCase() || ''
                  ),
                isFolder: node.isFolder,
                hasMissingExternalReferences:
                  node.hasMissingExternalReferences || false,
                ...
                canManageExternalReference:
                  nodePerms.canManageExternalReference,
                ...
                onRestore: !!onRestore,
                onPermanentlyDelete: !!onPermanentlyDelete,
                onDeleteNode: node.isRoot
                  ? !!nodePerms.onDeleteNode
                  : !!nodePerms.onDeleteNode || !!onDelete,
                isSearchResult: !!isSearchResult,
                onOpen: !!onFileOpen,
                onOpenInNewTab: !!onOpenInNewTab,
                onOpenFileLocation: !!onOpenFileLocation,
              ...
              const { main, destructive } = getActionGroups(availableActions);
              return (
                <>
                  {main.map((action) => (
                    <Menu.Item
                  ...
                  {destructive.length > 0 && main.length > 0 && (
                    <Menu.Separator />
                  )}
              ...
              <Menu.Item
                ...
                icon={
                  ...
                  >
                    ...
                    <path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2" />
                  </svg>
                }
                onClick={() => {
                   onClearTrash();
