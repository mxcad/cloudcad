...
export default function Gallery() {
  ...
  const isBlocksRoute = location.pathname.includes('/blocks');
  ...
  const fetchTypes = async () => {
    try {
      ...
      const newTypes = response.data?.allblocks || [];
      setTypes(newTypes);
      // 分类加载完成后自动获取文件列表
      fetchFiles(0);
    } catch (error) {
  ...
  const handleRemoveFromGallery = async (file: GalleryFile) => {
    ...
    try {
      ...
      const errorMessage = (error as Error).message || '移除失败，请稍后重试';
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await galleryApi.updateGalleryItem(galleryType, editingFile.uuid, {
      ...
      const errorMessage = (error as Error).message || '更新失败，请稍后重试';
      showToast(errorMessage, 'error');
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button
              variant={galleryType === 'drawings' ? 'primary' : 'ghost'}
            ...
            <Button
              variant={galleryType === 'blocks' ? 'primary' : 'ghost'}
