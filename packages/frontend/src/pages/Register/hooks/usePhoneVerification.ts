...
export function usePhoneVerification({
  setFieldErrors,
}: PhoneVerificationOptions): PhoneVerificationReturn {
  ...
  const handleSendCode = useCallback(async () => {
    ...
    try {
      ...
      const { authControllerCheckFieldUniqueness, authControllerSendSmsCode } =
      ...
      // 手机号可用，发送验证码
      const sendRes = await authControllerSendSmsCode({
        body: {
          phone: phoneForm.phone,
        },
      });
      // SDK 默认不抛错：失败时响应体在 result.error
      if (sendRes.error) throw sendRes.error;
      if ((sendRes.data as { success?: boolean } | undefined)?.success) {
        setCountdown(60);
      } else {
        setFieldErrors((prev) => ({
          ...prev,
          phone:
            (sendRes.data as { message?: string } | undefined)?.message ||
            t('发送验证码失败'),
        }));
      }
    } catch (err: unknown) {
