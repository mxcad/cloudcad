 ...
 * 获取节点在父目录中的分页上下文
 *
 * 用于搜索结果「打开所在位置」时，计算目标文件在父目录中的页码和位置
 */
 ...
 * 获取节点属性信息
 *
 * 返回文件/文件夹的完整属性，包括统计信息、权限等
 */
export const fileSystemControllerGetNodeProperties = <ThrowOnError extends boolean = false>(options: Options<FileSystemControllerGetNodePropertiesData, ThrowOnError>) => (options.client ?? client).get<FileSystemControllerGetNodePropertiesResponses, FileSystemControllerGetNodePropertiesErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/v1/file-system/nodes/{nodeId}/properties',
    ...options
});
