...
export async function checkDuplicateFile(
  ...
  try {
    const duplicateCheck = await mxcadUploadControllerCheckFileExist({
      body: { fileHash, filename, nodeId, fileSize },
    });
    // SDK 默认不抛错：失败时错误在 result.error。静默返回"无重复"
    // 会让用户绕过重复确认直接上传产生重复文件（历史 bug）
    if (duplicateCheck.error) throw duplicateCheck.error;

    return {
      isDuplicate: !!duplicateCheck.data?.exists,
      existingNodeId: duplicateCheck.data?.nodeId ?? null,
    };
  } catch (error) {
