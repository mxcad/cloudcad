...
export const useFileSystemNavigation = ({
    ...
    (node: FileSystemNode) => {
      ...
      if (node.isFolder) {
        if (mode === 'personal-space') {
          ...
          const effectiveProjectId = node.isRoot
            ? node.id
            : urlProjectId || node.projectId || '';
          navigate(`/projects/${effectiveProjectId}/files/${node.id}`);
        }
      } else {
        if (
  ...
  const handleDownload = useCallback(
    async (node: FileSystemNode) => {
      // 判断是否为 CAD 文件
      const isCadFile = CAD_EXTENSIONS.some((ext) =>
      ...
      try {
        ...
        // 内容为 "undefined" 的损坏文件并弹"下载成功"（历史 bug）
        ...
        // SDK error 是普通对象（非 Error 实例），统一用 getErrorMessage 提取；
        // CORS/网络错误是真实 Error 实例，走专门文案
  ...
  const handleDownloadWithFormat = useCallback(
    async (
      ...
      try {
        ...
        // useLibraryOperations 的同场景已写 if (error) throw error）
        if (result.error) throw result.error;
        const blobData = result.data;
        const blob =
          blobData instanceof Blob
        ...
        // SDK error 是普通对象（非 Error 实例），统一用 getErrorMessage 提取后端消息
        const errorMessage = getErrorMessage(error) || t('文件下载失败');
