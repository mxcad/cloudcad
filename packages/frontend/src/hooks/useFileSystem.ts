...
import type { CreateProjectDto, ProjectDto, TrashItemDto } from '../types/api-client';
import { ToastType, Toast } from '../components/ui/Toast';
import { PaginationMeta } from '../components/ui/Pagination';
import { handleError } from '../utils/errorHandler';
import { useFileSystemStore } from '../stores/fileSystemStore';
import { PdfOptions } from '@/components/modals/DownloadFormatModal';
...
export const useFileSystem = () => {
  ...
  const prevParamsRef = useRef< UseFileSystemResult | null>(null);
  ...
  const handleSelectAll = useCallback(() => {
    ...
    if (selectedNodes.size === allNodeIds.length) {
      ...
      // 设置最后选中的节点为第一个节点
      if (allNodeIds.length > 0 && allNodeIds[0]) {
  ...
  const handleUpdateProject = useCallback(
    async (id: string, data: CreateProjectDto) => {
      try {
      ...
      showConfirm(
        ...
        async () => {
          try {
            ...
            console.warn('[useFileSystem] 恢复节点失败', error);
            showToast('恢复失败');
          }
        },
        'warning'
