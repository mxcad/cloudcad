...
export function useSidebarSettings() {
  ...
  // 设置默认图纸子 Tab
  const setDefaultDrawingsSubTab = useCallback(
    (tab: DrawingsSubTab) => {
  ...
  const setWidth = useCallback(
    (width: number) => {
      // 限制宽度范围
      const clampedWidth = Math.max(340, Math.min(600, width));
  ...
  // 设置上次激活的 Tab
  const setLastActiveTab = useCallback(
    (tab: SidebarTab | null) => {
  ...
  // 设置上次激活的图纸子 Tab
  const setLastDrawingsSubTab = useCallback(
    (tab: DrawingsSubTab | null) => {
