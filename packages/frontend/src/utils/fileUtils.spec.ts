...
describe('fileUtils', () => {
  ...
  describe('getLibraryThumbnailUrl', () => {
    it('should return drawing library thumbnail URL', () => {
      expect(getLibraryThumbnailUrl('node-1', 'drawing')).toContain(
        '/library/drawing/nodes/node-1/thumbnail'
      );
    });

    it('should return block library thumbnail URL', () => {
      expect(getLibraryThumbnailUrl('node-1', 'block')).toContain(
        '/library/block/nodes/node-1/thumbnail'
      );
    });

    it('should return empty string when node has no id', () => {
      expect(getLibraryThumbnailUrl('', 'drawing')).toBe('');
    });
  });
