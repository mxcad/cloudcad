...
export const TourProvider: React.FC<TourProviderProps> = ({
  ...
  const visibleGuides = useMemo(
    () => filterVisibleGuides(effectiveGuides),
    [effectiveGuides]
  );
  ...
  // 路径过滤由 GlobalTourRenderer 的 shouldShowStartModal 处理，不在 effect 中判断
  useEffect(() => {
    if (!isAuthenticated) return;
    if (initializedAfterLoginRef.current) return;
  ...
  const completeTour = useCallback(() => {
    ...
    if (originalLocationRef.current) {
      console.log(
        `[Tour] Navigating back to original location: ${originalLocationRef.current}`
      );
