...
vi.mock('../ui/Modal', () => ({
  Modal: ({
    ...
    return (
      <div data-testid="modal" data-size={size} data-zindex={zIndex}>
...
function renderTourStartModal(
  ...
  const { isOpen = true, onDismiss = vi.fn(), onViewNow = vi.fn() } = options;
...
describe('TourStartModal', () => {
  ...
  describe('欢迎标题显示', () => {
    ...
    it('应该显示副标题提示', () => {
      ...
      expect(
        screen.getByText('我们为您准备了新手引导，帮助您快速上手')
      ).not.toBeNull();
    });
  ...
  describe('功能特性列表显示', () => {
    ...
    it('应该显示业务流程教程的特性描述', () => {
      ...
      expect(
        screen.getByText('学习文件上传、成员管理等核心功能')
      ).not.toBeNull();
    });
  ...
  describe('按钮点击事件', () => {
    ...
    it('点击"稍后再说"按钮应该调用 onDismiss', () => {
      ...
      const dismissButton = screen.getByTestId('button-secondary');
  ...
  describe('底部提示文字显示', () => {
    it('应该显示底部提示文字', () => {
      ...
      expect(
        screen.getByText('您可以随时从侧边栏的「帮助引导」入口访问引导中心')
      ).not.toBeNull();
    });
  });
  ...
  // ============================================
  describe('边界情况', () => {
    it('当快速点击"稍后再说"按钮时，每次点击都会触发 onDismiss（共 3 次）', () => {
      const { onDismiss } = renderTourStartModal();
