...
describe('TourOverlay', () => {
  ...
  describe('基础渲染', () => {
    ...
    it('应该在 isActive 为 true 时渲染遮罩层', async () => {
      ...
      vi.spyOn(document, 'querySelectorAll').mockImplementation(function(this: Document, selector: string) {
  ...
  describe('目标元素查找', () => {
    it('应该使用 data-tour 属性查找目标元素', async () => {
      ...
      expect(document.querySelectorAll).toHaveBeenCalledWith('[data-tour="my-button"]');
    });
  ...
  describe('SVG 遮罩渲染', () => {
    ...
    it('应该在有目标元素时渲染高亮边框', async () => {
      ...
      // 验证高亮边框已渲染
      await waitFor(() => {
        expect(document.querySelector('.tour-highlight-border')).not.toBeNull();
      });
    });
  });
