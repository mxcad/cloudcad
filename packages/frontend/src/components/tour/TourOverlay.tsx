...
export const TourOverlay: React.FC<TourOverlayProps> = ({
  ...
  useEffect(() => {
    ...
    const timeout =
      step.waitForElement === 'auto' ? 5000 : (step.waitForElement ?? 3000);

    const findElement = async () => {
      // 先检查跳过条件，满足则不经等待直接跳过
      if (checkSkipCondition(step)) {
        console.log(
          `[Tour] Step "${step.target}" skipped by condition (pre-check): ${step.skipCondition?.type}`
        );
        setHighlight({ rect: null, element: null, isLoading: false });
        onSkipStep();
        return;
      ...
      if (result.element) {
        ...
        // 元素未找到
        // 1. 再次检查 skipCondition（此时元素可能因动态变化而消失）
        if (checkSkipCondition(step)) {
          // 跳过条件满足，自动跳到下一步
          console.log(
            `[Tour] Step "${step.target}" skipped by condition (post-check): ${step.skipCondition?.type}`
          );
          onSkipStep();
          return;
        }
        ...
        if (step.fallbackContent) {
          ...
          // 3. 没有任何处理方式，显示错误状态
          console.warn(
            `[Tour] Element "${step.target}" not found and no fallback/skipCondition defined`
          );
          setHighlight({
            rect: null,
            element: null,
  ...
  const handleBlockerClick = useCallback(() => {
    if (!isInteractiveMode && highlight.rect !== null) {
      // 展示模式且目标元素已定位：点击暗部 → 下一步
      // 目标元素未定位时（fallback/loading）阻止前进，防止用户跳过未完成的步骤
      onNext();
    }
    // 交互模式：点击暗部 → 不做任何事（阻止误操作）
  }, [isInteractiveMode, highlight.rect, onNext]);
  ...
  const hlLeft = highlightRect
    ? Math.max(0, highlightRect.left - HIGHLIGHT_PADDING)
    : 0;
  const hlTop = highlightRect
    ? Math.max(0, highlightRect.top - HIGHLIGHT_PADDING)
    : 0;
        ...
        <>
          ...
          <div
            style={{
              ...
              left: 0,
              width: hlLeft,
              height: Math.max(0, hlBottom - hlTop),
              pointerEvents: 'auto',
            }}
            onClick={handleBlockerClick}
          />
