...
import { nodeControllerGetNode, nodeControllerGetRootNode } from '@/api-sdk';
...
export const useDrawingOpener = (isAuthenticated: boolean) => {
  // 查询私人空间（使用共享 hook）
  ...
  const getNode = useCallback(async (nodeId: string) => {
    ...
    // SDK 默认不抛错：显式抛出让调用方 catch 展示真实错误，
    // 否则失败时 file 为 undefined 会被误报为"文件尚未转换完成"
  ...
  const getRootNode = useCallback(async (nodeId: string) => {
    const result = await nodeControllerGetRootNode({ path: { nodeId } });
    if (result.error) throw result.error;
    return result.data;
  }, []);
  ...
  const handleDrawingOpen = useCallback(
    async (node: FileSystemNode, libraryType?: 'drawing' | 'block') => {
      try {
        ...
        globalShowToast(getErrorMessage(error), 'error');
      }
    },
    [getNode, getRootNode]
  );
