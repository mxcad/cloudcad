...
function getDisplayName(
  ...
  const level = categories.findIndex((c) =>
  ...
  const sameNameItems = currentLevelItems.items.filter(
...
const CategoryLevelRow: React.FC<{
  ...
  return (
    <div
      className={`${styles.categoryRow} ${isLastLevel ? styles.lastRow : ''}`}
    >
    ...
    <div ref={containerRef} className={styles.categoryTabsWrapper}>
      ...
      <div className={styles.categoryTabsContainer}>
        {categories.map((categoryLevel, index) => {
          // 任何级别如果只有"全部"则不显示
          const isHidden =
            categoryLevel.items.length === 1 &&
            categoryLevel.items[0]?.id === 'all';
          if (isHidden) {
            return null;
          }
          ...
          // 判断是否是最后一个可见级别
          const isLastLevel = categories
            .slice(index + 1)
            .every((c) => c.items.length === 1 && c.items[0]?.id === 'all');
          ...
          return (
            <CategoryLevelRow
              ...
              onSelect={(categoryId) =>
                onSelect(categoryLevel.level, categoryId)
              }
