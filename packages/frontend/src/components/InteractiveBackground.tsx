    ...
    const animate = () => {
      ...
      orbs.forEach((orb) => {
        ...
        const driftX =
          ...
          Math.cos(time * orb.speedX * 600 + orb.phase * 1.5) *
            (orb.driftRange * 0.6) +
          Math.sin(time * orb.speedX * 300 + orb.phase * 0.5) *
            (orb.driftRange * 0.3);
        const driftY =
          Math.cos(time * orb.speedY * 1000 + orb.phase) * orb.driftRange +
          Math.sin(time * orb.speedY * 800 + orb.phase * 0.7) *
            (orb.driftRange * 0.6) +
          Math.cos(time * orb.speedY * 400 + orb.phase * 1.2) *
            (orb.driftRange * 0.3);
        ...
        gradient.addColorStop(
          0.4,
          `rgba(${orb.color[0]}, ${orb.color[1]}, ${orb.color[2]}, ${orb.opacity * 0.5})`
        );
        gradient.addColorStop(
          0.8,
          `rgba(${orb.color[0]}, ${orb.color[1]}, ${orb.color[2]}, ${orb.opacity * 0.1})`
        );
      ...
      for (let x = 0; x <= width; x += gridSize) {
        ...
        for (let i = 0; i < segments; i++) {
          ...
          const opacity =
            baseGridOpacity + (maxGridOpacity - baseGridOpacity) * lighting;
