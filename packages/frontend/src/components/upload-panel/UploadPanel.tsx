...
export function UploadPanel() {
  ...
  return createPortal(
    <div
      className="upload-panel-portal"
      style={{ zIndex: Z_LAYERS.UPLOAD_PANEL }}
    >
      {expanded ? (
        <div className="upload-panel">
          ...
          <div className="upload-panel-header">
            ...
            <div className="upload-panel-actions">
              {hasActive && (
                <button title={t('全部暂停')} onClick={pauseAll}>
                  <Pause size={14} />
                </button>
              )}
              {stats.paused > 0 && (
                <button title={t('全部恢复')} onClick={resumeAll}>
                  <Play size={14} />
                </button>
              )}
              <button title={t('收起')} onClick={() => setExpanded(false)}>
                <ChevronDown size={14} />
              </button>
            </div>
          </div>
