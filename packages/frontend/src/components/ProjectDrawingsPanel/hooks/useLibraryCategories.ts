...
async function fetchCategories(
  ...
  const catData = categoriesResponse.data as
    { categories?: CategoryLevel[] } | undefined;
  let finalCategories = getPlaceholderCategories();
  if (catData?.categories && catData.categories.length > 0) {
    const processed = catData.categories.map((level) => ({
      ...level,
      items: [{ id: 'all', name: t('全部') }, ...level.items],
    }));
  ...
  const getDefaultCategoryPath = (): string[] => {
    try {
      const saved = localStorage.getItem(
        `library_category_path_${libraryType}`
      );
      if (saved) {
