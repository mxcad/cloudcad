...
export interface TruncateTextProps {
  children: string;
  mode?: TruncateMode;
  /** 最多显示行数（>1 时使用 CSS line-clamp 多行截断，悬停 title 显示全文） */
  maxLines?: number;
  maxChars?: number;
 ...
 * direction='vertical' 时按高度检测（line-clamp 多行截断场景）。
 */
...
export const TruncateText: React.FC<TruncateTextProps> = ({
  ...
  const overflowDirection: 'horizontal' | 'vertical' =
    maxLines && maxLines > 1 ? 'vertical' : 'horizontal';
  const cssOverflowing = useOverflowDetector(textRef, overflowDirection);
  ...
  if (mode === 'middle') {
    ...
    const hasExtension =
      lastDotIndex !== -1 && lastDotIndex < children.length - 1;
  ...
  // --- maxLines 模式：CSS line-clamp 多行截断（优先于单行模式） ---
  if (maxLines && maxLines > 1) {
    ...
    return (
      <span
        ...
        style={cssStyle}
        title={
          showTooltip && tooltipDisplayText ? tooltipDisplayText : undefined
        }
      >
        {children}
      </span>
