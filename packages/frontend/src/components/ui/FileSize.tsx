  ...
  return (
    <div className={`flex items-center gap-0 ${className ?? ''}`}>
      <input
        ...
        step={step}
        className="flex-1 px-2 py-1 text-xs rounded-l-[3px] min-w-0"
        style={{
      ...
      <select
        ...
        disabled={disabled}
        className="px-1.5 py-1 text-xs rounded-r-[3px] cursor-pointer shrink-0"
        style={{
          background: disabled ? 'var(--bg-tertiary)' : 'var(--bg-primary)',
          border: '1px solid var(--border-default)',
          borderLeft: 'none',
...
export function MbFileSizeInput({
  mbValue,
  onChange,
  disabled,
  min,
}: {
  mbValue: number | undefined;
  onChange: (mb: number | undefined) => void;
  disabled?: boolean;
  min?: number;
}) {
  return (
    <FileSizeInput
      value={toBytes(mbValue ?? 0, 'MB')}
      onChange={(bytes) =>
        onChange(bytes !== undefined ? fromBytes(bytes, 'MB') : undefined)
      }
      units={['MB', 'GB']}
      defaultUnit="MB"
      min={min}
