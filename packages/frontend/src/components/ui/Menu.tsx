...
Menu.Content = ({
  ...
  return (
    <DropdownMenu.Portal>
      ...
      >
        <div style={{ width, minWidth: width || undefined }}>{children}</div>
      </DropdownMenu.Content>
    </DropdownMenu.Portal>
  );
};
...
Menu.Item = ({
  ...
  return (
    ...
    >
      {icon && (
        <span className="flex-shrink-0 w-6 h-6 flex items-center justify-center text-[var(--text-tertiary)]">
          {icon}
        </span>
      )}
      <div className="flex-1">
        ...
        {description && (
          <div className="text-[var(--text-xs)] text-[var(--text-muted)] whitespace-nowrap mt-0.5">
            {description}
          </div>
        )}
      </div>
...
Menu.Submenu = ({ children, label, icon, disabled }) => (
  <DropdownMenu.Sub>
    ...
    >
      ...
      <ChevronRight
        size={14}
        className="flex-shrink-0 text-[var(--text-muted)] group-data-[highlighted]:text-[var(--text-secondary)] transition-colors duration-150"
      />
    </DropdownMenu.SubTrigger>
    <DropdownMenu.Portal>
...
Menu.Loading = ({ children, className = '' }) => (
  <div
    className={`flex items-center justify-center gap-2 px-2.5 py-3 text-[var(--text-sm)] text-[var(--text-muted)] ${className}`}
  >
    {children}
  </div>
);
...
Menu.Empty = ({ children, className = '' }) => (
  <div
    className={`flex flex-col items-center justify-center gap-2 px-2.5 py-4 text-[var(--text-sm)] text-[var(--text-muted)] ${className}`}
  >
    {children}
  </div>
);
...
Menu.Error = ({ children, className = '' }) => (
  <div
    className={`px-2.5 py-3 text-[var(--text-sm)] text-[var(--error)] text-center ${className}`}
  >
    {children}
  </div>
);
