...
export const Modal: React.FC<ModalProps> = ({
  ...
  const parseTransform = (el: HTMLElement) => {
    ...
    if (m3d)
      return { dx: parseFloat(m3d[1] ?? '0'), dy: parseFloat(m3d[2] ?? '0') };
    const m2d = t.match(/translate\(([^,]+),\s*([^)]+)\)/);
    if (m2d)
      return { dx: parseFloat(m2d[1] ?? '0'), dy: parseFloat(m2d[2] ?? '0') };
    return { dx: 0, dy: 0 };
  };
  ...
  React.useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (w <= viewW) {
        ...
        newLeft = Math.max(
          -(w - minVisible),
          Math.min(newLeft, viewW - minVisible)
        );
      }
      if (h <= viewH) {
        ...
        newTop = Math.max(
          -(h - minVisible),
          Math.min(newTop, viewH - minVisible)
        );
      }
