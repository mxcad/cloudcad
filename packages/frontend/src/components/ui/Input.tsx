  ...
  (
    ...
    return (
      ...
      >
        {LeftIcon && (
          <span
            className={`absolute ${cfg.leftOffset} pointer-events-none`}
        ...
        {showPasswordToggle && type === 'password' && (
          ...
          >
            {passwordVisible ? (
              <EyeOff size={cfg.btnSize} />
            ) : (
              <Eye size={cfg.btnSize} />
            )}
          </button>
        )}
        {RightIcon && !showPasswordToggle && (
          <span
            className={`absolute ${cfg.rightOffset} pointer-events-none`}
            style={{ color: 'var(--text-muted)' }}
          >
        ...
        {rightNode && !showPasswordToggle && (
          <div
            className={`absolute ${cfg.rightOffset} top-1/2 -translate-y-1/2 flex items-center gap-0.5 ${rightNodeClassName}`}
          >
            {rightNode}
          </div>
        )}
