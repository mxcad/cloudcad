...
import {
  ...
  FileSizeInput,
  type SizeUnit,
} from '@/components/ui/FileSize';
import { t, $t } from '@/languages';
import { useIsMobile } from '@/lib/useIsMobile';
import { appendToken, removeToken } from './search-utils';
...
function parseSortValue(value: string): {
  ...
  return {
    sortBy: parts[0],
    sortOrder: (parts[1] as 'asc' | 'desc') || 'desc',
  };
}
...
export function getActiveFilterChips(
  ...
  if (scope !== 'project') {
    if (filters.extensions && filters.extensions.length > 0) {
      chips.push({
        ...
        onRemove: () =>
          onChange(cleanFilters({ ...filters, extensions: undefined })),
      });
    }
    ...
    if (filters.timeRange) {
      ...
      chips.push({
        ...
        onRemove: () =>
          onChange(cleanFilters({ ...filters, timeRange: undefined })),
      });
    }

    const modifiedLabel = getDateLabel(
    ...
    const createdLabel = getDateLabel(
    ...
    if (sizeLabel) {
      chips.push({
        ...
        onRemove: () =>
          onChange(
            cleanFilters({ ...filters, sizeMin: undefined, sizeMax: undefined })
          ),
      });
    }
  }
  ...
  if (filters.sortBy) {
    const opts =
      scope === 'project' ? getProjectSortOptions() : getSortOptions();
    ...
    chips.push({
      key: 'sort',
      label: $t('排序: {label}', {
        label: sortOption?.label || `${filters.sortBy} ${filters.sortOrder}`,
      }),
      onRemove: () =>
        onChange(
          cleanFilters({ ...filters, sortBy: undefined, sortOrder: undefined })
        ),
    });
  }
...
export const SearchFilters: React.FC<SearchFiltersProps> = ({
  ...
  const extQueryPrefix = 'ext:';

  useEffect(() => {
    if (!onSearchQueryChange || !searchQuery) return;
    const parsedExtensions = searchQuery
      .split(/\s+/)
      .filter((t) => t.startsWith(extQueryPrefix))
      .map((t) => t.slice(extQueryPrefix.length));
    const currentExtensions = filters.extensions || [];
    const hasDiff =
      parsedExtensions.length !== currentExtensions.length ||
      parsedExtensions.some((e) => !currentExtensions.includes(e)) ||
      currentExtensions.some((e) => !parsedExtensions.includes(e));
    if (hasDiff) {
      // query 中 ext: token 全部删除时同步清空（否则列表仍按旧格式过滤）
      onChange({
        ...filters,
        extensions: parsedExtensions.length > 0 ? parsedExtensions : undefined,
      });
    }
  }, [searchQuery]);

  const syncExtToQuery = useCallback(
    (extensions: string[] | undefined) => {
      if (!onSearchQueryChange) return;
      const prevExts = filters.extensions || [];
      const nextExts = extensions || [];
      let q = searchQuery;
      for (const ext of prevExts) {
        if (!nextExts.includes(ext)) {
          q = removeToken(q, `${extQueryPrefix}${ext}`);
        }
      }
      for (const ext of nextExts) {
        if (!prevExts.includes(ext)) {
          q = appendToken(q, `${extQueryPrefix}${ext}`);
        }
      }
      onSearchQueryChange(q);
    },
    [filters.extensions, onSearchQueryChange, searchQuery]
  );
        ...
        <div className="space-y-1.5">
          ...
          <CheckboxList
            ...
            onChange={(v) => {
              ...
              onChange(cleanFilters({ ...filters, extensions: next }));
              syncExtToQuery(next);
            }}
          />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <Select
            ...
            value={
              filters.sizeMin !== undefined || filters.sizeMax !== undefined
                ? getSizePresets().find(
                    (p) =>
                      p.min === filters.sizeMin && p.max === filters.sizeMax
                  )?.value || ''
                : ''
            }
            onChange={(v) => {
          ...
          <div className="flex items-center gap-1">
            <FileSizeInput
              ...
              onChange={(v) =>
                onChange(cleanFilters({ ...filters, sizeMin: v }))
              }
              unit={sizeUnit}
            ...
            <FileSizeInput
              ...
              onChange={(v) =>
                onChange(cleanFilters({ ...filters, sizeMax: v }))
              }
              unit={sizeUnit}
              onUnitChange={setSizeUnit}
              placeholder={t('最大')}
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="flex-1 min-w-0">
              <DatePicker
                size="xs"
                value={dateOnlyToIso(filters.modifiedAtFrom)}
                onChange={(v) =>
                  onChange(
                    cleanFilters({
                      ...filters,
                      modifiedAtFrom: isoToDateOnly(v) || undefined,
                      timeRange: undefined,
                    })
                  )
                }
            ...
            <div className="flex-1 min-w-0">
              <DatePicker
                size="xs"
                value={dateOnlyToIso(filters.modifiedAtTo)}
                onChange={(v) =>
                  onChange(
                    cleanFilters({
                      ...filters,
                      modifiedAtTo: isoToDateOnly(v) || undefined,
                      timeRange: undefined,
                    })
                  )
                }
                  ...
                  onClick={() =>
                    onChange(
                      cleanFilters({
                        ...filters,
                        timeRange:
                          filters.timeRange === p.value ? undefined : p.value,
                        modifiedAtFrom: undefined,
                        modifiedAtTo: undefined,
                      })
                    )
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="flex-1 min-w-0">
              <DatePicker
                size="xs"
                value={dateOnlyToIso(filters.createdAtFrom)}
                onChange={(v) =>
                  onChange(
                    cleanFilters({
                      ...filters,
                      createdAtFrom: isoToDateOnly(v) || undefined,
                    })
                  )
                }
              />
            ...
            <div className="flex-1 min-w-0">
              <DatePicker
                size="xs"
                value={dateOnlyToIso(filters.createdAtTo)}
                onChange={(v) =>
                  onChange(
                    cleanFilters({
                      ...filters,
                      createdAtTo: isoToDateOnly(v) || undefined,
                    })
                  )
                }
              />
  ...
  return (
    <div className="inline-flex">
      <div ref={buttonRef} className="flex items-center gap-1">
        <Tooltip
          content={hasActiveFilters ? t('已启用筛选条件') : t('筛选条件')}
          disabled={open}
        >
          <button
            type="button"
        ...
        createPortal(
          <>
            ...
            {isMobile ? (
              ...
              >
                <div className="p-4 space-y-4">{panelContent}</div>
              </div>
            ) : (
              <div
