...
interface BreadcrumbNavigationProps {
  ...
  /** 是否可编辑（default 模式下生效） */
  editable?: boolean;
  /** 编辑时是否保留根节点名称（项目管理模式为 true） */
  preserveRootInEdit?: boolean;
  /** 返回按钮 */
  onBack?: () => void;
  /** 返回项目列表 */
...
export const BreadcrumbNavigation: React.FC<BreadcrumbNavigationProps> = ({
  ...
  // 显示用路径（包含根目录名称）
  const displayPathString = useMemo(
    () => '/' + breadcrumbs.map((b) => b.name).join(' / '),
    [breadcrumbs]
  );

  // 编辑用路径
  // - 项目管理（preserveRootInEdit=true）：保留项目名称，格式 "项目名称/xxx"
  // - 其他：跳过根节点，用 "/" 表示根目录，格式 "/xxx"
  // - 根节点使用 editName（标识符）用于路由匹配
  const editPathString = useMemo(() => {
    const getName = (b: BreadcrumbItem) => b.editName || b.name;
    if (preserveRootInEdit) {
      return breadcrumbs.map(getName).join(' / ');
    }
    return breadcrumbs.length <= 1
      ? '/'
      : '/' + breadcrumbs.slice(1).map(getName).join(' / ');
  }, [breadcrumbs, preserveRootInEdit]);

  const startEditing = useCallback(() => {
    setEditValue(editPathString);
    setIsEditing(true);
  ...
  const submitEditing = useCallback(() => {
    ...
    if (trimmed && trimmed !== editPathString) {
      // 项目管理模式：直接提交（保留项目名称）
      // 其他模式：去掉开头的"/"，因为后端使用" > "分隔
      const pathForSubmit = preserveRootInEdit
        ? trimmed
        : trimmed.replace(/^\//, '');
      onPathSubmit?.(pathForSubmit);
    }
    setIsEditing(false);
  }, [editValue, editPathString, preserveRootInEdit, onPathSubmit]);
  ...
  return (
    <div
      ref={containerRef}
      className="flex items-center gap-0.5 text-xs sm:text-sm whitespace-nowrap"
    >
      {variant === 'panel' && (
        <>
          <button
          ...
          <ChevronRightIcon
            size={12}
            className="text-slate-400 flex-shrink-0 mx-0.5"
          />
          {onBackToProjects && (
            <>
              <button
      ...
      collapsedBreadcrumb.visible[0] ? (
        <>
              ...
              <Menu.Content align="start" side="bottom" sideOffset={4}>
                {collapsedBreadcrumb.collapsed.map((item) => (
                  <Menu.Item
                    key={item.id}
                    onClick={() =>
                      onNavigate(item, document.createElement('button'))
                    }
                  >
                    {item.name}
                  </Menu.Item>
          ...
          {collapsedBreadcrumb.visible.slice(1).map((item, idx) => {
            ...
            return renderCrumb(
              item,
              breadcrumbs.length - collapsedBreadcrumb.visible.length + 1 + idx,
              isLast
            );
          })}
        </>
        ...
        breadcrumbs.map((crumb, index) =>
          renderCrumb(crumb, index, index === breadcrumbs.length - 1)
        )
      ) : (
        <>
          {breadcrumbs.map((crumb, index) =>
