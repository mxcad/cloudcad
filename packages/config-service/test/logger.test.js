...
describe('logger (零依赖 JSON 日志)', () => {
  it('输出 JSON 单行，含 time/level/service/message', () => {
    ...
    assert.equal(record.service, 'config-service');
  ...
  it('日志落盘到 data/logs/<service>/app-YYYY-MM-DD.log', () => {
    ...
    const file = path.join(process.env.LOG_DIR, 'config-service', `app-${dateStr}.log`);
  ...
  it('过期日志文件按保留策略清理', async () => {
    const dir = path.join(process.env.LOG_DIR, 'config-service');
