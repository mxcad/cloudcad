...
const CONFIG_GROUPS = [
  {
    ...
    items: [
      ...
      { key: 'FRONTEND_URL', label: '前端地址', type: 'text', sensitive: false },
    ]
  },
  {
    name: '安全配置',
    ...
    description: '修改后重启服务即可生效',
    items: [
      { key: 'JWT_SECRET', label: 'JWT 密钥', type: 'secret', sensitive: true, canGenerate: true },
    ]
  },
  {
    name: '邮件配置',
    ...
    items: [
      ...
      { key: 'MAIL_PASSWORD', label: 'SMTP 密码', type: 'password', sensitive: true },
  ...
  },
  {
    ...
    key: 'database',
    controllable: false,
    description: '⚠️ 修改此项需同时修改 PostgreSQL 服务配置，否则会导致连接失败',
    items: [
      ...
      { key: 'DB_PASSWORD', label: '数据库密码', type: 'password', sensitive: true },
    ]
  },
  {
    name: 'Redis 配置',
    ...
    items: [
      ...
      { key: 'REDIS_PASSWORD', label: 'Redis 密码', type: 'password', sensitive: true },
    ]
  }
];
...
function checkLoginLock(ip) {
  ...
  if (attempts.count >= MAX_LOGIN_ATTEMPTS) {
    ...
    if (elapsed < LOCKOUT_TIME) {
      return { locked: true, remaining: Math.ceil((LOCKOUT_TIME - elapsed) / 1000) };
    }
...
const PM2_SERVICES = ['postgresql', 'redis', 'cooperate', 'config-service', 'backend', 'frontend'];

function runPm2Command(args, options = {}) {
  try {
    // 使用 pm2.cmd/pm2 包装脚本（确保 PM2 daemon 能找到 node）
    const pm2Cmd = process.platform === 'win32'
...
function startService(serviceName) {
  ...
  if (INFRASTRUCTURE_SERVICES.includes(serviceName)) {
    ...
    const result = runPm2Command(['start', ECOSYSTEM_PATH, '--only', serviceName]);
  ...
  if (serviceName === 'backend' || serviceName === 'frontend') {
    const tempConfigPath = path.join(PROJECT_ROOT, 'offline-data', 'pm2-deploy.config.js');
    if (fs.existsSync(tempConfigPath)) {
      const result = runPm2Command(['start', tempConfigPath, '--only', serviceName]);
      log('info', `启动服务: ${serviceName}`);
      return result;
    } else {
...
async function handleApi(req, res, pathname) {
  ...
  if (pathname === '/api/auth/login' && method === 'POST') {
    ...
    if (lockStatus.locked) {
      sendJson(res, 429, { error: `登录失败次数过多，请 ${lockStatus.remaining} 秒后重试` });
      return;
    }
  ...
  if (pathname === '/api/config' && method === 'PUT') {
    ...
    sendJson(res, 200, { success: true, message: '配置已保存，需要重启服务才能生效' });
    return;
  }
  ...
  if (pathname === '/api/config/regenerate-jwt' && method === 'POST') {
    ...
    sendJson(res, 200, { success: true, message: 'JWT 密钥已重新生成，需要重启服务才能生效' });
    return;
  }
