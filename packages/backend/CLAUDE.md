...
**Never `import type` for classes used in constructor injection.** The decorator metadata gets stripped, breaking DI. Any `organizeImports`-style tool or the AI can convert a plain `import` to `import type` automatically — manually verify after running them.
...
pnpm test:unit                    # Unit（默认语义，与 pnpm test 一致，无 DB 可跑）
pnpm test:integration             # Integration only（需 DB，串行）
...
The whole repo (root, backend, frontend) uses **Prettier** (root `.prettierrc`). Backend does **not** use Biome — a `biome.json` was removed (never wired into scripts/CI). Formatting style follows `.prettierrc`: single quotes, trailing commas es5, semicolons always, 80 width, 2-space.
...
- **Controller ordering does NOT resolve wildcard conflicts in NestJS 11 + Express 5.** If Controller A has `GET file/*path` and Controller B (registered after A) has `GET file/:nodeId/preloading`, the wildcard in Controller A matches ALL requests regardless of controller order. **Fix:** Place specific routes (with `:param`) in the SAME controller BEFORE the wildcard route — Express registers routes within a single controller in definition order. See `MxcadFileAccessController` in `src/mxcad/infra/` for the pattern: `getFilePreloading()` and `getFileDownloadExternalRef()` are defined before `getFile()` (the `file/*path` handler). External ref files uploaded by user via the UI are fetched by the CAD engine (`mxcad-app`) through `GET file/:nodeId/*` paths automatically — the preloading endpoint `GET file/:nodeId/preloading` returns file metadata, and individual external ref files are served via `file/*path` from storage.
