...
export default (): AppConfig => {
	...
	return {
		...
		jwt: {
			...
			refreshSecret: getRequiredEnv('JWT_REFRESH_SECRET', 'dev-only-jwt-refresh-secret-change-me'),
	...
	session: {
		secret: getRequiredEnv('SESSION_SECRET', 'dev-only-session-secret-change-me'),
		maxAge:
	...
	// 存储路径配置（默认在 data/ 目录下）
	filesDataPath,
	avatarPath: resolvePath(process.env.AVATAR_PATH || path.join(filesDataPath, "avatars")),
	...
	log: {
		...
		accessEnabled:
			process.env.LOG_ACCESS_ENABLED !== "false",
  ...
  authRateLimit: {
    ...
    loginWindowSeconds: parseInt(process.env.AUTH_RATE_LIMIT_LOGIN_WINDOW_SECONDS || "60", 10),
    passwordResetMax: parseInt(process.env.AUTH_RATE_LIMIT_PASSWORD_RESET_MAX || "5", 10),
    passwordResetWindowSeconds: parseInt(process.env.AUTH_RATE_LIMIT_PASSWORD_RESET_WINDOW_SECONDS || "3600", 10),
    registerMax: parseInt(process.env.AUTH_RATE_LIMIT_REGISTER_MAX || "5", 10),
    registerWindowSeconds: parseInt(process.env.AUTH_RATE_LIMIT_REGISTER_WINDOW_SECONDS || "3600", 10),
  },

  // 审计日志配置（#207/ADR-0045：默认保留 180 天；旧变量 AUDIT_RETENTION_DAYS 兼容回退）
  audit: {
    retentionDays: parseInt(
      process.env.AUDIT_LOG_RETENTION_DAYS || process.env.AUDIT_RETENTION_DAYS || "180",
      10,
    ) || 180,
  },
  ...
  taskRun: {
    retentionDays: parseInt(
      process.env.TASK_RUN_RETENTION_DAYS || "30",
      10,
    ) || 30,
  },
	...
	adminIpWhitelist: {
		...
		trustedProxies: (
			process.env.ADMIN_TRUSTED_PROXY_IPS || "127.0.0.1,::1"
		)
			.split(",")
			.map((ip) => ip.trim())
			.filter(Boolean),
  ...
  // 批量下载配置
  batchDownload: {
    exportDir: resolvePath(process.env.BATCH_DOWNLOAD_EXPORT_DIR || "data/exports"),
    minDiskSpace: parseInt(process.env.BATCH_DOWNLOAD_MIN_DISK_SPACE || "524288000", 10) || 524288000, // 500MB
    zipRetentionHours: parseInt(process.env.BATCH_DOWNLOAD_ZIP_RETENTION_HOURS || "24", 10) || 24,
    dbRetentionDays: parseInt(process.env.BATCH_DOWNLOAD_DB_RETENTION_DAYS || "7", 10) || 7,
    maxConcurrency: parseInt(process.env.BATCH_DOWNLOAD_MAX_CONCURRENCY || "3", 10) || 3,
    // 批量转换是否委托 conversion-service 服务（默认关闭，走进程内 mxcad conversionService）
    delegateWorkflow: parseBoolean(process.env.BATCH_DOWNLOAD_DELEGATE_WORKFLOW, false),
    // conversion-service 服务地址
    conversionServiceUrl: process.env.CONVERSION_SERVICE_URL || "http://localhost:3100",
    // workflow 任务轮询间隔（毫秒）
    workflowPollIntervalMs: parseInt(process.env.BATCH_DOWNLOAD_WORKFLOW_POLL_INTERVAL_MS || "1500", 10) || 1500,
    // workflow 任务轮询超时（毫秒）
    workflowTimeoutMs: parseInt(process.env.BATCH_DOWNLOAD_WORKFLOW_TIMEOUT_MS || "600000", 10) || 600000,
  },
