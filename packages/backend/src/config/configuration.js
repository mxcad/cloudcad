...
export default () => {
    ...
    return {
        ...
        database: {
            ...
            connectionTimeoutMillis: parseInt(process.env.DB_CONNECTION_TIMEOUT || "30000", 10) || 30000,
            idleTimeoutMillis: parseInt(process.env.DB_IDLE_TIMEOUT || "30000", 10) || 30000,
        },
        redis: {
        ...
        session: {
            ...
            cookieSameSite: process.env.SESSION_COOKIE_SAME_SITE ||
                "lax",
        ...
        cache: {
            ...
            versionMaxAge: parseInt(process.env.CACHE_VERSION_MAX_AGE || "3600000", 10) ||
                60 * 60 * 1000, // 1小时
        },
        userCleanup: {
        ...
        mxcad: {
            assemblyPath: resolvePath(process.env.MXCAD_ASSEMBLY_PATH ||
                (isWindows
            ...
            fontsPath: resolvePath(process.env.MXCAD_FONTS_PATH ||
                (isWindows
        ...
        // 字体配置
        fonts: {
            backendPath: resolvePath(process.env.MXCAD_FONTS_PATH ||
                (isWindows
                    ? "runtime/windows/mxcad/fonts"
                    : "runtime/linux/mxcad/fonts")),
            frontendPath: resolvePath(process.env.FRONTEND_FONTS_PATH || "frontend/dist/mxcadAppAssets/fonts"),
        },
        // 存储路径配置（默认在 data/ 目录下）
        ...
        // 文件限制配置
        fileLimits: {
            zipMaxTotalSize: parseInt(process.env.FILE_LIMIT_ZIP_MAX_TOTAL_SIZE || "2147483648", 10) ||
                2 * 1024 * 1024 * 1024, // 2GB
            zipMaxFileCount: parseInt(process.env.FILE_LIMIT_ZIP_MAX_FILE_COUNT || "10000", 10) ||
                10000,
            zipMaxDepth: parseInt(process.env.FILE_LIMIT_ZIP_MAX_DEPTH || "50", 10) || 50,
            zipMaxSingleFileSize: parseInt(process.env.FILE_LIMIT_ZIP_MAX_SINGLE_FILE_SIZE || "524288000", 10) || 500 * 1024 * 1024, // 500MB
        ...
        timeout: {
            ...
            directoryAllocator: parseInt(process.env.TIMEOUT_DIRECTORY_ALLOCATOR || "300000", 10) ||
                300000, // 5分钟
        },
        // 产品信息配置
        product: {
            name: process.env.PRODUCT_NAME || "CloudCAD",
            defaultSender: process.env.PRODUCT_DEFAULT_SENDER || "CloudCAD <noreply@cloudcad.com>",
        },
        // 缓存预热配置
        cacheWarmup: {
        ...
        sms: {
            ...
            limits: {
                // 每个手机号每日发送上限，默认 10 次
                dailyLimitPerPhone: parseInt(process.env.SMS_DAILY_LIMIT_PER_PHONE || "10", 10),
                // 每个 IP 每小时发送上限，默认 20 次
                hourlyLimitPerIp: parseInt(process.env.SMS_HOURLY_LIMIT_PER_IP || "20", 10),
            },
        },
        // 协同服务配置
        cooperate: {
        ...
        thumbnail: {
            ...
            dwg2JpgPath: resolvePath(process.env.MXCAD_DWG2JPG_PATH ||
                (isWindows
                    ? "runtime/windows/mxcad/tool/MxWebDwg2Jpg.exe"
                    : "runtime/linux/mxcad/tool/MxWebDwg2Jpg")),
            // 是否启用后端自动生成缩略图
            autoGenerateEnabled: parseBoolean(process.env.THUMBNAIL_AUTO_GENERATE_ENABLED, true),
            // 缩略图宽度（像素），默认 120
 ...
 * @returns 日志级别数组
 */
function parseLogLevels(value, nodeEnv) {
    if (value) {
        return value
            .split(",")
            .map((l) => l.trim());
