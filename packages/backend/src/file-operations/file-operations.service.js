...
let FileOperationsService = (() => {
    ...
    var FileOperationsService = _classThis = class {
        constructor(prisma, storageManager, configService, versionControlService, storageInfoService, fileTreeService, storageProvider, projectPermissionService, permissionService) {
            this.prisma = prisma;
        ...
        async getParentProjectId(nodeId) {
            const node = await this.prisma.fileSystemNode.findUnique({
                where: { id: nodeId },
            ...
            try {
                ...
                if (!node.isRoot && node.parentId) {
                    ...
                    if (libraryKey) {
                        ...
                        const hasPermission = await this.permissionService.checkSystemPermission(userId, requiredPermission);
                        if (!hasPermission) {
                        ...
                        if (parentProjectId) {
                            const hasPermission = await this.projectPermissionService.checkPermission(userId, parentProjectId, ProjectPermission.FILE_OPEN);
                            if (!hasPermission) {
        ...
        async copyNodeRecursive(sourceNodeId, targetParentId, newName, ownerId) {
            ...
            if (sourceNode.isFolder && sourceNode.children.length > 0) {
                ...
                for (const child of sourceNode.children) {
                    ...
                    // 生成唯一名称
                    while (existingNamesSet.has(childUniqueName) ||
        ...
        async deleteFileIfNotReferenced(tx, nodeId, nodePath, fileHash) {
            if (!nodePath)
                return;
            ...
            if (fileHash) {
                ...
                if (otherRefCount > 0) {
                    this.logger.warn(`跳过物理文件删除: fileHash ${fileHash} 仍被 ${otherRefCount} 个其他节点引用, nodePath: ${nodePath}`);
                    return;
                }
            }
        ...
        async collectChildNodes(nodeId, nodesToCollect) {
            const children = await this.prisma.fileSystemNode.findMany({
                where: { parentId: nodeId },
                select: { id: true },
            });
