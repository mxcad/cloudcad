...
describe("FileOperationsService", () => {
	...
	describe("generateUniqueName", () => {
		it("should return original name when no conflict exists", async () => {
			...
			const result = await service.generateUniqueName(
				"parent-1",
				"myfile.dwg",
				false,
			);
	...
	describe("deleteProject", () => {
		...
		it("should throw when attempting to delete personal space", async () => {
			...
			await expect(service.deleteProject("ps-1")).rejects.toThrow(
				BadRequestException,
			);
		});
	});
	...
	describe("getProjectTrash", () => {
		...
		it("should throw when project does not exist", async () => {
			prisma.fileSystemNode.findUnique.mockResolvedValue(null);
			await expect(
				service.getProjectTrash("nonexistent", "user-1"),
			).rejects.toThrow(NotFoundException);
		});
	});
	...
	describe("moveNode", () => {
		...
		it("should throw when node does not exist", async () => {
			...
			await expect(service.moveNode("nonexistent", "tgt")).rejects.toThrow(
				NotFoundException,
			);
		});
		...
		it("should throw when attempting to move root node", async () => {
			...
			await expect(service.moveNode("root", "tgt")).rejects.toThrow(
				BadRequestException,
			);
		});
		...
		it("should throw when target parent does not exist", async () => {
			prisma.fileSystemNode.findUnique
				.mockResolvedValueOnce(srcNode)
				.mockResolvedValueOnce(null);
			await expect(service.moveNode("node-1", "bad")).rejects.toThrow(
				NotFoundException,
			);
		});
		...
		it("should throw when target parent is not a folder", async () => {
			...
			await expect(service.moveNode("node-1", "file")).rejects.toThrow(
				BadRequestException,
			);
		});
		...
		it("should throw when moving node to itself", async () => {
			prisma.fileSystemNode.findUnique
				.mockResolvedValueOnce(srcNode)
				.mockResolvedValueOnce(srcNode);
			await expect(service.moveNode("node-1", "node-1")).rejects.toThrow(
				BadRequestException,
			);
		});
	...
	describe("copyNode", () => {
		...
		it("should throw when node does not exist", async () => {
			prisma.fileSystemNode.findUnique.mockResolvedValueOnce(null);
			await expect(service.copyNode("nonexistent", "tgt")).rejects.toThrow(
				NotFoundException,
			);
		});
		...
		it("should throw when attempting to copy root node", async () => {
			...
			await expect(service.copyNode("root", "tgt")).rejects.toThrow(
				BadRequestException,
			);
		});
		...
		it("should throw when target parent does not exist", async () => {
			prisma.fileSystemNode.findUnique
				.mockResolvedValueOnce(srcNode)
				.mockResolvedValueOnce(null);
			await expect(service.copyNode("node-1", "bad")).rejects.toThrow(
				NotFoundException,
			);
		});
		...
		it("should throw when target parent is not a folder", async () => {
			prisma.fileSystemNode.findUnique
				.mockResolvedValueOnce(srcNode)
				.mockResolvedValueOnce({ ...tgt, isFolder: false });
			await expect(service.copyNode("node-1", "file")).rejects.toThrow(
				BadRequestException,
			);
		});
		...
		it("should throw when copying node to itself", async () => {
			prisma.fileSystemNode.findUnique
				.mockResolvedValueOnce(srcNode)
				.mockResolvedValueOnce(srcNode);
			await expect(service.copyNode("node-1", "node-1")).rejects.toThrow(
				BadRequestException,
			);
		});
	...
	describe("copyNodeRecursive", () => {
		...
		it("should throw when source node does not exist", async () => {
			prisma.fileSystemNode.findUnique.mockResolvedValue(null);
			await expect(
				service.copyNodeRecursive("bad", "tgt", "name", "u1"),
			).rejects.toThrow(NotFoundException);
		});
	});
	...
	describe("permanentlyDeleteTrashItems", () => {
		...
		it("should throw when no deleted items found", async () => {
			prisma.fileSystemNode.findMany.mockResolvedValue([]);
			await expect(
				service.permanentlyDeleteTrashItems(["bad"]),
			).rejects.toThrow(NotFoundException);
		});
	});
	...
	describe("updateNode", () => {
		...
		it("should check name uniqueness when name changes", async () => {
			...
			prisma.fileSystemNode.findFirst.mockRejectedValue(
				new BadRequestException("同名文件"),
			);
			await expect(
				service.updateNode("n1", { name: "dupe.dwg" }),
			).rejects.toThrow(BadRequestException);
		});
		...
		it("should throw when file extension is changed", async () => {
			prisma.fileSystemNode.findUnique.mockResolvedValueOnce(node);
			await expect(
				service.updateNode("n1", { name: "new.pdf" }),
			).rejects.toThrow(BadRequestException);
		});
		...
		it("should throw when node does not exist", async () => {
			prisma.fileSystemNode.findUnique.mockResolvedValueOnce(null);
			await expect(service.updateNode("bad", { name: "x" })).rejects.toThrow(
				NotFoundException,
			);
		});
	});
