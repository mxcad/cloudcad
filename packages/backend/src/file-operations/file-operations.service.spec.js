...
describe("FileOperationsService", () => {
    ...
    beforeEach(async () => {
        ...
        const module = await Test.createTestingModule({
            ...
            .compile();
        service = module.get(FileOperationsService);
    ...
    describe("checkNameUniqueness", () => {
        it("should pass when project name is unique at root level", async () => {
            ...
            await expect(service.checkNameUniqueness("UniqueProject", "user-1", null)).resolves.toBeUndefined();
        ...
        it("should throw when duplicate project name exists at root level", async () => {
            ...
            await expect(service.checkNameUniqueness("ExistingProject", "user-1", null)).rejects.toThrow(BadRequestException);
        });
        it("should pass when file name is unique in folder", async () => {
            prisma.fileSystemNode.findFirst.mockResolvedValue(null);
            await expect(service.checkNameUniqueness("unique.dwg", "user-1", "parent-1")).resolves.toBeUndefined();
            expect(prisma.fileSystemNode.findFirst).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it("should throw when duplicate file name exists in folder", async () => {
            ...
            await expect(service.checkNameUniqueness("duplicate.dwg", "user-1", "parent-1")).rejects.toThrow(/同名文件/);
        });
        it("should throw when duplicate folder name exists in folder", async () => {
            ...
            await expect(service.checkNameUniqueness("duplicate", "user-1", "parent-1")).rejects.toThrow(/同名文件夹/);
        });
    ...
    describe("restoreNode", () => {
        ...
        it("should throw when node does not exist", async () => {
            prisma.fileSystemNode.findUnique.mockResolvedValue(null);
            await expect(service.restoreNode("nonexistent", "user-1")).rejects.toThrow(NotFoundException);
        });
        it("should throw when node is not deleted", async () => {
            ...
            await expect(service.restoreNode("node-1", "user-1")).rejects.toThrow(BadRequestException);
        });
        it("should throw when parent node is deleted", async () => {
            prisma.fileSystemNode.findUnique
                .mockResolvedValueOnce(deletedNode)
                .mockResolvedValueOnce({ deletedAt: new Date() });
            await expect(service.restoreNode("node-1", "user-1")).rejects.toThrow(/父节点已被删除/);
        });
        it("should throw when parent node does not exist", async () => {
            prisma.fileSystemNode.findUnique
                .mockResolvedValueOnce(deletedNode)
                .mockResolvedValueOnce(null);
            await expect(service.restoreNode("node-1", "user-1")).rejects.toThrow(NotFoundException);
        });
    ...
    describe("getProjectTrash", () => {
        it("should return deleted nodes with pagination", async () => {
            ...
            prisma.fileSystemNode.findMany.mockResolvedValue([
                { id: "deleted-1", name: "old.dwg" },
            ]);
            prisma.fileSystemNode.count.mockResolvedValue(1);
            const result = await service.getProjectTrash("proj-1", "user-1", {
                page: 1,
                limit: 20,
            });
        ...
        it("should filter by search term", async () => {
            ...
            await service.getProjectTrash("proj-1", "user-1", {
                search: "drawing",
                page: 1,
                limit: 20,
            });
    ...
    describe("moveNode", () => {
        ...
        it("should throw when attempting to move root node", async () => {
            prisma.fileSystemNode.findUnique.mockResolvedValueOnce({
                ...srcNode,
                isRoot: true,
            });
        ...
        it("should throw when target parent is not a folder", async () => {
            prisma.fileSystemNode.findUnique
                .mockResolvedValueOnce(srcNode)
                .mockResolvedValueOnce({ ...tgt, isFolder: false });
    ...
    describe("copyNode", () => {
        ...
        it("should copy file to another folder", async () => {
            prisma.fileSystemNode.findUnique
                ...
                .mockResolvedValueOnce(tgt);
            prisma.fileSystemNode.findFirst.mockResolvedValue(null);
            prisma.fileSystemNode.create.mockResolvedValue({
                id: "copied",
                ...srcNode,
            });
    ...
    describe("softDeleteDescendants", () => {
        it("should soft delete all child nodes via transaction", async () => {
            ...
            prisma.fileSystemNode.findMany.mockResolvedValue([]);
            await service.softDeleteDescendants(tx, "parent");
        ...
        it("should do nothing when no children exist", async () => {
            ...
            await service.softDeleteDescendants(tx, "empty-parent");
        });
    });
    describe("deleteDescendantsWithFiles", () => {
        it("should delete all child nodes and their files via transaction", async () => {
            ...
            await service.deleteDescendantsWithFiles(tx, "parent");
        ...
        it("should do nothing when no children exist", async () => {
            ...
            await service.deleteDescendantsWithFiles(tx, "empty");
    ...
    describe("restoreTrashItems", () => {
        ...
        it("should throw when no deleted items found", async () => {
            prisma.fileSystemNode.findMany.mockResolvedValue([]);
            await expect(service.restoreTrashItems(["bad"], "user-1")).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe("updateNode", () => {
        ...
        it("should update node name", async () => {
            prisma.fileSystemNode.findUnique
                ...
                .mockResolvedValueOnce({ ...node, name: "new.dwg" });
            prisma.fileSystemNode.findFirst.mockResolvedValue(null);
            prisma.fileSystemNode.update.mockResolvedValue({
                ...node,
                name: "new.dwg",
        ...
        it("should update node description", async () => {
            prisma.fileSystemNode.findUnique
                .mockResolvedValueOnce(node)
                .mockResolvedValueOnce({ ...node, description: "desc" });
            prisma.fileSystemNode.update.mockResolvedValue({
                ...node,
                description: "desc",
            });
        ...
        it("should preserve extension when new name has no extension", async () => {
            prisma.fileSystemNode.findUnique
                .mockResolvedValueOnce(node)
                .mockResolvedValueOnce({ ...node, name: "newname.dwg" });
            prisma.fileSystemNode.findFirst.mockResolvedValue(null);
            prisma.fileSystemNode.update.mockResolvedValue({
                ...node,
                name: "newname.dwg",
...
//# sourceMappingURL=file-operations.service.spec.js.map