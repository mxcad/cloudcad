...
describe("ProjectCrudService", () => {
    ...
    beforeEach(async () => {
        ...
        const module = await Test.createTestingModule({
            ...
            .compile();
        service = module.get(ProjectCrudService);
    ...
    describe("createNode", () => {
        ...
        it("should throw when parent node does not exist", async () => {
            ...
            await expect(service.createNode(userId, "Test", { parentId: "invalid-id" })).rejects.toThrow(NotFoundException);
        });
        it("should throw when parent is not a folder", async () => {
            ...
            await expect(service.createNode(userId, "Test", { parentId: "node-1" })).rejects.toThrow(BadRequestException);
        });
        it("should throw when PROJECT_OWNER role does not exist", async () => {
            prisma.projectRole.findFirst.mockResolvedValue(null);
            await expect(service.createNode(userId, projectName)).rejects.toThrow(InternalServerErrorException);
        });
        it("should throw when project name is not unique", async () => {
            fileOperationsService.checkNameUniqueness.mockRejectedValue(new BadRequestException("项目名称已存在"));
            await expect(service.createNode(userId, projectName)).rejects.toThrow("项目名称已存在");
        });
        it("should throw when folder name is not unique", async () => {
            ...
            fileOperationsService.checkNameUniqueness.mockRejectedValue(new BadRequestException("文件夹名称已存在"));
            await expect(service.createNode(userId, "Folder", { parentId })).rejects.toThrow("文件夹名称已存在");
        });
    });
    ...
    describe("createFolder", () => {
        ...
        it("should create a new folder", async () => {
            ...
            expect(result).toEqual(mockFolder);
            expect(fileOperationsService.checkNameUniqueness).toHaveBeenCalledWith(folderName, userId, parentId);
    ...
    describe("getUserProjects", () => {
        ...
        it("should return projects owned by user when filter is owned", async () => {
            ...
            expect(prisma.fileSystemNode.findMany).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it("should return paginated results", async () => {
            ...
            expect(prisma.fileSystemNode.findMany).toHaveBeenCalledWith(expect.objectContaining({ skip: 10, take: 10 }));
        });
        ...
        it("should sort by specified field and order", async () => {
            ...
            expect(prisma.fileSystemNode.findMany).toHaveBeenCalledWith(expect.objectContaining({ orderBy: { name: "asc" } }));
        });
        it("should sort by updatedAt desc by default", async () => {
            await service.getUserProjects(userId);
            expect(prisma.fileSystemNode.findMany).toHaveBeenCalledWith(expect.objectContaining({ orderBy: { updatedAt: "desc" } }));
        });
    ...
    describe("getUserDeletedProjects", () => {
        ...
        it("should sort by deletedAt desc by default", async () => {
            ...
            expect(prisma.fileSystemNode.findMany).toHaveBeenCalledWith(expect.objectContaining({ orderBy: { deletedAt: "desc" } }));
        });
        it("should exclude personal space and library projects", async () => {
            await service.getUserDeletedProjects(userId);
            expect(prisma.fileSystemNode.findMany).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe("getProject", () => {
        ...
        it("should exclude library projects", async () => {
            ...
            expect(prisma.fileSystemNode.findFirst).toHaveBeenCalledWith(expect.objectContaining({
                where: expect.objectContaining({ libraryKey: null }),
            }));
        });
        it("should only return non-deleted children", async () => {
            ...
            const call = prisma.fileSystemNode.findFirst.mock.calls[0][0];
    ...
    describe("updateProject", () => {
        ...
        it("should update project status", async () => {
            ...
            await service.updateProject("p1", { status: "ARCHIVED" });
            expect(prisma.fileSystemNode.update).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe("getStoragePath", () => {
        it("should return full path for node with path", () => {
            ...
            expect(service.getStoragePath({ path: "rel/path" })).toBe("/abs/path");
        });
        it("should throw when node has no path", () => {
            expect(() => service.getStoragePath({ path: null })).toThrow(NotFoundException);
        });
    });
    describe("getFullPath", () => {
        ...
        it("should throw when nodePath is null", () => {
            expect(() => service.getFullPath(null)).toThrow(NotFoundException);
        });
    });
...
//# sourceMappingURL=project-crud.service.spec.js.map