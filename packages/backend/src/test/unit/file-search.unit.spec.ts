...
describe('文件搜索 + 权限过滤 + 分页 链路单元测试', () => {
  ...
  beforeEach(async () => {
    ...
    mockFtsQueryBuilder.buildSearchOrConditions.mockImplementation((keyword: string, ftsMatch: { ids: Set<string>; matched: boolean }) => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: AncestorQueryService,
          useValue: { buildAncestorPaths: jest.fn().mockResolvedValue(new Map()) },
        },
      ],
    }).compile();
  ...
  describe('测试 1: 项目文件搜索 - 权限过滤', () => {
    ...
    it('应返回有权限的文件', async () => {
      ...
      const projectId = 'project-1';
      const mockFiles = [
        { id: 'file-1', name: 'test-file-1.dwg', nodeType: NodeType.FILE },
        { id: 'file-2', name: 'test-file-2.dwg', nodeType: NodeType.FILE },
      ];
  ...
  describe('测试 4: 资源库搜索', () => {
    it('应返回图纸库搜索结果', async () => {
      ...
      const mockLibraryItems = [
        { id: 'lib-1', name: 'library-dwg-1.dwg', nodeType: 'LIBRARY_DRAWING' },
      ];
  ...
  describe('测试 5: 项目搜索 - 权限过滤', () => {
    it('应返回用户有权访问的项目', async () => {
      ...
      const userId = 'user-1';
      const mockProjects = [
        { id: 'proj-1', name: 'My Project', nodeType: 'PROJECT', _count: { children: 5, projectMembers: 3 } },
        { id: 'proj-2', name: 'Team Project', nodeType: 'PROJECT', _count: { children: 10, projectMembers: 5 } },
      ];
      ...
      expect(call[0].where.nodeType).toBe('PROJECT');
