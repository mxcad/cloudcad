...
import { SearchScope } from '../../file-system/dto/search.dto';
...
describe('文件搜索 → 权限过滤 → 分页 链路集成测试', () => {
    ...
    describe('测试 1: 项目文件搜索 - 权限过滤', () => {
        it('应返回空结果，当用户无访问权限时', async () => {
            ...
            expect(mockFileSystemPermissionService.checkNodePermission).toHaveBeenCalledWith(userId, projectId, ProjectPermission.FILE_OPEN);
        });
        it('应返回有权限的文件', async () => {
            // 准备
            const userId = 'user-1';
...
//# sourceMappingURL=file-search.integration.spec.js.map