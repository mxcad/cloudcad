 ...
 * - 批量权限检查
 */
let PermissionTestRunner = (() => {
    let _classDecorators = [Injectable()];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    var PermissionTestRunner = _classThis = class {
        constructor(prisma, permissionService, roleInheritanceService) {
            this.prisma = prisma;
            this.permissionService = permissionService;
            this.roleInheritanceService = roleInheritanceService;
            this.logger = new Logger(PermissionTestRunner.name);
        }
        /**
         * 运行完整的权限测试套件
         */
        ...
        async runTestSuite(suite) {
            ...
            this.logger.log(`测试套件 ${suite.name} 完成: ${passedTests}/${results.length} 通过`);
            return report;
        }
        /**
         * 运行单个测试用例（使用预创建的用户）
         */
        async runTestCaseWithExistingUser(testCase, testUser) {
            const startTime = Date.now();
            try {
                // 对于缓存测试，执行缓存验证逻辑
                ...
                // 检查用户是否有该权限
                const hasPermission = await this.permissionService.checkSystemPermission(testUser.id, testCase.permission);
                // 验证用户是否应该有该权限（基于角色）
                const roleHasPermission = testCase.expectedRoles.includes(testUser.role.name);
                // 验证用户是否不应该有该权限（检查意外角色）
                const roleUnexpectedlyHasPermission = testCase.unexpectedRoles?.includes(testUser.role.name);
        ...
        async runTestCase(testCase) {
            ...
            try {
                ...
                const hasPermission = await this.permissionService.checkSystemPermission(testUser.id, testCase.permission);
                // 验证期望结果
                const shouldHavePermission = testCase.expectedRoles.includes(testUser.role.name);
         ...
         * 运行缓存相关测试
         */
        async runCacheTest(userId, testCase) {
            const isFirstCheck = testCase.name.includes('缓存命中');
            const isClearCache = testCase.name.includes('缓存失效');
            // 第一次检查
            const firstCheck = await this.permissionService.checkSystemPermission(userId, testCase.permission);
            // 第二次检查（应该使用缓存）
            const secondCheck = await this.permissionService.checkSystemPermission(userId, testCase.permission);
            if (isFirstCheck) {
                // 缓存命中测试：验证两次结果一致
            ...
            if (isClearCache) {
                // 缓存失效测试：验证清除缓存后重新检查
                const thirdCheck = await this.permissionService.checkSystemPermission(userId, testCase.permission);
                // 清除缓存
                await this.permissionService.clearUserCache(userId);
                // 第四次检查（应该重新查询数据库）
                const fourthCheck = await this.permissionService.checkSystemPermission(userId, testCase.permission);
                const passed = firstCheck === secondCheck &&
         ...
         * 创建权限继承测试套件
         */
        createPermissionInheritanceTestSuite() {
            return {
                name: '权限继承测试',
         ...
         * 创建缓存一致性测试套件
         */
        createCacheConsistencyTestSuite() {
            return {
                name: '缓存一致性测试',
         ...
         * 创建批量权限测试套件
         */
        createBatchPermissionTestSuite() {
            return {
                name: '批量权限检查测试',
         ...
         * @returns 创建的测试用户数组
         */
        async batchCreateTestUsers(testCases) {
            if (testCases.length === 0) {
                return [];
            }
            // 提取所有需要的角色名称
            const roleNames = Array.from(new Set(testCases.map((tc) => tc.expectedRoles[0])));
            // 批量查询所有角色（避免 N+1 查询）
         ...
         */
        async batchCleanupTestUsers(userIds) {
            if (userIds.length === 0) {
                return;
            }
            try {
         ...
         * 导出测试报告为 JSON
         */
        exportReportAsJson(reports) {
            return JSON.stringify(reports, null, 2);
        }
        /**
         * 导出测试报告为 Markdown
         */
        exportReportAsMarkdown(reports) {
            let markdown = '# 权限测试报告\n\n';
    ...
    __setFunctionName(_classThis, "PermissionTestRunner");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        PermissionTestRunner = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return PermissionTestRunner = _classThis;
})();
export { PermissionTestRunner };
//# sourceMappingURL=permission-test-runner.js.map