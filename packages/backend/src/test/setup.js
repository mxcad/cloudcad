...
expect.extend({
    ...
    toHavePermission(received, permission) {
        ...
        if (pass) {
            return {
                message: () => `expected permissions ${received} not to include ${permission}`,
                pass: true,
            };
        }
        else {
            return {
                message: () => `expected permissions ${received} to include ${permission}`,
                pass: false,
            };
        }
    },
...
global.createTestProject = (overrides = {}) => ({
    id: global.generateId(),
    name: "Test Project",
...
global.createTestFile = (overrides = {}) => ({
    id: global.generateId(),
    name: "test-file.dwg",
    size: 1024,
