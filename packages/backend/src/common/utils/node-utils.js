...
export class NodeUtils {
     ...
     * @returns 唯一标识符
     */
    static generateNodeIdentifier(fileHash, ownerId) {
     ...
     * @returns 锁名称
     */
    static generateLockName(type, identifier) {
     ...
     * @returns 存储目录名称
     */
    static generateStorageDirectoryName(nodeId, fileHash) {
        return `${nodeId}`;
    }
    /**
     * 检查节点状态是否允许操作
     ...
     * @returns 是否允许
     */
    static canPerformOperation(fileStatus, operation) {
        switch (operation) {
            case 'read':
                // 所有非删除状态都可以读取
     ...
     * @returns 唯一文件名
     */
    static generateUniqueFileName(baseName, extension, existingNames) {
     ...
     * @returns 相对路径
     */
    static buildRelativePath(directory, nodeId, fileName) {
     ...
     */
    static extractNodeIdFromPath(relativePath) {
     ...
     * @returns 是否支持
     */
    static isSupportedCADFile(extension) {
     ...
     * @returns 是否支持
     */
    static isSupportedImageFile(extension) {
     ...
     * @param action 操作类型
     */
    static logNodeOperation(nodeId, name, fileHash, action) {
...
NodeUtils.logger = new Logger(NodeUtils.name);
