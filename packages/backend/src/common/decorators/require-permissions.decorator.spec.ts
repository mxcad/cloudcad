...
import { RequirePermissions, PermissionCheckMode, PERMISSIONS_KEY, PERMISSIONS_MODE_KEY } from './require-permissions.decorator';
...
describe('RequirePermissions Decorator', () => {
  describe('方法装饰器', () => {
    it('应该正确设置权限元数据到方法', () => {
      ...
      const metadata = Reflect.getMetadata(PERMISSIONS_KEY, TestController.prototype, 'testMethod');
      expect(metadata).toEqual([SystemPermission.SYSTEM_USER_READ]);
    });

    it('应该正确设置权限检查模式元数据到方法', () => {
      class TestController {
        @RequirePermissions([SystemPermission.SYSTEM_USER_READ, SystemPermission.SYSTEM_USER_CREATE], PermissionCheckMode.ANY)
        testMethod() {}
      }

      const mode = Reflect.getMetadata(PERMISSIONS_MODE_KEY, TestController.prototype, 'testMethod');
  ...
  describe('类装饰器', () => {
    ...
    it('应该正确设置权限检查模式元数据到类', () => {
      @RequirePermissions([SystemPermission.SYSTEM_ADMIN], PermissionCheckMode.ANY)
      class TestController {}
  ...
  describe('边界情况', () => {
    ...
    it('方法装饰器和类装饰器应该可以同时使用', () => {
      ...
      const methodMetadata = Reflect.getMetadata(PERMISSIONS_KEY, TestController.prototype, 'testMethod');
    ...
    it('方法装饰器应该覆盖类装饰器的模式', () => {
      @RequirePermissions([SystemPermission.SYSTEM_ADMIN], PermissionCheckMode.ALL)
      class TestController {
        @RequirePermissions([SystemPermission.SYSTEM_USER_READ], PermissionCheckMode.ANY)
        testMethod() {}
      }

      const classMode = Reflect.getMetadata(PERMISSIONS_MODE_KEY, TestController);
      const methodMode = Reflect.getMetadata(PERMISSIONS_MODE_KEY, TestController.prototype, 'testMethod');
  ...
  describe('回归测试 - 修复的问题', () => {
    it('修复：方法装饰器元数据应该被正确设置（使用 Reflect.defineMetadata）', () => {
      ...
      // 验证元数据确实被设置到方法上
      const hasMetadata = Reflect.hasMetadata(PERMISSIONS_KEY, TestController.prototype, 'testMethod');
    ...
    it('修复：方法装饰器模式元数据应该被正确设置', () => {
      ...
      // 验证模式元数据也被正确设置
      const hasModeMetadata = Reflect.hasMetadata(PERMISSIONS_MODE_KEY, TestController.prototype, 'testMethod');
