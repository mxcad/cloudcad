    ...
    switch (exception.code) {
      case 'P2002': {
        ...
        if (Array.isArray(targets) && targets.length > 0) {
          message = i18n?.t('error.database.unique_constraint', { args: { fields: targets.join(', ') } }) ?? `字段 ${targets.join(', ')} 已存在`;
        } else {
          message = i18n?.t('error.database.unique_constraint', { args: { fields: '' } }) ?? '数据已存在，请勿重复提交';
        }
        break;
      }
      case 'P2003':
        status = HttpStatus.BAD_REQUEST;
        message = i18n?.t('error.database.foreign_key_constraint') ?? '数据关联错误，请检查关联数据是否存在';
        code = 'FOREIGN_KEY_CONSTRAINT';
        break;
      case 'P2004':
        status = HttpStatus.BAD_REQUEST;
        message = i18n?.t('error.database.constraint_failed') ?? '数据库约束校验失败';
      ...
      case 'P2025':
        ...
        message = (I18nContext.current()?.t('error.resource.not_found') ?? '请求的资源不存在');
        code = 'NOT_FOUND';
        break;
      default:
        this.logger.error(
