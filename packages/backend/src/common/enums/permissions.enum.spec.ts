...
describe('权限枚举', () => {
  ...
  describe('SYSTEM_ROLE_PERMISSIONS', () => {
    ...
    it('USER_MANAGER 角色应该拥有用户管理权限', () => {
      const userManagerPermissions = SYSTEM_ROLE_PERMISSIONS[SystemRole.USER_MANAGER];
      expect(userManagerPermissions).toContain(SystemPermission.SYSTEM_USER_READ);
      expect(userManagerPermissions).toContain(SystemPermission.SYSTEM_USER_CREATE);
      expect(userManagerPermissions).toContain(SystemPermission.SYSTEM_USER_UPDATE);
      expect(userManagerPermissions).toContain(SystemPermission.SYSTEM_USER_DELETE);
    });

    it('FONT_MANAGER 角色应该拥有字体管理权限', () => {
      const fontManagerPermissions = SYSTEM_ROLE_PERMISSIONS[SystemRole.FONT_MANAGER];
      expect(fontManagerPermissions).toContain(SystemPermission.SYSTEM_FONT_READ);
      expect(fontManagerPermissions).toContain(SystemPermission.SYSTEM_FONT_UPLOAD);
      expect(fontManagerPermissions).toContain(SystemPermission.SYSTEM_FONT_DELETE);
      expect(fontManagerPermissions).toContain(SystemPermission.SYSTEM_FONT_DOWNLOAD);
    });
  ...
  describe('SYSTEM_ROLE_HIERARCHY', () => {
    ...
    it('USER_MANAGER 应该继承自 USER', () => {
      expect(SYSTEM_ROLE_HIERARCHY[SystemRole.USER_MANAGER]).toBe(SystemRole.USER);
    });

    it('FONT_MANAGER 应该继承自 USER', () => {
      expect(SYSTEM_ROLE_HIERARCHY[SystemRole.FONT_MANAGER]).toBe(SystemRole.USER);
    });
  });
  ...
  describe('DEFAULT_PROJECT_ROLE_PERMISSIONS', () => {
    it('OWNER 角色应该拥有所有项目权限', () => {
      ...
      expect(ownerPermissions).toContain(ProjectPermission.PROJECT_MEMBER_MANAGE);
    ...
    it('ADMIN 角色应该拥有大部分项目权限（除了删除和转让）', () => {
      ...
      expect(adminPermissions).not.toContain(ProjectPermission.PROJECT_TRANSFER);
    });
    ...
    it('修复：所有项目角色的权限数组不应该有重复项', () => {
      ...
      for (const [role, permissions] of Object.entries(DEFAULT_PROJECT_ROLE_PERMISSIONS)) {
        const uniquePermissions = new Set(permissions);
        const hasDuplicates = permissions.length !== uniquePermissions.size;
        expect(hasDuplicates).toBe(false);
      }
    ...
    it('权限应该按照层级递减', () => {
      ...
      const ownerPerms = new Set(DEFAULT_PROJECT_ROLE_PERMISSIONS[ProjectRole.OWNER]);
      const adminPerms = new Set(DEFAULT_PROJECT_ROLE_PERMISSIONS[ProjectRole.ADMIN]);
      const memberPerms = new Set(DEFAULT_PROJECT_ROLE_PERMISSIONS[ProjectRole.MEMBER]);
      const editorPerms = new Set(DEFAULT_PROJECT_ROLE_PERMISSIONS[ProjectRole.EDITOR]);
      const viewerPerms = new Set(DEFAULT_PROJECT_ROLE_PERMISSIONS[ProjectRole.VIEWER]);
