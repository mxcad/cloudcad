...
const mockConfigService = {
    get: (key, opts) => {
        if (opts?.infer && mockConfig[key] !== undefined)
...
<logentry revision="1"><author>admin</author><date>2024-01-01T10:00:00.000000Z</date><msg>first</msg></logentry>
    ...
    describe('executeWithLockRetry', () => {
        it('retries on locked error and succeeds', async () => {
            let calls = 0;
            const op = () => { calls++; if (calls === 1)
                throw new Error('E155004: Locked'); return 'ok'; };
            expect(await service.executeWithLockRetry(op, 't')).toBe('ok');
            expect(calls).toBe(2);
        });
        it('throws non-lock errors directly', async () => {
            await expect(service.executeWithLockRetry(() => { throw new Error('Other'); }, 't')).rejects.toThrow('Other');
        });
    });
...
//# sourceMappingURL=version-control.service.spec.js.map