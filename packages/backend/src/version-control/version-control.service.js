...
let VersionControlService = (() => {
    ...
    let _classThis;
    var VersionControlService = _classThis = class {
        constructor(configService) {
            this.configService = configService;
            this.logger = new Logger(VersionControlService.name);
         ...
         */
        async commitNodeDirectory(nodeDirectory, message, userId, userName) {
            ...
            try {
                ...
                // 提交目标目录（递归）
                const result = await svnCommitAsync([nodeDirectory], fullMessage, true, // 递归提交
                null, null);
         ...
         */
        async deleteNodeDirectory(nodeDirectory) {
            await this.ensureInitialized();
            if (!this.isInitialized) {
            ...
            try {
                // 从 SVN 删除目录（仅标记删除）
                const result = await svnDeleteAsync([nodeDirectory], true, // 递归删除
        ...
        async commitWorkingCopy(message) {
            ...
            try {
                ...
                const result = await svnCommitAsync([filesDataPath], message, true, // 递归提交
                null, null);
        ...
        async getFileHistory(filePath, limit) {
            ...
            try {
                ...
                const xmlResult = await svnLogAsync(repoUrl, limit || 50, // 默认返回 50 条记录
                true, // 详细模式，显示变更的文件列表
                null, null);
                // 解析 XML 结果
    ...
    __setFunctionName(_classThis, "VersionControlService");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        VersionControlService = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return VersionControlService = _classThis;
})();
