...
export class L1CacheProvider<T = unknown> implements IL1CacheManager<T> {
  ...
  /**
   * 根据模式删除缓存
   * 遍历 Map 匹配 pattern（支持 * 和 ? 通配符，与 L2 的 Redis glob 语法一致）
   */
  async deleteByPattern(pattern: string): Promise<number> {
    ...
    if (deleted > 0) {
      this.logger.debug(`根据模式删除了 ${deleted} 条 L1 缓存: ${pattern}`);
    }
    return deleted;
  }
  ...
  setMaxCapacity(capacity: number): void {
    if (capacity < 1) {
      throw new BadRequestException(I18nContext.current()?.t('error.cache.capacity_must_be_positive') ?? '容量必须大于 0');
    }
  ...
  /**
   * 将 glob 模式转换为正则表达式（支持 * 和 ? 通配符，与 Redis glob 语法一致）
   */
