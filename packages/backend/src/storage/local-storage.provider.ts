...
export class LocalStorageProvider {
  ...
  private validatePath(key: string): void {
    if (!key || typeof key !== 'string') {
      throw new BadRequestException(I18nContext.current()?.t('error.storage.path_invalid') ?? '无效的路径');
    }

    // 检查路径遍历攻击
    if (key.includes('..') || key.includes('~')) {
      this.logger.error(`[validatePath] 路径包含 .. 或 ~: ${key}`);
      throw new BadRequestException(I18nContext.current()?.t('error.file.path_illegal_chars') ?? '路径包含非法字符');
    }

    // 检查绝对路径（MxCAD-App 访问路径例外）
    if (path.isAbsolute(key) && !key.startsWith('/mxcad/file/')) {
      this.logger.error(`[validatePath] 不允许使用绝对路径: ${key}`);
      throw new BadRequestException(I18nContext.current()?.t('error.storage.absolute_path_not_allowed') ?? '不允许使用绝对路径');
    }
    ...
    // 检查非法字符（Windows 不允许的字符），替换为下划线而非拒绝
    const illegalChars = /[<>:"|?*]/;
    if (illegalChars.test(key)) {
      this.logger.warn(`[validatePath] 路径包含 Windows 非法字符，已自动替换: ${key}`);
      key = key.replace(illegalChars, '_');
    }
    ...
    if (key.length > MAX_PATH_LENGTH) {
      throw new BadRequestException(I18nContext.current()?.t('error.storage.path_too_long') ?? '路径过长');
    }
  ...
  async downloadFile(key: string): Promise<Buffer> {
    try {
      ...
      if (!(await this.fileExists(key))) {
        throw new NotFoundException(I18nContext.current()?.t('error.file_extra.file_not_exist_detail', { args: { key } }) ?? `文件不存在: ${key}`);
      }
  ...
  async getFileStream(key: string): Promise<NodeJS.ReadableStream> {
    try {
      ...
      if (stats.isDirectory()) {
        throw new BadRequestException(I18nContext.current()?.t('error.file_extra.path_is_directory_not_file', { args: { key } }) ?? `路径是目录而非文件: ${key}`);
      }
