...
describe('RoleInheritanceService', () => {
  ...
  describe('isInheritedFrom', () => {
    ...
    it('should return true when parent role is in ancestor chain', async () => {
      ...
      // Two findFirst calls: one for child (with parent), one for parent (no parent)
      mockPrisma.role.findFirst
        .mockResolvedValueOnce({
          name: childRole,
          parentId: 'parent-id',
  ...
  describe('clearRoleCacheRecursive', () => {
    ...
    it('should clear cache for role and all child roles', async () => {
      // Find current role (use mockResolvedValueOnce to avoid infinite recursion)
      mockPrisma.role.findFirst.mockResolvedValueOnce({
        id: 'role-admin',
      });
      // Find children (use mockResolvedValueOnce so recursive calls return empty)
