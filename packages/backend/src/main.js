...
async function bootstrap() {
    ...
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Redis 连接超时')), 5000));
    ...
    server.use('/api/cooperate', createProxyMiddleware({
    ...
    app.use((req, res, next) => {
        ...
        res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' data:; connect-src 'self' ws: wss:");
        next();
    });
    // 配置 Tus 上传端点
    try {
        ...
        logger.warn('⚠️ Tus 服务挂载失败:', error.message);
    }
    // 配置 Swagger 文档
