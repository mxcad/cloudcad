...
let ThumbnailGenerationService = (() => {
    ...
    var ThumbnailGenerationService = _classThis = class {
        constructor(configService) {
            ...
            this.tempPath =
                this.configService.get('mxcadTempPath') || 'data/temp';
        }
        /**
         * 模块初始化时检查 exe 是否可用
         */
        async onModuleInit() {
            ...
            try {
                ...
                this.logger.warn(`缩略图自动生成功能将不可用: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        /**
         * 获取默认的 MxWebDwg2Jpg.exe 路径
         */
        getDefaultDwg2JpgPath() {
            ...
            return path.join(projectRoot, 'runtime', 'windows', 'mxcad', 'tool', 'MxWebDwg2Jpg.exe');
        }
        /**
         * 检查缩略图生成功能是否可用
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        this.logger.log(`${logPrefix} 缩略图生成成功: ${autoGeneratedPath} (${stats.size} bytes)`);
