...
import { getThumbnailFileName, THUMBNAIL_FORMATS, } from './thumbnail-utils';
...
let FileMergeService = (() => {
    ...
    var FileMergeService = _classThis = class {
        constructor(configService, fileSystemService, fileSystemServiceMain, fileSystemNodeService, cacheManager, storageManager, versionControlService, fileConversionService, externalRefService, uploadUtilityService, thumbnailGenerationService) {
                ...
                if (chunks === stack.length) {
                    ...
                    try {
                        ...
                        if (context && context.userId && context.nodeId) {
                            ...
                            const existingFile = existingNodes.find((node) => !node.isFolder && node.name.toLowerCase() === fileName.toLowerCase());
                            if (existingFile) {
                                if (strategy === 'skip') {
                            ...
                            if (matchingFiles.length > 0) {
                                ...
                                for (const file of matchingFiles) {
                                    ...
                                    await copyFileOrDir(sourcePath, targetPath, { fileHash: fileMd5, newNodeId });
                                ...
                                // 处理缩略图（仅 CAD 文件）
                                const isCadFile = extension.toLowerCase() === '.dwg' || extension.toLowerCase() === '.dxf';
                                if (isCadFile) {
                                    try {
                                        const nodeDirectory = storageInfo.nodeDirectoryPath;
                                        ...
                                        // 检查缓存是否存在
                                        const cacheExists = await fsPromises.access(thumbnailCachePath).then(() => true).catch(() => false);
                                        if (cacheExists) {
                                            // 从缓存拷贝
                                            this.logger.log(`[mergeConvertFile] 从缓存拷贝缩略图: ${newNodeId}`);
                                            await fsPromises.copyFile(thumbnailCachePath, thumbnailTargetPath);
                                            this.logger.log(`[mergeConvertFile] 缩略图拷贝成功: ${thumbnailTargetPath}`);
                                        }
                                            ...
                                            if (result.success) {
                                                // 拷贝到节点目录
                                                await fsPromises.copyFile(thumbnailCachePath, thumbnailTargetPath);
                                                this.logger.log(`[mergeConvertFile] 缩略图生成成功: ${thumbnailTargetPath}`);
                                            }
                                            else {
        ...
        async mergeChunksWithPermission(options) {
            ...
            if (!FileTypeDetector.isMxwebFile(name) && this.fileConversionService.needsConversion(name)) {
                const mergeResult = await this.mergeConvertFile({ hash, name, size, chunks, context, srcDwgNodeId });
                await this.cacheManager.delete('file-upload', mergeKey);
                return mergeResult;
            }
                        ...
                        if (matchingFiles.length > 0) {
                            ...
                            if (isCadFile) {
                                try {
                                    ...
                                    // 检查缩略图是否存在
                                    const thumbnailExists = await fsPromises.access(thumbnailCachePath).then(() => true).catch(() => false);
                                    if (thumbnailExists) {
                                        // 从缓存拷贝
                                        this.logger.log(`[performFileExistenceCheck] 拷贝缩略图: ${newNodeId}`);
                                        await fsPromises.copyFile(thumbnailCachePath, thumbnailTargetPath);
                                        this.logger.log(`[performFileExistenceCheck] 缩略图拷贝成功: ${thumbnailTargetPath}`);
                                    }
                                    else if (this.thumbnailGenerationService.isEnabled()) {
                                        // 手动生成缩略图（直接生成到缓存路径）
                                        ...
                                        if (result.success) {
                                            // 拷贝到节点目录
                                            await fsPromises.copyFile(thumbnailCachePath, thumbnailTargetPath);
                                            this.logger.log(`[performFileExistenceCheck] 缩略图生成成功: ${thumbnailTargetPath}`);
                                        }
                                        else {
