...
/**
 * 缩略图生成服务
 * 使用 MxWebDwg2Jpg.exe 将 CAD 文件转换为 JPG 缩略图
 * 仅支持 Windows 平台，Linux 下自动禁用
 */
@Injectable()
export class ThumbnailGenerationService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    if (os.platform() === 'linux') {
      ...
      this.logger.log(
        '缩略图自动生成仅支持 Windows 平台，当前为 Linux，已禁用'
      );
      return;
    }
   ...
   * @param outputFileName 输出文件名（默认 thumbnail.jpg，按优先级查找）
   * @returns 生成结果
   */
  async generateThumbnail(
    ...
    try {
      ...
      // 参数说明：
      // file: 源DWG文件路径 这里的文件路径必须是绝对路径，并且要用正斜杠（/）分隔，避免 Windows 路径分隔符导致的问题
      ...
      try {
        ...
        try {
          ...
          if (autoGeneratedPath !== thumbnailPath) {
            ...
            this.logger.log(`${logPrefix} 缩略图已移动到: ${thumbnailPath}`);
          }
          ...
          this.logger.error(
            `${logPrefix} 缩略图文件未生成: ${autoGeneratedPath}`
          );
          return {
            success: false,
            error: '缩略图文件未生成',
    ...
    try {
      ...
      if (!node || !node.path) {
        ...
        return {
          success: false,
          message:
            I18nContext.current()?.t('error.file.node_not_found_or_no_path') ??
            '文件节点不存在或没有 path 字段',
        };
      }
      ...
      if (!fs.existsSync(filePath)) {
        return {
          success: false,
          message:
            I18nContext.current()?.t('error.file.upload_file_not_found') ??
            '上传文件不存在',
        };
      }
      ...
      if (ext !== 'jpg' && ext !== 'jpeg') {
        this.logger.warn(
          `[uploadThumbnail] 仅支持 jpg 格式缩略图，收到: ${ext || '未知'}`
        );
        return {
          success: false,
          message: '仅支持 jpg 格式的缩略图',
        };
      }
      ...
      try {
        ...
        return {
          success: false,
          message:
            I18nContext.current()?.t('error.mxcad_extra.upload_local_failed', {
              args: { error: error.message },
            }) ?? `上传到本地失败: ${error.message}`,
        };
      }
      ...
      return {
        success: false,
        message:
          I18nContext.current()?.t('error.mxcad_extra.upload_failed_detail', {
            args: { error: error.message },
          }) ?? `上传失败: ${error.message}`,
      };
    }
