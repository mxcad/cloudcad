...
describe("MxCadController", () => {
    ...
    const mockConfigService = {
        get: jest.fn((key) => {
            if (key === "mxcad.fileExt")
                return ".mxweb";
            if (key === "cacheTTL")
                return { mxcad: 300 };
            if (key === "filesDataPath")
                return "/fake/filesData";
            return undefined;
        }),
    };
    ...
    beforeEach(async () => {
        ...
        controller = module.get(MxCadController);
    });
    afterEach(() => {
    ...
    describe("checkFileExist", () => {
        it("should return exists=true when file exists", async () => {
            ...
            const result = await controller.checkFileExist({ filename: "f.dwg", fileHash: "abc", fileSize: 1000 }, {
    ...
    describe("checkDuplicateFile", () => {
        it("should return isDuplicate=true for duplicate file", async () => {
            ...
            const result = await controller.checkDuplicateFile({ filename: "test.dwg", fileHash: "abc", nodeId: "n1" }, {});
        ...
        it("should return isDuplicate=false for new file", async () => {
            ...
            const result = await controller.checkDuplicateFile({ filename: "new.dwg", fileHash: "xyz", nodeId: "n1" }, {});
    ...
    describe("getPreloadingData", () => {
        ...
        it("should throw NotFoundException when data not found", async () => {
            ...
            await expect(controller.getPreloadingData("missing")).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe("checkExternalReference", () => {
        ...
        it("should throw BadRequestException without fileName", async () => {
            await expect(controller.checkExternalReference("node-1", {})).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe("saveMxwebToNode", () => {
        it("should save mxweb file successfully", async () => {
            ...
            const result = await controller.saveMxwebToNode("node-1", { path: path.join(os.tmpdir(), "test.mxweb"), originalname: "test.mxweb" }, "commit msg", undefined, { user: { id: "u1", username: "tester" } });
        ...
        it("should throw on save failure", async () => {
            ...
            await expect(controller.saveMxwebToNode("node-1", {}, "", undefined, {})).rejects.toThrow(BadRequestException);
        });
    });
    // ==================== saveMxwebAs ====================
    describe("saveMxwebAs", () => {
        it("should save as and return result", async () => {
            ...
            const result = await controller.saveMxwebAs({ path: path.join(os.tmpdir(), "f.mxweb"), originalname: "f.mxweb" }, {
                targetType: "project",
        ...
        it("should throw when user not logged in", async () => {
            await expect(controller.saveMxwebAs({}, {}, { user: null })).rejects.toThrow(UnauthorizedException);
        });
    });
    ...
    describe("uploadFile", () => {
        it("should handle merge request (no file, has chunks)", async () => {
            ...
            const result = await controller.uploadFile([], { hash: "abc", name: "f.dwg", size: 1000, chunks: 5 }, {
                headers: { authorization: "Bearer token" },
                body: { nodeId: "root-1" },
                query: {},
            });
        ...
        it("should throw when required params missing", async () => {
            await expect(controller.uploadFile([{}], { chunk: 0 }, {})).rejects.toThrow(BadRequestException);
        });
        it("should handle chunk upload", async () => {
            ...
            const result = await controller.uploadFile([{ path: path.join(os.tmpdir(), "chunk"), size: 100 }], { hash: "abc", name: "f.dwg", size: 1000, chunk: 0, chunks: 5 }, {
                headers: { authorization: "Bearer token" },
                body: { nodeId: "root-1" },
                query: {},
            });
...
//# sourceMappingURL=mxcad.controller.spec.js.map