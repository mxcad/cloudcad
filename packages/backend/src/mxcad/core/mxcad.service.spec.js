...
jest.mock("../infra/thumbnail-utils", () => ({
    ...
    getThumbnailFileName: (fmt) => `thumbnail.${fmt}`,
...
describe("MxCadService", () => {
    ...
    const mockConfigService = {
        get: jest.fn((key, opts) => {
            if (key === "mxcadUploadPath")
                return "/fake/upload";
            if (key === "nodeEnv")
                return "test";
            if (opts?.infer) {
    ...
    beforeEach(async () => {
        ...
        const module = await Test.createTestingModule({
            ...
            .compile();
        service = module.get(MxCadService);
    });
    afterEach(() => {
    ...
    describe("checkChunkExist", () => {
        it("delegates to facade with validated context", async () => {
            ...
            expect(mockFacade.checkChunkExist).toHaveBeenCalledWith(expect.objectContaining({ chunk: 0, hash: "abc", name: "f.dwg" }));
        });
    ...
    describe("checkDuplicateFile", () => {
        ...
        it("excludes currentFileId when provided", async () => {
            ...
            expect(mockPrisma.fileSystemNode.findFirst).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe("uploadChunk", () => {
        ...
        it("delegates uploadChunkWithPermission", async () => {
            ...
            const r = await service.uploadChunkWithPermission("abc", "f.dwg", 100, 0, 2, { userId: "u1", nodeId: "n1" });
            expect(r.ret).toBe("ok");
        });
    });
    ...
    describe("uploadAndConvertFile", () => {
        it("creates default context and delegates", async () => {
            ...
            const r = await service.uploadAndConvertFile(path.join(os.tmpdir(), "f.dwg"), "abc", "f.dwg", 100);
            expect(r.ret).toBe("ok");
        });
    });
    ...
    describe("uploadAndConvertFileWithPermission", () => {
        it("delegates to facade", async () => {
            ...
            const r = await service.uploadAndConvertFileWithPermission(path.join(os.tmpdir(), "f.dwg"), "abc", "f.dwg", 100, { userId: "u1", nodeId: "n1" });
            expect(r.ret).toBe("ok");
        });
    });
    ...
    describe("mergeChunksWithPermission", () => {
        it("delegates to facade with srcDwgNodeId", async () => {
            ...
            const r = await service.mergeChunksWithPermission("abc", "f.dwg", 100, 2, { userId: "u1", nodeId: "n1" }, "src-1");
            expect(r.ret).toBe("ok");
            expect(mockFacade.mergeChunksWithPermission).toHaveBeenCalledWith(expect.objectContaining({ srcDwgNodeId: "src-1" }));
        });
    });
    // ==================== convertServerFile ====================
    describe("convertServerFile", () => {
        it("returns error for null param", async () => {
            const r = await service.convertServerFile(null);
    ...
    describe("external reference queries", () => {
        it("getPreloadingData delegates", async () => {
            mockExtRefUpdateService.getPreloadingData.mockResolvedValue({
                externalReference: [],
                images: [],
            });
            expect(await service.getPreloadingData("n1")).toEqual({
                externalReference: [],
                images: [],
            });
        });
    ...
    describe("inferContextForMxCadApp", () => {
        it("delegates to nodeService", async () => {
            mockNodeService.inferContextForMxCadApp.mockResolvedValue({
                nodeId: "n1",
                userId: "u1",
                userRole: "USER",
            });
            const r = await service.inferContextForMxCadApp("abc", {});
    ...
    describe("node queries", () => {
        it("getFileSystemNodeByPath delegates", async () => {
            ...
            expect(await service.getFileSystemNodeByPath("path")).toEqual({
                id: "n1",
            });
        });
        it("getFileSystemNodeByNodeId delegates", async () => {
            mockNodeService.findById.mockResolvedValue({ id: "n1" });
            expect(await service.getFileSystemNodeByNodeId("n1")).toEqual({
                id: "n1",
            });
        });
    });
    ...
    describe("external reference management", () => {
        it("getExternalReferenceStats delegates", async () => {
            mockExtRefUpdateService.getStats.mockResolvedValue({
                hasMissing: false,
                missingCount: 0,
                totalCount: 0,
                references: [],
        ...
        it("updateExternalReferenceInfo delegates", async () => {
            ...
            await service.updateExternalReferenceInfo("n1", stats);
    ...
    describe("external reference file ops", () => {
        it("handleExternalReferenceImage delegates", async () => {
            await service.handleExternalReferenceImage("hash", "src", "ref.png", path.join(os.tmpdir(), "f.png"), {});
            expect(mockFacade.handleExternalReferenceImage).toHaveBeenCalled();
        });
        it("handleExternalReferenceFile delegates", async () => {
            await service.handleExternalReferenceFile("hash", "src", "ref.dwg", path.join(os.tmpdir(), "f.dwg"));
    ...
    describe("uploadThumbnail", () => {
        it("returns failure when node not found", async () => {
            mockNodeService.findById.mockResolvedValue(null);
            const r = await service.uploadThumbnail("n1", path.join(os.tmpdir(), "f.png"));
        ...
        it("returns failure for unsupported format", async () => {
            ...
            const r = await service.uploadThumbnail("n1", path.join(os.tmpdir(), "f.gif"));
            expect(r.success).toBe(false);
        ...
        it("succeeds for jpg thumbnail", async () => {
            ...
            const r = await service.uploadThumbnail("n1", path.join(os.tmpdir(), "f.jpg"));
            expect(r.success).toBe(true);
        ...
        it("succeeds for webp thumbnail", async () => {
            ...
            const r = await service.uploadThumbnail("n1", path.join(os.tmpdir(), "f.webp"));
            expect(r.success).toBe(true);
    ...
    describe("saveMxwebFile", () => {
        ...
        it("fails when file is missing", async () => {
            const r = await service.saveMxwebFile("n1", null);
            expect(r.success).toBe(false);
            expect(r.message).toContain("缺少文件");
        });
        it("fails when file has no path", async () => {
            const r = await service.saveMxwebFile("n1", { path: null });
            expect(r.success).toBe(false);
        });
    ...
    describe("generateBinFiles", () => {
        it("logs success when bin conversion succeeds", async () => {
            ...
            expect(mockConversionService.convertFile).toHaveBeenCalledWith(expect.objectContaining({ outname: "f.mxweb.bin" }));
        });
    ...
    describe("context validation (via uploadChunk)", () => {
        it("throws when context userId is missing", async () => {
            await expect(service.uploadChunk("abc", "f.dwg", 100, 0, 2, { nodeId: "n1" })).rejects.toThrow(BadRequestException);
        });
        it("throws when context nodeId is missing", async () => {
            await expect(service.uploadChunk("abc", "f.dwg", 100, 0, 2, { userId: "u1" })).rejects.toThrow(BadRequestException);
        });
        it("fills userRole from role when missing", async () => {
            ...
            expect(r.ret).toBe("ok");
            expect(mockFacade.uploadChunk).toHaveBeenCalledWith(expect.objectContaining({
                context: expect.objectContaining({ userRole: "USER" }),
            }));
        });
    });
});
//# sourceMappingURL=mxcad.service.spec.js.map