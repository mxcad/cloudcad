...
export declare class MxCadService {
     ...
     * @returns 保存结果
     */
    saveMxwebFile(nodeId: string, file: Express.Multer.File, userId?: string, userName?: string, commitMessage?: string, skipBinGeneration?: boolean, expectedTimestamp?: string): Promise<{
        success: boolean;
        message: string;
        path?: string;
    }>;
     ...
     */
    findNodeByIdWithDeletedAt(id: string, select?: any): Promise<any>;
    /**
     * 根据 ID 查找文件系统节点（不含 deletedAt 过滤）
     */
    findNodeById(id: string, select?: any): Promise<any>;
    /**
     * 查找非文件夹文件节点（未删除）
     */
    findFileNodeByIdNotDeleted(id: string, select?: any): Promise<any>;
    /**
     * 根据 ID 查找用户
     */
    findUserById(id: string, select?: any): Promise<any>;
    /**
     * 获取父节点下的所有子节点
     */
    findChildrenByParentId(parentId: string, select?: any): Promise<any[]>;
    /**
     * 获取项目中的所有节点ID（递归）
     */
    getAllNodeIdsInProject(projectId: string): Promise<string[]>;
    /**
     * 通过文件哈希值查找 FileSystemNode
     */
    findFileNodeByHash(fileHash: string, projectId?: string): Promise<Pick<FileSystemNode, 'id' | 'name' | 'ownerId' | 'parentId' | 'isRoot' | 'fileHash'> | null>;
    /**
     * 根据节点 ID 查找项目根目录
     */
    getProjectRootByNodeId(nodeId: string): Promise<any>;
}
