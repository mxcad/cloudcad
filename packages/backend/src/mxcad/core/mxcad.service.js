...
import { findThumbnail, getThumbnailFileName, THUMBNAIL_FORMATS, } from '../infra/thumbnail-utils';
let MxCadService = (() => {
    let _classDecorators = [Injectable()];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    var MxCadService = _classThis = class {
        constructor(configService, fileUploadManager, fileSystemNodeService, fileConversionService, externalReferenceUpdateService, storageManager, versionControlService, prisma) {
            ...
            this.prisma = prisma;
            this.logger = new Logger(MxCadService.name);
         ...
         * 检查分片是否存在
         */
        async checkChunkExist(chunk, fileHash, size, chunks, fileName, context) {
        ...
        async checkDuplicateFile(filename, fileHash, nodeId, currentFileId) {
            try {
                ...
                if (existingFile) {
                    this.logger.log(`发现重复文件: ${filename} (hash: ${fileHash}), 节点ID: ${existingFile.id}`);
                    return {
                        isDuplicate: true,
         ...
         * 上传分片文件
         */
        async uploadChunk(hash, name, size, chunk, chunks, context) {
         ...
         * 上传完整文件并转换
         */
        async uploadAndConvertFile(filePath, hash, name, size) {
            ...
            try {
                ...
                if (param.async === 'true' && param.resultposturl) {
                    ...
                    this.fileConversionService
                        ...
                        .then((taskId) => {
                        // 这里应该发送回调，暂时省略
                        this.logger.log(`异步转换完成: ${param.srcPath || param.srcpath}, 任务ID: ${taskId}`);
                    });
                    return { code: 0, message: 'async calling' };
                }
                else {
                    // 同步转换
                    const { isOk, ret } = await this.fileConversionService.convertFile(conversionOptions);
         ...
         * @returns 文件是否存在
         */
        async checkExternalReferenceExists(nodeId, fileName) {
            return this.externalReferenceUpdateService.checkExists(nodeId, fileName);
        }
        /**
         * 为 MxCAD-App 推断上下文信息
         */
        async inferContextForMxCadApp(fileHash, request) {
         ...
         */
        async uploadChunkWithPermission(hash, name, size, chunk, chunks, context) {
         ...
         * 注意：权限验证已在 Controller 层通过 @RequireProjectPermission 装饰器处理
         */
        async mergeChunksWithPermission(hash, name, size, chunks, context, srcDwgNodeId) {
         ...
         * 注意：权限验证已在 Controller 层通过 @RequireProjectPermission 装饰器处理
         */
        async uploadAndConvertFileWithPermission(filePath, hash, name, size, context) {
         ...
         * @returns 外部参照统计信息
         */
        async getExternalReferenceStats(nodeId) {
         ...
         */
        async updateExternalReferenceInfo(nodeId, stats) {
         ...
         * @param nodeId 文件系统节点 ID
         */
        async updateExternalReferenceAfterUpload(nodeId) {
        ...
        async handleExternalReferenceImage(fileHash, srcDwgNodeId, extRefFileName, srcFilePath, context) {
            return this.fileUploadManager.handleExternalReferenceImage(fileHash, srcDwgNodeId, extRefFileName, srcFilePath, context);
        }
        /**
         * 处理外部参照 DWG 上传（公开方法）
         */
        async handleExternalReferenceFile(extRefHash, srcDwgFileHash, extRefFileName, srcFilePath) {
            return this.fileUploadManager.handleExternalReferenceFile(extRefHash, srcDwgFileHash, extRefFileName, srcFilePath);
        }
        /**
         * 创建默认上下文（用于没有上下文的操作）
         */
         ...
         * 验证文件路径安全性，防止路径遍历攻击
         */
        validateFilePath(filePath, uploadPath) {
         ...
         * 验证预加载数据文件名合法性
         */
        isValidPreloadingFileName(fileName) {
         ...
         * @returns 文件节点或 null
         */
        async getFileSystemNodeByPath(storagePath) {
         ...
         * @returns 文件节点或 null
         */
        async getFileSystemNodeByNodeId(nodeId) {
         ...
         */
        async checkThumbnailExists(nodeId) {
            try {
                // 通过 nodeId 查找节点
                const node = await this.fileSystemNodeService.findById(nodeId);
                if (!node || !node.path || !node.fileHash) {
                    this.logger.warn(`[checkThumbnailExists] 节点不存在或没有 path 字段: ${nodeId}`);
        ...
        async uploadThumbnail(nodeId, filePath) {
            try {
                ...
                const node = await this.fileSystemNodeService.findById(nodeId);
                if (!node || !node.path) {
                    this.logger.warn(`[uploadThumbnail] 节点不存在或没有 path 字段: ${nodeId}`);
                    return {
                        success: false,
                ...
                try {
                    ...
                    if (fs.existsSync(targetLocalPath)) {
                        ...
                        this.logger.log(`[uploadThumbnail] 删除已存在的本地缩略图: ${targetLocalPath}`);
                    }
                    // 拷贝文件到目标位置
                    fs.copyFileSync(filePath, targetLocalPath);
                    this.logger.log(`[uploadThumbnail] 缩略图上传到本地成功: ${filePath} -> ${targetLocalPath}`);
                }
                catch (error) {
                    this.logger.error(`[uploadThumbnail] 上传到本地文件系统失败: ${error.message}`, error.stack);
                    return {
                        success: false,
         ...
         * @returns 保存结果
         */
        async saveMxwebFile(nodeId, file, userId, userName, commitMessage, skipBinGeneration = false, expectedTimestamp) {
            try {
                ...
                if (expectedTimestamp && fullNode.updatedAt) {
                    ...
                    if (expectedTime !== actualTime) {
                        this.logger.warn(`[saveMxwebFile] 乐观锁冲突: nodeId=${nodeId}, expected=${expectedTimestamp}, actual=${fullNode.updatedAt.toISOString()}`);
                        throw new ConflictException('文件已被他人修改，请刷新后重试');
                    }
                }
                // 获取节点目录路径
                const nodeFullPath = this.storageManager.getFullPath(node.path);
                ...
                if (fs.existsSync(targetPath) && !fs.existsSync(initialMxwebPath)) {
                    this.logger.log(`[saveMxwebFile] 备份初始版本: ${targetPath} -> ${initialMxwebPath}`);
                ...
                if (!fullNode.libraryKey) {
                    ...
                    const commitResult = await this.versionControlService.commitNodeDirectory(nodeDirectory, message, userId, userName);
                    ...
                    this.logger.log(`[saveMxwebFile] 跳过 SVN 提交: ${node.name} (公共资源库: ${node.libraryKey})`);
                }
                // 删除临时上传文件
                try {
                ...
                this.logger.error(`[saveMxwebFile] 保存失败: ${error.message}`, error.stack);
                return {
                    success: false,
         ...
         */
        async generateBinFiles(mxwebPath, nodeName) {
            try {
         ...
         */
        async findNodeByIdWithDeletedAt(id, select) {
         ...
         * 查找非文件夹文件节点（未删除）
         */
        async findFileNodeByIdNotDeleted(id, select) {
            return this.prisma.fileSystemNode.findFirst({
         ...
         * 获取父节点下的所有子节点
         */
        async findChildrenByParentId(parentId, select) {
        ...
        async getAllNodeIdsInProject(projectId) {
            const nodeIds = [];
            const collectNodeIds = async (nodeId) => {
         ...
         */
        async findFileNodeByHash(fileHash, projectId) {
            try {
                const where = {
                    fileHash: fileHash,
                    isFolder: false,
    ...
    __setFunctionName(_classThis, "MxCadService");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        MxCadService = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return MxCadService = _classThis;
})();
export { MxCadService };
//# sourceMappingURL=mxcad.service.js.map