...
export class ExternalReferenceHandler {
  ...
  async handleExternalReferenceRequest(
    ...
    try {
      ...
      if (node.nodeType !== NodeType.FILE) {
        throw new BadRequestException(I18nContext.current()?.t('error.file.node_is_folder_not_file') ?? '该节点是文件夹，不是文件');
      }
      ...
      if (!node.fileHash) {
        ...
        throw new BadRequestException(I18nContext.current()?.t('error.file.conversion_incomplete') ?? '文件尚未转换完成');
      }

      // 3. 获取外部参照目录名称（从 preloading.json 中提取 src_file_md5）
      // preloading.json 文件命名：{nodeId}.{ext}.mxweb_preloading.json
      ...
      if (!fs.existsSync(externalRefFilePath)) {
        ...
        throw new NotFoundException(I18nContext.current()?.t('error.file.external_ref_not_found') ?? '外部参照文件不存在');
      }
  ...
  private async streamFile(
    ...
    fileStream.on('error', (error: Error) => {
      ...
      if (!res.headersSent) {
        res.status(500).json({ message: (I18nContext.current()?.t('error.file_extra_v2.read_failed') ?? '文件读取失败') });
      }
    });
  }
