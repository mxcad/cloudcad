...
describe("RuntimeConfigService", () => {
    ...
    describe("set", () => {
        ...
        it("should throw BadRequestException for unknown key", async () => {
            await expect(service.set("unknown-key", "value")).rejects.toThrow(BadRequestException);
        });
    ...
    describe("resetToDefault", () => {
        ...
        it("should throw BadRequestException for unknown key", async () => {
            await expect(service.resetToDefault("unknown-key")).rejects.toThrow(BadRequestException);
        });
    });
    // ==================== getDefinitions ====================
    ...
    describe("parseValue", () => {
        it("should parse boolean type", () => {
            const result = service.parseValue(JSON.stringify(true), "boolean");
            expect(typeof result).toBe("boolean");
            expect(result).toBe(true);
        });
        it("should parse number type", () => {
            const result = service.parseValue(JSON.stringify(42), "number");
            expect(typeof result).toBe("number");
            expect(result).toBe(42);
        });
        it("should parse string type", () => {
            const result = service.parseValue(JSON.stringify("test"), "string");
            expect(typeof result).toBe("string");
            expect(result).toBe("test");
        });
        it("should return original value on parse error", () => {
            const result = service.parseValue("invalid-json", "string");
            expect(result).toBe("invalid-json");
        });
    });
});
//# sourceMappingURL=runtime-config.service.spec.js.map