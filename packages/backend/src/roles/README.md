...
**系统角色 `SystemRole`**（用户表 `role` 字段）：`ADMIN`（全部系统权限）、`USER_MANAGER`（用户+角色管理）、`FONT_MANAGER`（字体库管理）、`USER`（基础，仅 `PROJECT_CREATE`）
- 直接权限映射：`SYSTEM_ROLE_PERMISSIONS`；继承链：`SYSTEM_ROLE_HIERARCHY`（USER_MANAGER、FONT_MANAGER 继承 USER）
...
**系统权限 `SystemPermission`**（24 项）：`SYSTEM_USER_READ/CREATE/UPDATE/DELETE/MEMBERSHIP_MANAGE`、`SYSTEM_ROLE_READ/CREATE/UPDATE/DELETE/PERMISSION_MANAGE`、`SYSTEM_FONT_READ/UPLOAD/DELETE/DOWNLOAD`、`SYSTEM_ADMIN`、`SYSTEM_BILLING_READ/WRITE`、`SYSTEM_MONITOR`、`SYSTEM_CONFIG_READ/WRITE`、`SYSTEM_IP_BLACKLIST_MANAGE`、`LIBRARY_DRAWING_MANAGE`、`LIBRARY_BLOCK_MANAGE`、`PROJECT_CREATE`
