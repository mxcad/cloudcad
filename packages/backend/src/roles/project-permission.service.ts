...
import { IProjectPermissionService } from './interfaces/project-permission-service.interface';
...
export class ProjectPermissionService implements IProjectPermissionService {
  ...
  async getUserPermissions(
    ...
    try {
      ...
      // 无成员行回退：私人空间（"我的图纸"）零成员零角色（ADR-00XX），
      // 权限按 ownerId 判定——owner 拥有全部项目权限
      // （与 RequireProjectPermissionGuard / PrismaPermissionStore 语义一致）。
  ...
  async getUserRole(
    ...
    try {
      ...
      // 无成员行回退：私人空间 owner 视为项目所有者（ADR-00XX，与权限接口语义一致）
      const personalSpaceOwnerId = await this.getPersonalSpaceOwnerId(projectId);
      if (personalSpaceOwnerId === userId) {
        this.cacheService.set(
          cacheKey,
          ProjectRole.OWNER,
          CACHE_TTL.PROJECT_MEMBER_ROLE
        );
   ...
   * 私人空间零成员零角色（ADR-00XX），权限按 ownerId 判定。
   */
