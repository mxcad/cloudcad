...
export class FileValidationService {
  ...
  validateFileType(file: Express.Multer.File): void {
    ...
    if (this.blockedExtensions.includes(extension)) {
      ...
      throw new BadRequestException(I18nContext.current()?.t('error.file.extension_forbidden', { args: { extension } }) ?? `禁止上传 ${extension} 类型文件`);
    }
  ...
  validateFileMagicNumber(filePath: string, extension: string): void {
    try {
      ...
      throw new BadRequestException(I18nContext.current()?.t('error.file.magic_number_failed') ?? `文件魔数验证失败: ${error.message}`);
    }
  }
  ...
  private validateDwgFileStructure(buffer: Buffer): void {
    ...
    if (buffer.length < 100) {
      throw new BadRequestException(I18nContext.current()?.t('error.file.dwg_too_small') ?? 'DWG 文件太小，可能已损坏');
    }
  ...
  private validateDxfFileStructure(buffer: Buffer): void {
    ...
    if (!hasDxfStructure) {
      throw new BadRequestException(I18nContext.current()?.t('error.file.dxf_missing_marker') ?? 'DXF 文件缺少必需的 DXF 标记，可能已损坏');
    }
  }
  ...
  validateFilename(filename: string): void {
    try {
      ...
      throw new BadRequestException(I18nContext.current()?.t('error.file.name_sanitize_failed') ?? `文件名验证失败: ${error.message}`);
    }
  }
  ...
  sanitizeFilename(filename: string): string {
    try {
      ...
      // 4. 防止空文件名
      if (!sanitized) {
        throw new BadRequestException(I18nContext.current()?.t('error.file.name_empty') ?? '文件名不能为空');
      }

      // 5. 防止文件名只有点号（隐藏文件）
      if (sanitized === '.' || sanitized === '..') {
        throw new BadRequestException(I18nContext.current()?.t('error.file.name_dot_only') ?? '文件名不能仅为点号');
      }
      ...
      throw new BadRequestException(I18nContext.current()?.t('error.file.name_sanitize_failed') ?? `文件名清理失败: ${error.message}`);
    }
  }
  ...
  private validateSanitizedFilename(filename: string): void {
    ...
    if (
      ...
      throw new BadRequestException(I18nContext.current()?.t('error.file.name_sanitize_contains_path') ?? '清理后的文件名仍包含路径字符');
    }

    // 验证文件名不以点开头（防止隐藏文件）
    if (filename.startsWith('.')) {
      throw new BadRequestException(I18nContext.current()?.t('error.file.name_sanitize_starts_with_dot') ?? '清理后的文件名不能以点开头');
    }

    // 验证文件名不以空格开头或结尾
    if (filename !== filename.trim()) {
      throw new BadRequestException(I18nContext.current()?.t('error.file.name_sanitize_has_spaces') ?? '清理后的文件名包含前导或尾随空格');
    }
  }
