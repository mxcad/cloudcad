...
describe('ProjectMemberService', () => {
    ...
    describe('getProjectMembers', () => {
        ...
        it('should throw NotFoundException if project not found', async () => {
            ...
            await expect(service.getProjectMembers('invalid')).rejects.toThrow(NotFoundException);
        });
    });
    describe('addProjectMember', () => {
        it('should add project member successfully', async () => {
            ...
            const result = await service.addProjectMember('project1', 'user1', 'role1', 'operator1');
            expect(mockProjectPermissionService.checkPermission).toHaveBeenCalledWith('operator1', 'project1', ProjectPermission.PROJECT_MEMBER_MANAGE);
        ...
        it('should throw ForbiddenException if no permission', async () => {
            ...
            await expect(service.addProjectMember('project1', 'user1', 'role1', 'operator1')).rejects.toThrow(ForbiddenException);
        });
        it('should throw NotFoundException if project not found', async () => {
            mockProjectPermissionService.checkPermission.mockResolvedValue(true);
            mockPrisma.fileSystemNode.findUnique.mockResolvedValue(null);
            await expect(service.addProjectMember('project1', 'user1', 'role1', 'operator1')).rejects.toThrow(NotFoundException);
        });
        it('should throw BadRequestException if personal space', async () => {
            ...
            mockProjectPermissionService.checkPermission.mockResolvedValue(true);
            mockPrisma.fileSystemNode.findUnique.mockResolvedValue(mockProject);
            await expect(service.addProjectMember('project1', 'user1', 'role1', 'operator1')).rejects.toThrow(BadRequestException);
        });
    ...
    describe('updateProjectMember', () => {
        it('should update project member role successfully', async () => {
            ...
            const result = await service.updateProjectMember('project1', 'user1', 'role2', 'operator1');
            expect(result).toBeDefined();
        });
        it('should throw ForbiddenException if no permission', async () => {
            mockProjectPermissionService.checkPermission.mockResolvedValue(false);
            await expect(service.updateProjectMember('project1', 'user1', 'role1', 'operator1')).rejects.toThrow(ForbiddenException);
        });
        it('should throw NotFoundException if project not found', async () => {
            mockProjectPermissionService.checkPermission.mockResolvedValue(true);
            mockPrisma.fileSystemNode.findUnique.mockResolvedValue(null);
            await expect(service.updateProjectMember('project1', 'user1', 'role1', 'operator1')).rejects.toThrow(NotFoundException);
        });
        it('should throw BadRequestException if personal space', async () => {
            const mockProject = {
                id: 'project1',
            ...
            mockProjectPermissionService.checkPermission.mockResolvedValue(true);
            mockPrisma.fileSystemNode.findUnique.mockResolvedValue(mockProject);
            await expect(service.updateProjectMember('project1', 'user1', 'role1', 'operator1')).rejects.toThrow(BadRequestException);
        });
    ...
    describe('removeProjectMember', () => {
        it('should remove project member successfully', async () => {
            ...
            const result = await service.removeProjectMember('project1', 'user1', 'operator1');
            expect(result).toEqual({ message: '成员移除成功' });
        });
        it('should throw ForbiddenException if no permission', async () => {
            mockProjectPermissionService.checkPermission.mockResolvedValue(false);
            await expect(service.removeProjectMember('project1', 'user1', 'operator1')).rejects.toThrow(ForbiddenException);
        });
        it('should throw NotFoundException if project not found', async () => {
            mockProjectPermissionService.checkPermission.mockResolvedValue(true);
            mockPrisma.fileSystemNode.findUnique.mockResolvedValue(null);
            await expect(service.removeProjectMember('project1', 'user1', 'operator1')).rejects.toThrow(NotFoundException);
        });
        it('should throw BadRequestException if personal space', async () => {
            const mockProject = {
                id: 'project1',
            ...
            mockProjectPermissionService.checkPermission.mockResolvedValue(true);
            mockPrisma.fileSystemNode.findUnique.mockResolvedValue(mockProject);
            await expect(service.removeProjectMember('project1', 'user1', 'operator1')).rejects.toThrow(BadRequestException);
        });
    ...
    describe('transferProjectOwnership', () => {
        it('should transfer project ownership successfully', async () => {
            ...
            mockPrisma.fileSystemNode.findUnique.mockResolvedValue(mockProject);
            mockPrisma.projectMember.findUnique.mockResolvedValue(mockNewOwnerMember);
            mockPrisma.projectRole.findFirst.mockResolvedValueOnce(mockOwnerRole).mockResolvedValueOnce(mockAdminRole);
            const result = await service.transferProjectOwnership('project1', 'newOwner', 'currentOwner');
            expect(mockPrisma.$transaction).toHaveBeenCalled();
            expect(result).toEqual({ message: '项目所有权转让成功' });
        });
        it('should throw NotFoundException if project not found', async () => {
            mockPrisma.fileSystemNode.findUnique.mockResolvedValue(null);
            await expect(service.transferProjectOwnership('project1', 'newOwner', 'currentOwner')).rejects.toThrow(NotFoundException);
        });
        it('should throw BadRequestException if personal space', async () => {
            const mockProject = {
                id: 'project1',
            ...
            mockPrisma.fileSystemNode.findUnique.mockResolvedValue(mockProject);
            await expect(service.transferProjectOwnership('project1', 'newOwner', 'currentOwner')).rejects.toThrow(BadRequestException);
        });
        it('should throw ForbiddenException if not owner', async () => {
            ...
            mockPrisma.fileSystemNode.findUnique.mockResolvedValue(mockProject);
            await expect(service.transferProjectOwnership('project1', 'newOwner', 'currentOwner')).rejects.toThrow(ForbiddenException);
        });
        it('should throw BadRequestException if transfer to self', async () => {
            ...
            mockPrisma.fileSystemNode.findUnique.mockResolvedValue(mockProject);
            await expect(service.transferProjectOwnership('project1', 'currentOwner', 'currentOwner')).rejects.toThrow(BadRequestException);
        });
...
//# sourceMappingURL=project-member.service.spec.js.map