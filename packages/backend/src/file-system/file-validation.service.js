...
let FileValidationService = (() => {
    ...
    let _classThis;
    var FileValidationService = _classThis = class {
        constructor(configService, runtimeConfigService) {
            this.configService = configService;
            this.runtimeConfigService = runtimeConfigService;
            this.logger = new Logger(FileValidationService.name);
        ...
        async getMaxFileSize() {
            ...
            const maxFileSizeMB = await this.runtimeConfigService.getValue('maxFileSize', 100);
        ...
        validateFileType(file) {
            ...
            if (this.blockedExtensions.includes(extension)) {
                this.logger.error(`文件类型被阻止: 文件=${file.originalname}, 扩展名=${extension}`);
                throw new BadRequestException(`禁止上传 ${extension} 类型文件`);
            }
            if (!this.allowedExtensions.includes(extension)) {
                this.logger.error(`文件类型不允许: 文件=${file.originalname}, 扩展名=${extension}`);
                throw new BadRequestException(`仅允许上传以下类型文件: ${this.allowedExtensions.join(', ')}`);
            }
            // 严格验证 MIME 类型
            ...
            if (expectedMimeType) {
                ...
                if (!file.mimetype.includes('octet-stream') &&
                    ...
                    this.logger.error(`MIME类型不匹配: 文件=${file.originalname}, 扩展名=${extension}, 期望=${expectedMimeType}, 实际=${file.mimetype}`);
                    throw new BadRequestException(`文件类型验证失败: 文件 ${file.originalname} 的 MIME 类型 ${file.mimetype} 与扩展名 ${extension} 不匹配`);
                }
            }
        ...
        async validateFileSize(file) {
            ...
            if (file.size > maxFileSize) {
                throw new BadRequestException(`文件大小超过限制: ${(file.size / 1024 / 1024).toFixed(2)}MB > ${(maxFileSize / 1024 / 1024).toFixed(2)}MB`);
            }
        }
        /**
         * 验证文件魔数（Magic Number）
         ...
         */
        validateFileMagicNumber(filePath, extension) {
            try {
                const config = this.configurableFileTypes.find((c) => c.extension === extension && c.enabled);
                ...
                if (!isMatch) {
                    ...
                    const actual = fileHeader
                        ...
                        .map((b) => '0x' + b.toString(16).padStart(2, '0'))
                        .join(' ');
                    this.logger.error(`文件魔数不匹配: 扩展名=${extension}, 期望=${expected}, 实际=${actual}`);
                    throw new BadRequestException(`文件内容与扩展名 ${extension} 不匹配，可能为恶意文件`);
                }
                // 额外的深度验证：检查文件内容结构
         ...
         * @returns 是否匹配
         */
        validateMagicNumberDeep(fileHeader, magicNumbers) {
            // 精确匹配：文件头与魔数完全匹配
         ...
         * @param extension 文件扩展名
         */
        validateFileContentDeep(buffer, extension) {
         ...
         * @param buffer 文件内容缓冲区
         */
        validateDwgFileStructure(buffer) {
            // DWG 文件应该有合理的结构
            // 检查文件大小（至少 100 字节）
         ...
         * @param buffer 文件内容缓冲区
         */
        validateDxfFileStructure(buffer) {
         ...
         */
        validateSanitizedFilename(filename) {
            // 验证文件名不包含路径遍历字符
         ...
         * @param file 文件对象
         */
        async validateFileWithMagicNumber(filePath, file) {
    ...
    __setFunctionName(_classThis, "FileValidationService");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        FileValidationService = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return FileValidationService = _classThis;
})();
