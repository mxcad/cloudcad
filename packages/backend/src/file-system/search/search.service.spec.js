...
describe("SearchService", () => {
    ...
    describe("search scope dispatch", () => {
        it("throws for unknown scope", async () => {
            await expect(service.search("u1", { keyword: "test", scope: "invalid" })).rejects.toThrow(BadRequestException);
        });
    });
    // ==================== PROJECT scope ====================
    ...
    describe("scope = PROJECT_FILES", () => {
        it("throws when projectId is missing", async () => {
            await expect(service.search("u1", {
                keyword: "test",
        ...
        it("searches project files with permission", async () => {
            ...
            expect(mockPermissionService.checkNodePermission).toHaveBeenCalledWith("u1", "proj-1", ProjectPermission.FILE_OPEN);
    ...
    describe("scope = LIBRARY", () => {
        ...
        it("handles pagination", async () => {
            const nodes = Array.from({ length: 3 }, (_, i) => makeNode({ id: `n${i}`, name: `f${i}.dwg` }));
    ...
    describe("edge cases", () => {
        ...
        it("defaults to PROJECT_FILES scope when not specified", async () => {
            ...
            await expect(service.search("u1", { keyword: "test" })).rejects.toThrow(BadRequestException);
        });
    });
});
