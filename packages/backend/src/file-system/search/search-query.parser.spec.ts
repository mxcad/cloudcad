...
describe('parseSearchQuery', () => {
  ...
  it('accumulates multiple ext filters as comma-separated multi-value', () => {
    // 多选场景（前端 CheckboxList 生成 "ext:.dwg ext:.mxweb"）：
    // 应累积而非 last-wins 覆盖，否则只剩最后一个扩展名（回归：格式多选后列表空白/只显示单格式）
    const result = parseSearchQuery('ext:.dwg ext:.mxweb');
    expect(result.extension).toBe('.dwg,.mxweb');
    expect(result.hasSyntax).toBe(true);
  });

  it('accumulates multiple ext filters interleaved with keyword', () => {
    const result = parseSearchQuery('drawing ext:.dwg notes ext:.dxf');
    expect(result.keyword).toBe('drawing notes');
    expect(result.extension).toBe('.dwg,.dxf');
  });

  it('accumulates extension alias with normalization', () => {
    const result = parseSearchQuery('extension:mxweb ext:.dwg');
    expect(result.extension).toBe('.mxweb,.dwg');
  });
