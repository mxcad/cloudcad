  ...
  async handleDownload(
    ...
    try {
      ...
      const { stream, filename, mimeType } =
        await this.fileDownloadExportService.downloadNode(nodeId, userId);
      ...
      if (node && node.nodeType === NodeType.FILE && (node.fileHash || node.id)) {
      ...
      stream.on('error', (error: Error) => {
        ...
        if (!res.headersSent) {
          res.status(500).json({ message: (I18nContext.current()?.t('error.file.download_failed') ?? '文件下载失败') });
        ...
        const status =
          error instanceof NotFoundException
            ...
            : error instanceof ForbiddenException
              ...
              : 500;
        const errorMessage =
          error instanceof Error ? error.message : (I18nContext.current()?.t('error.file.download_failed') ?? '文件下载失败');
