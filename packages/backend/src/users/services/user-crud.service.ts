...
export class UserCrudService {
  ...
  async create(createUserDto: CreateUserDto): Promise<ICreatedUser> {
    try {
      ...
      if (requireEmailVerification && !createUserDto.email) {
        throw new BadRequestException(I18nContext.current()?.t('error.user.email_required') ?? '邮箱验证已启用，邮箱为必填项');
      }
      ...
      if (createUserDto.email) {
        ...
        if (existingEmail) {
          throw new ConflictException(I18nContext.current()?.t('error.user.email_exists') ?? '邮箱已存在');
        }
      }
      ...
      if (existingUsername) {
        throw new ConflictException(I18nContext.current()?.t('error.user.username_exists') ?? '用户名已存在');
      }
      ...
      if (createUserDto.phone) {
        ...
        if (existingPhone) {
          throw new ConflictException(I18nContext.current()?.t('error.user.phone_exists') ?? '手机号已存在');
        }
      }

      const defaultRole = await this.prisma.role.findFirst({ where: { name: 'USER' } });
      if (!defaultRole) {
        throw new NotFoundException(I18nContext.current()?.t('error.user.default_role_not_found') ?? '默认角色不存在，请联系管理员');
      }
      ...
      const user = await this.prisma.$transaction(async (tx) => {
        const newUser = await tx.user.create({
          data: {
            ...
            phoneVerified: createUserDto.phoneVerified ?? (createUserDto.phone ? true : false),
          ...
          select: {
            ...
            phone: true, phoneVerified: true, wechatId: true, provider: true,
        ...
        if (!ownerRole) {
          throw new NotFoundException(I18nContext.current()?.t('error.user.project_owner_role_not_found') ?? 'PROJECT_OWNER 角色不存在');
        }

        await tx.fileSystemNode.create({
          data: {
            name: '我的图纸', nodeType: NodeType.PERSONAL_SPACE,
            projectStatus: ProjectStatus.ACTIVE, ownerId: newUser.id,
            projectMembers: { create: { userId: newUser.id, projectRoleId: ownerRole.id } },
          },
        });
      ...
      this.eventEmitter.emit('user.created', {
        userId: user.id, email: user.email, username: user.username, timestamp: new Date(),
  ...
  async findAll(query: QueryUsersDto, userId?: string) {
    try {
      const { search, roleId, page = 1, limit = 10, sortBy, sortOrder, projectId, tierLevel } = query;
      ...
      if (tierLevel !== undefined && tierLevel !== '') {
        ...
        if (tierLevel === '0') {
          ...
          andList.push({ OR: [{ membership: null }, { membership: { tierLevel: 0 } }] });
        } else if (tierLevel === 'expired') {
          // 已过期：tierLevel > 0 且 expiresAt < 现在
          andList.push({ membership: { tierLevel: { gt: 0 }, expiresAt: { lt: new Date() } } });
        } else {
          // 特定等级
      ...
      if (projectId && userId) {
        ...
        if (!project) {
          throw new BadRequestException(I18nContext.current()?.t('error.project.not_found') ?? '项目不存在');
        }
        if (project.ownerId !== userId) {
          ...
          if (!isMember) {
            throw new BadRequestException(I18nContext.current()?.t('error.user.not_project_member') ?? '您不是该项目的成员');
          }
        }
      }
      ...
      const [users, total] = await Promise.all([
        this.prisma.user.findMany({
          ...
          select: {
            ...
            username: true, nickname: true, avatar: true,
            wechatId: true, provider: true,
            role: { select: { id: true, name: true, description: true, isSystem: true, permissions: { select: { permission: true } } } },
      ...
      const usersWithMembership = users.map((user) => this.flattenMembership(user));

      return { users: usersWithMembership, total, page, limit, totalPages: Math.ceil(total / safeLimit) };
    } catch (error) {
  ...
  private async findByIdInternal(id: string) {
    try {
      const user = await this.prisma.user.findUnique({
        ...
        select: {
          id: true, email: true, username: true, nickname: true, avatar: true,
          phone: true, phoneVerified: true, password: true, wechatId: true, provider: true,
          role: { select: { id: true, name: true, description: true, isSystem: true, permissions: { select: { permission: true } } } },
          membership: { select: { tierLevel: true, expiresAt: true } },
          status: true, createdAt: true, updatedAt: true,
        },
      ...
      const { password: _, membership: membershipRow, ...userWithoutPassword } = user;
    ...
    try {
      return await this.prisma.user.findUnique({
        ...
        select: {
          ...
          phone: true, phoneVerified: true, wechatId: true, provider: true,
          role: { select: { id: true, name: true, description: true, isSystem: true, permissions: { select: { permission: true } } } },
          status: true, password: true, createdAt: true, updatedAt: true,
        },
      });
    } catch (error) {
  ...
  async update(id: string, updateUserDto: UpdateUserDto): Promise<ICreatedUser> {
    try {
      ...
      if (updateUserDto.roleId) updateData.role = { connect: { id: updateUserDto.roleId } };
  ...
  async updateMembership(id: string, tierLevel: number, expiresAt?: string, adjustDays?: number) {
    try {
      const existingUser = await this.prisma.user.findUnique({ where: { id } });
      if (!existingUser) {
        throw new NotFoundException(I18nContext.current()?.t('error.user.not_found') ?? '用户不存在');
      ...
      if (adjustDays !== undefined && adjustDays !== 0 && tierLevel > 0) {
        const existingMembership = await this.prisma.userMembership.findUnique({ where: { userId: id } });
        const now = new Date();
        const base = existingMembership?.expiresAt && existingMembership.expiresAt > now
          ? existingMembership.expiresAt
          : now;
        const adjusted = new Date(base.getTime() + adjustDays * DAY_MS);
        // 调整后不能早于现在（不设已过期时间）
        finalExpiresAt = adjusted > now ? adjusted.toISOString() : now.toISOString();
      }

      if (tierLevel > 0) {
        if (!finalExpiresAt) {
          throw new BadRequestException(
            I18nContext.current()?.t('error.vip.expires_at_required') ?? '有效会员必须设置到期时间',
          );
        }
        const maxTier = await this.prisma.vipTier.aggregate({ _max: { level: true } });
        const maxLevel = maxTier._max.level ?? 0;
        if (tierLevel > maxLevel) {
          throw new BadRequestException(
            I18nContext.current()?.t('error.vip.tier_not_found') ?? `VIP 等级不能超过 ${maxLevel}`,
          );
        }
      }
      ...
      if (tierLevel === 0) {
        ...
        this.logger.log(`用户会员已更新：id=${id}, tierLevel=${tierLevel}, expiresAt=${finalExpiresAt}`);
      }
  ...
  private flattenMembership<T extends { membership?: { tierLevel: number; expiresAt: Date | null } | null }>(
    user: T,
  ): Omit<T, 'membership'> & {
    ...
    const membershipValid = !!membershipRow && (membershipRow.expiresAt === null || membershipRow.expiresAt > new Date());
  ...
  async getDashboardStats(userId: string) {
    try {
      ...
      const [projectCount, totalFiles, todayUploads, fileTypeStats, storageUsed] = await Promise.all([
        this.prisma.fileSystemNode.count({
          where: { nodeType: NodeType.PROJECT, deletedAt: null, OR: [{ ownerId: userId }, { projectMembers: { some: { userId } } }] },
        }),
        this.prisma.fileSystemNode.count({
          where: { nodeType: NodeType.FILE, deletedAt: null, OR: [{ ownerId: userId }, { parent: { OR: [{ ownerId: userId }, { projectMembers: { some: { userId } } }] } }] },
        }),
        this.prisma.fileSystemNode.count({
          where: { nodeType: NodeType.FILE, deletedAt: null, createdAt: { gte: todayStart }, ownerId: userId },
        }),
      ...
      return { projectCount, totalFiles, todayUploads, fileTypeStats: fileTypeResult, storage: { used: usedStorage, total: totalStorage, remaining: remainingStorage, usagePercent } };
    } catch (error) {
