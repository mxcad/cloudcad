...
let UsersService = (() => {
    ...
    var UsersService = _classThis = class {
        constructor(prisma, permissionCacheService, userCleanupService, configService, runtimeConfigService, passwordHasher, verificationStrategies, smsVerificationService, emailVerificationService, eventEmitter) {
            this.prisma = prisma;
                ...
                const user = await this.prisma.$transaction(async (tx) => {
                    ...
                    const newUser = await tx.user.create({
                        data: {
                            ...
                            phoneVerified: createUserDto.phoneVerified ??
         ...
         * 使用策略模式委派各验证方式
         */
        async deactivate(userId, password, phoneCode, emailCode, wechatConfirm) {
            try {
                const user = await this.prisma.user.findUnique({
                    where: { id: userId },
                });
                ...
                // 构建验证参数
                const params = { password, phoneCode, emailCode, wechatConfirm };
                // 使用策略模式查找匹配的验证策略
                ...
                for (const strategy of matchingStrategies) {
                    ...
                    if (!result.valid) {
                        throw new UnauthorizedException(result.message || this.getVerifyFailedError(strategy.type));
                    }
                    verified = true;
                    break;
                }
         ...
         * 恢复已注销账户（冷静期内自助恢复）
         */
        async restoreAccount(userId, verificationMethod, code) {
            try {
                const user = await this.prisma.user.findUnique({
                    where: { id: userId },
                });
                ...
                const cleanupDelayDays = this.configService.get('userCleanup.delayDays', 30);
                ...
                // 构建 VerificationParams，复用 deactivate 中的策略模式
                const params = {};
                ...
                const result = await strategy.verify(user, params);
                if (!result.valid) {
                    throw new UnauthorizedException(result.message || this.getRestoreVerifyFailedError(strategy.type));
                }
                await this.prisma.user.update({
                    where: { id: userId },
                    data: {
        ...
        async getDashboardStats(userId) {
            try {
                ...
                let quotaGB = await this.runtimeConfigService.getValue('userStorageQuota', 10);
         ...
         */
        getRestorePrerequisiteError(type) {
            const messages = {
                password: '该账户未设置密码',
                phoneCode: '该账户未绑定手机',
                emailCode: '该账户未绑定邮箱',
         ...
         */
        getRestoreVerifyFailedError(type) {
            const messages = {
                password: '密码不正确',
                phoneCode: '验证码不正确',
