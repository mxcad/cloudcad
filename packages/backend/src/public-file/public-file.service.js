...
let PublicFileService = (() => {
    ...
    let _classThis;
    var PublicFileService = _classThis = class {
        constructor(uploadService, fileConversionService) {
            this.uploadService = uploadService;
            this.fileConversionService = fileConversionService;
            this.logger = new Logger(PublicFileService.name);
        }
        /**
         * 检查分片是否存在
         */
         ...
         */
        async saveChunk(fileBuffer, hash, chunkIndex) {
        ...
        async checkFile(dto) {
            ...
            const { exist, mxwebPath } = await this.uploadService.checkFileExist(filename, fileHash);
            if (exist && mxwebPath) {
                this.logger.log(`[checkFile] 秒传成功: filename=${filename}, hash=${fileHash}`);
                return {
                    exist: true,
                    hash: fileHash,
                    fileName: filename,
         ...
         */
        async findFileInDir(hash, filename) {
         ...
         * @returns 上传结果
         */
        async uploadExtReference(fileBuffer, srcFileHash, extRefFileName, fileHash) {
            ...
            try {
                if (isDwgFile) {
                    ...
                    try {
                        ...
                        logger.log(`[uploadExtReference] 外部参照文件上传并转换成功: ${extRefFileName} -> ${targetPath}`);
                    }
                    catch (convertError) {
                        logger.error(`[uploadExtReference] 转换文件失败: ${convertError.message}`, convertError.stack);
                        // 转换失败时，删除临时文件
                        if (fs.existsSync(tempFilePath)) {
                            await fs.promises.unlink(tempFilePath);
                        }
                    ...
                    logger.log(`[uploadExtReference] 外部参照文件上传成功: ${extRefFileName} -> ${targetPath}`);
                }
                return { ret: 'ok', hash };
            }
            catch (error) {
                logger.error(`[uploadExtReference] 处理文件失败: ${error.message}`, error.stack);
         ...
         * @returns 是否存在
         */
        async checkExtReferenceExists(srcFileHash, extRefFileName) {
            const uploadPath = this.uploadService.getUploadPath();
            const srcDir = path.join(uploadPath, srcFileHash);
            const ext = path.extname(extRefFileName).toLowerCase();
            const isDwgFile = ['.dwg', '.dxf'].includes(ext);
    ...
    __setFunctionName(_classThis, "PublicFileService");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        PublicFileService = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return PublicFileService = _classThis;
})();
export { PublicFileService };
//# sourceMappingURL=public-file.service.js.map