...
export class UserCleanupTriggerResponseDto {
  ...
  @ApiProperty({ description: '删除的支付订单数', example: 0 })
  deletedPaymentOrders: number;

  @ApiProperty({ description: '删除的会员记录数', example: 0 })
  deletedMemberships: number;

  @ApiProperty({ description: '删除的文件分享数', example: 0 })
  deletedFileShares: number;

  @ApiProperty({ description: '删除的批量下载任务数', example: 0 })
