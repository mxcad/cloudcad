...
export class PolicyConfigService {
  ...
  > {
    try {
      ...
      if (!existing) {
        throw new NotFoundException(I18nContext.current()?.t('error.policy.config_not_found') ?? `策略配置不存在: ${policyId}`);
      }
  ...
  /**
   * 清除缓存
   * 清除所有策略配置缓存（policy_config:*）和策略评估缓存（policy:*）
   */
  private async clearCache(): Promise<void> {
    await Promise.all([
      this.cacheService.clearPattern(`${this.cachePrefix}*`),
      this.cacheService.clearPattern('policy:*'),
    ]);
  }
}
