...
export class PolicyFactoryService {
  ...
  createPolicy(
    ...
    if (!factory) {
      throw new BadRequestException(I18nContext.current()?.t('error.policy.unknown_type') ?? `未知的策略类型: ${type}`);
    }

    const policy = factory(policyId, config);

    if (!policy.validateConfig(config)) {
      throw new BadRequestException(I18nContext.current()?.t('error.policy.config_validation_failed') ?? `策略配置验证失败: ${type}`);
    }
