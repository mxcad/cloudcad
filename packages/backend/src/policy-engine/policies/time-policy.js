 ...
 */
export class TimePolicy extends BasePolicy {
    ...
    async evaluate(context) {
        ...
        if (!inTimeRange) {
            return this.createDeniedResult(`当前时间 ${hour.toString().padStart(2, '0')}:${minute.toString().padStart(2, '0')} 不在允许的时间范围 ${this.config.startTime}-${this.config.endTime} 内`);
        }
        // 检查是否在允许的工作日内
        if (this.config.allowedDays && this.config.allowedDays.length > 0) {
            if (!this.config.allowedDays.includes(day)) {
                ...
                return this.createDeniedResult(`当前是 ${dayNames[day]}，不在允许的工作日内`);
            }
        }
