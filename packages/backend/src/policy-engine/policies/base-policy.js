    ...
    validateConfig(config) {
        ...
        for (const [key, value] of Object.entries(config)) {
            ...
            if (!this.validatePropertyType(value, property)) {
                this.logger.warn(`策略配置验证失败: 字段 ${key} 类型不正确，期望 ${property.type}`);
                return false;
            }
            // 检查枚举值
            if (property.enum && typeof value === 'string') {
                if (!property.enum.includes(value)) {
                    this.logger.warn(`策略配置验证失败: 字段 ${key} 的值 ${value} 不在允许的枚举值中`);
                    return false;
                }
            }
            // 检查数值范围
            if (property.type === 'number') {
                if (property.minimum !== undefined &&
                    ...
                    this.logger.warn(`策略配置验证失败: 字段 ${key} 的值 ${value} 小于最小值 ${property.minimum}`);
                    return false;
                }
                if (property.maximum !== undefined &&
                    typeof value === 'number' &&
                    value > property.maximum) {
                    this.logger.warn(`策略配置验证失败: 字段 ${key} 的值 ${value} 大于最大值 ${property.maximum}`);
                    return false;
                }
            }
        }
