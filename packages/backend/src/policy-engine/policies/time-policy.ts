...
export class TimePolicy extends BasePolicy implements IPermissionPolicy {
  ...
  private parseTime(timeStr: string): [number, number] {
    ...
    if (parts.length !== 2) {
      throw new BadRequestException(I18nContext.current()?.t('error.policy.invalid_time_format') ?? '无效的时间格式');
    }
    ...
    if (
      ...
      throw new BadRequestException(I18nContext.current()?.t('error.policy.invalid_time_value') ?? '无效的时间值');
    }
    return [hour, minute];
  }
