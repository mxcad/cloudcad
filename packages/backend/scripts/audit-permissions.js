...
import { DEFAULT_PROJECT_ROLE_PERMISSIONS, ProjectRole, SYSTEM_ROLE_PERMISSIONS, SystemRole, } from "../src/common/enums/permissions.enum";
// 将枚举键转换为字符串键，方便审计脚本使用
const SYSTEM_ROLE_PERMISSIONS_MAP = {
    [SystemRole.ADMIN]: SYSTEM_ROLE_PERMISSIONS[SystemRole.ADMIN],
    [SystemRole.USER_MANAGER]: SYSTEM_ROLE_PERMISSIONS[SystemRole.USER_MANAGER],
    [SystemRole.FONT_MANAGER]: SYSTEM_ROLE_PERMISSIONS[SystemRole.FONT_MANAGER],
    [SystemRole.USER]: SYSTEM_ROLE_PERMISSIONS[SystemRole.USER],
};
const PROJECT_ROLE_PERMISSIONS_MAP = {
    [ProjectRole.OWNER]: DEFAULT_PROJECT_ROLE_PERMISSIONS[ProjectRole.OWNER],
    [ProjectRole.ADMIN]: DEFAULT_PROJECT_ROLE_PERMISSIONS[ProjectRole.ADMIN],
    [ProjectRole.MEMBER]: DEFAULT_PROJECT_ROLE_PERMISSIONS[ProjectRole.MEMBER],
    [ProjectRole.EDITOR]: DEFAULT_PROJECT_ROLE_PERMISSIONS[ProjectRole.EDITOR],
    [ProjectRole.VIEWER]: DEFAULT_PROJECT_ROLE_PERMISSIONS[ProjectRole.VIEWER],
};
async function auditPermissions() {
    ...
    try {
        ...
        for (const [roleName, expectedPerms] of Object.entries(SYSTEM_ROLE_PERMISSIONS_MAP)) {
            ...
            const status = missingPerms.length === 0 && extraPerms.length === 0
                ? "ok"
        ...
        if (needsFixing.length > 0) {
            ...
            for (const result of needsFixing) {
                ...
                const permTable = result.roleType === "system"
                    ? '"RolePermission"'
                ...
                if (result.missingPerms.length > 0) {
                    ...
                    const values = result.missingPerms
                        .map((p) => `  ((SELECT id FROM ${roleTable} WHERE name = '${result.roleName}'), '${p}')`)
                        .join(",\n");
...
// 运行审计
auditPermissions();
//# sourceMappingURL=audit-permissions.js.map