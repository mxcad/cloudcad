...
const SYSTEM_ROLE_PERMISSIONS_MAP: Record<string, string[]> = {
	...
	[SystemRole.USER_MANAGER]: SYSTEM_ROLE_PERMISSIONS[
		SystemRole.USER_MANAGER
	] as string[],
	[SystemRole.FONT_MANAGER]: SYSTEM_ROLE_PERMISSIONS[
...
const PROJECT_ROLE_PERMISSIONS_MAP: Record<string, string[]> = {
	[ProjectRole.OWNER]: DEFAULT_PROJECT_ROLE_PERMISSIONS[
		ProjectRole.OWNER
	] as string[],
	[ProjectRole.ADMIN]: DEFAULT_PROJECT_ROLE_PERMISSIONS[
		ProjectRole.ADMIN
	] as string[],
	[ProjectRole.MEMBER]: DEFAULT_PROJECT_ROLE_PERMISSIONS[
		ProjectRole.MEMBER
	] as string[],
	[ProjectRole.EDITOR]: DEFAULT_PROJECT_ROLE_PERMISSIONS[
		ProjectRole.EDITOR
	] as string[],
	[ProjectRole.VIEWER]: DEFAULT_PROJECT_ROLE_PERMISSIONS[
		ProjectRole.VIEWER
	] as string[],
};
	...
	try {
		...
		for (const [roleName, expectedPerms] of Object.entries(
			...
			const status: "ok" | "warning" | "error" =
				missingPerms.length === 0 && extraPerms.length === 0
					? "ok"
		...
		if (needsFixing.length > 0) {
			...
			for (const result of needsFixing) {
				const roleTable =
					result.roleType === "system" ? '"Role"' : '"ProjectRole"';
				const permTable =
					result.roleType === "system"
						? '"RolePermission"'
						: '"ProjectRolePermission"';
				const roleIdField =
					result.roleType === "system" ? '"roleId"' : '"projectRoleId"';

				if (result.extraPerms.length > 0) {
					console.log(`-- 删除 ${result.roleName} 的多余权限`);
					console.log(`DELETE FROM ${permTable}`);
					console.log(
						`WHERE ${roleIdField} = (SELECT id FROM ${roleTable} WHERE name = '${result.roleName}')`,
					);
					console.log(
						`  AND permission IN (${result.extraPerms.map((p) => `'${p}'`).join(", ")});`,
					);
					console.log("");
				}
				...
				if (result.missingPerms.length > 0) {
					...
					const values = result.missingPerms
						...
						.join(",\n");
					console.log(
						`INSERT INTO ${permTable} (${roleIdField}, permission) VALUES`,
					);
					console.log(`${values};`);
					console.log("");
				}
