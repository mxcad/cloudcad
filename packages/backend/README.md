...
```bash
# 单元测试（默认语义：纯 unit，本地无 DB 可全绿；集成/E2E 已从默认配置排除）
pnpm test

# 测试覆盖率
pnpm test:cov

# 单元测试（与 pnpm test 同语义，显式别名）
pnpm test:unit

# 集成测试（需要 Docker，自动起 postgres/redis 测试栈 + 建库 + migrate + 跑测试）
pnpm test:integration:db

# 集成测试（已有本地 postgres/redis，直接用 5432/6379 默认连接；串行执行）
...
> 测试栈实现（8236062a）：`docker-compose.test.yml` 定义 `cloudcad-test-postgres`（5433）/`cloudcad-test-redis`（6380）容器，`scripts/test-db.mjs` 负责启停、等待就绪、建库与 migrate；环境变量 `TEST_DATABASE_URL` / `TEST_REDIS_URL` 可覆盖测试库连接（`src/test/setup.ts` 优先读取，缺省回退 5432/6379 本机）。
