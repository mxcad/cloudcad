...
describe('FileConversionUploadService - CAD Upload→Convert→Open Integration', () => {
    ...
    let fileSystemServiceMain;
    let fileSystemNodeService;
    ...
    beforeEach(async () => {
        ...
        service = module.get(FileConversionUploadService);
    ...
    describe('uploadAndConvertFileWithPermission', () => {
        describe('Case 1: Normal Flow - CAD File Upload, Conversion, and Node Creation', () => {
            it('should successfully upload, convert CAD file, and create file node', async () => {
                ...
                expect(fileSystemService.writeStatusFile).toHaveBeenCalledWith('test.dwg', 1024000, 'abc123def456', '/tmp/upload/test.dwg');
                expect(storageManager.allocateNodeStorage).toHaveBeenCalledWith('new-node-789', 'test.dwg');
                expect(fileSystemServiceMain.createFileNode).toHaveBeenCalledWith(expect.objectContaining({
                    name: 'test.dwg',
                    fileHash: 'abc123def456',
                    size: 1024000,
                    extension: '.dwg',
                ...
                expect(fileSystemServiceMain.updateNodePath).toHaveBeenCalledWith('new-node-789', expect.stringContaining('mxweb'));
                expect(versionControlService.commitNodeDirectory).toHaveBeenCalledWith(mockStorageInfo.nodeDirectoryPath, 'Upload file: test.dwg', 'user-123', 'Useruser-123');
            });
            it('should verify converted file name follows expected pattern', async () => {
                ...
                expect(uploadUtilityService.getConvertedFileName).toHaveBeenCalledWith('abc123def456', 'test.dwg');
        ...
        describe('Case 2: Conversion Failure', () => {
            ...
            it('should return error when conversion throws exception', async () => {
                fileConversionService.convertFile.mockRejectedValue(new Error('Conversion program not found'));
                const result = await service.uploadAndConvertFileWithPermission(mockUploadOptions);
                expect(result.ret).toBe(MxUploadReturn.kConvertFileError);
                expect(storageManager.allocateNodeStorage).not.toHaveBeenCalled();
                expect(fileSystemServiceMain.createFileNode).not.toHaveBeenCalled();
        ...
        describe('Case 3: Edge Cases', () => {
            ...
            it('should handle SVN commit failure gracefully', async () => {
                ...
                versionControlService.commitNodeDirectory.mockRejectedValue(new Error('SVN commit failed'));
                const result = await service.uploadAndConvertFileWithPermission(mockUploadOptions);
                expect(result.ret).toBe(MxUploadReturn.kOk);
                expect(versionControlService.commitNodeDirectory).toHaveBeenCalled();
            });
...
//# sourceMappingURL=cad-upload-convert.integration.spec.js.map