...
describe('Auth Token Refresh Integration Tests', () => {
    ...
    let mockSmsVerificationService;
    ...
    beforeEach(async () => {
        ...
        localAuthProvider = module.get(LocalAuthProvider);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('T23: Token过期刷新流程', () => {
        ...
        it('T23-S2: 使用过期RefreshToken刷新失败', async () => {
            ...
            await expect(authTokenService.refreshToken(expiredRefreshToken)).rejects.toThrow();
        ...
        it('T23-S3: 使用无效RefreshToken刷新失败', async () => {
            ...
            await expect(authTokenService.refreshToken(invalidRefreshToken)).rejects.toThrow(UnauthorizedException);
        });
        it('T23-S4: 使用数据库中不存在的RefreshToken刷新失败', async () => {
            ...
            await expect(authTokenService.refreshToken(nonExistentRefreshToken)).rejects.toThrow(UnauthorizedException);
        });
        it('T23-S5: RefreshToken过期但签名有效但数据库中已过期', async () => {
            ...
            mockDatabaseService.refreshToken.findFirst.mockResolvedValue(null);
            await expect(authTokenService.refreshToken(expiredButSignedToken)).rejects.toThrow(UnauthorizedException);
        });
        ...
        it('T23-S7: 用户被禁用后刷新Token失败', async () => {
            ...
            await expect(authTokenService.refreshToken(mockRefreshToken)).rejects.toThrow(UnauthorizedException);
        });
...
//# sourceMappingURL=auth-token-refresh.integration.spec.js.map