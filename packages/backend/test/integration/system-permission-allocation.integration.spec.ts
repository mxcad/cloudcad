...
import { PermissionCacheService } from '../../src/permission/services/permission-cache.service';
import { RoleInheritanceService } from '../../src/permission/services/role-inheritance.service';
import { DatabaseService } from '../../src/database/database.service';
import { AuditLogService } from '../../src/audit/audit-log.service';
import { SystemRole, SystemPermission, RoleCategory } from '../../src/common/enums/permissions.enum';
...
describe('T30 - System Permission Allocation Integration Tests', () => {
  ...
  let mockPermissionCacheService: any;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: 'IStorePermissionStrategy',
          useValue: {
            isEnabled: jest.fn().mockReturnValue(false),
            checkSystemPermission: jest.fn().mockResolvedValue(null),
            checkSystemPermissionsBatch: jest.fn().mockResolvedValue(null),
            clearUserCache: jest.fn().mockResolvedValue(true),
          },
        },
        {
          provide: 'IContextPermissionStrategy',
          useValue: {
            checkContextRules: jest.fn().mockResolvedValue(false),
          },
        },
        {
          provide: ClsService,
  ...
  describe('T30 - Permission Allocation and Cache Clearing', () => {
    it('T30-S1 - Allocate permission to role and clear cache', async () => {
      ...
      // 分配权限
      const result = await rolesService.addPermissions(
        mockRoleId,
    ...
    it('T30-S2 - Remove permission from role and clear cache', async () => {
      ...
      // 移除权限
      const result = await rolesService.removePermissions(
        mockRoleId,
        [SystemPermission.SYSTEM_USER_READ]
      );
    ...
    it('T30-S5 - Cache cleared, permission check gets fresh data', async () => {
      ...
      const hasPermissionBefore = await permissionService.checkSystemPermission(
        mockUserId,
        SystemPermission.SYSTEM_USER_READ
      );
      ...
      const hasPermissionAfter = await permissionService.checkSystemPermission(
        mockUserId,
        SystemPermission.SYSTEM_USER_READ
      );
    ...
    it('T30-S9 - System role permissions checked with inheritance', async () => {
      ...
      mockRoleInheritanceService.checkUserPermissionWithInheritance.mockImplementation(
        (userId, permission) => {
          const permissions = [
            SystemPermission.PROJECT_CREATE,
            SystemPermission.SYSTEM_USER_READ,
      ...
      // 测试继承的权限（来自USER）
      const hasProjectCreate = await mockRoleInheritanceService.checkUserPermissionWithInheritance(
        mockUserId,
        SystemPermission.PROJECT_CREATE
      );

      // 测试自己的权限
      const hasUserRead = await mockRoleInheritanceService.checkUserPermissionWithInheritance(
        mockUserId,
        SystemPermission.SYSTEM_USER_READ
      );

      // 测试没有的权限
      const hasFontRead = await mockRoleInheritanceService.checkUserPermissionWithInheritance(
        mockUserId,
    ...
    it('T30-S10 - Role permissions change takes effect immediately after cache clear', async () => {
      ...
      // 第一次检查权限（没有权限）
      const hasPermission1 = await permissionService.checkSystemPermission(
        mockUserId,
        SystemPermission.SYSTEM_USER_CREATE
      );
      ...
      await rolesService.addPermissions(
        mockRoleId,
        [SystemPermission.SYSTEM_USER_CREATE]
      );
      ...
      // 第二次检查权限（有权限了）
      const hasPermission2 = await permissionService.checkSystemPermission(
        mockUserId,
        SystemPermission.SYSTEM_USER_CREATE
      );
