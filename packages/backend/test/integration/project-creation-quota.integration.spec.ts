...
describe("Project Creation → Quota Allocation → Root Node Generation Integration", () => {
	...
	beforeAll(async () => {
		...
		testUserName = `projectuser${Date.now().toString().slice(-8)}`;
	...
	// beforeEach 创建的项目必须在 afterEach 删除，否则累计项目数超过配额上限（默认 5）导致后续用例 403
	...
	describe("T1: Project Creation Flow", () => {
		...
		it("T1-S1: Should successfully create a new project", async () => {
			...
			expect(response.body.data.description).toBe(projectDescription);
			expect(response.body.data.nodeType).toBe("PROJECT");
	...
	describe("T2: Project Query Flow", () => {
		...
		it("T2-S2: Should get project details by ID", async () => {
			...
			expect(response.body).toBeDefined();
			expect(response.body.data.id).toBe(projectIds[0]);
		});
	});
	...
	describe("T3: Project Update Flow", () => {
		...
		beforeEach(async () => {
			...
			updateProjectId = response.body.data.id;
		});

		afterEach(async () => {
			await deleteProjects([updateProjectId]);
		});
	...
	describe("T4: Project Archive/Unarchive Flow", () => {
		...
		beforeEach(async () => {
			const response = await request(app.getHttpServer())
				...
				.set("Authorization", `Bearer ${testUserAuthToken}`)
				.send({
					name: `Archive Test Project ${Date.now()}`,
					description: "Project for archive testing",
				});
			archiveProjectId = response.body.data.id;
		});

		afterEach(async () => {
			await deleteProjects([archiveProjectId]);
		});
