...
describe("File Operations - Create, Move, Copy, Rename Integration", () => {
	...
	async function setupTestUserAndProject() {
		const userRegister = await request(app.getHttpServer())
			...
			.send({
				email: testUserEmail,
				username: `fileopsuser${Date.now().toString().slice(-8)}`,
	...
	describe("T1: File and Folder Creation", () => {
		it("T1-S1: Should create a new folder in project", async () => {
			const createFolderResponse = await request(app.getHttpServer())
				.post(`/v1/file-system/nodes/${testProjectId}/folders`)
				.set("Authorization", `Bearer ${testUserAuthToken}`)
				.send({
					name: "Test Folder 1",
				});
			...
			if (createFolderResponse.body.data.id) {
				const folder = await prisma.fileSystemNode.findUnique({
					where: { id: createFolderResponse.body.data.id },
				});
		...
		it("T1-S2: Should create nested folders structure", async () => {
			...
			if (createParentResponse.body.data.id) {
				const parentFolderId = createParentResponse.body.data.id;

				const createChildResponse = await request(app.getHttpServer())
					.post(`/v1/file-system/nodes/${parentFolderId}/folders`)
					.set("Authorization", `Bearer ${testUserAuthToken}`)
					.send({
						name: "Child Folder",
					});
				...
				if (createChildResponse.body.data.id) {
					const childFolder = await prisma.fileSystemNode.findUnique({
						where: { id: createChildResponse.body.data.id },
					});
	...
	describe("T2: File and Folder Rename", () => {
		...
		beforeEach(async () => {
			...
			testFolderId = createResponse.body.data.id;
		});
	...
	describe("T3: File and Folder Move", () => {
		...
		beforeEach(async () => {
			...
			sourceFolderId = sourceResponse.body.data.id;
			...
			targetFolderId = targetResponse.body.data.id;

			const fileResponse = await request(app.getHttpServer())
				.post(`/v1/file-system/nodes/${sourceFolderId}/folders`)
				.set("Authorization", `Bearer ${testUserAuthToken}`)
				.send({
					name: "Test File Node",
				});

			testFileId = fileResponse.body.data.id;
		});
	...
	describe("T5: Folder Navigation and Children Listing", () => {
		...
		beforeEach(async () => {
			...
			const parentResponse = await request(app.getHttpServer())
				...
				.set("Authorization", `Bearer ${testUserAuthToken}`)
				.send({
					name: `Navigation Parent ${unique}`,
				});

			parentFolderId = parentResponse.body.data.id;
			...
			childFolderId1 = child1Response.body.data.id;
			...
			childFolderId2 = child2Response.body.data.id;
		});
	...
	describe("T6: Complete File Operations Chain Integration", () => {
		it("T6-S1: Should complete full file operations chain - create → rename → move → copy → verify", async () => {
			...
			const targetFolderId = targetFolderResponse.body.data.id;
