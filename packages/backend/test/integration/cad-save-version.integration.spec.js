...
describe('CAD Save → SVN Commit → Version History Integration', () => {
    ...
    describe('T1: CAD Save → SVN Commit Chain', () => {
        it('T1-S1: Save CAD file to project space triggers SVN commit', async () => {
            ...
            const commitCall = mockVersionControl.commitNodeDirectory.mock.calls[0];
        ...
        it('T1-S3: SVN commit failure does not break save operation', async () => {
            mockVersionControl.commitNodeDirectory.mockRejectedValueOnce(new Error('SVN commit failed'));
            const mockFile = {
                path: tempFilePath,
                originalname: 'test.mxweb',
                mimetype: 'application/octet-stream',
    ...
    describe('T2: Version History Retrieval', () => {
        it('T2-S1: getFileHistory returns correct commit entries after save', async () => {
            const historyResult = await mockVersionControl.getFileHistory('project/node-123/test.dwg', 10);
        ...
        it('T2-S2: getFileHistory handles empty history', async () => {
            ...
            const historyResult = await mockVersionControl.getFileHistory('project/node-123/empty.dwg', 10);
            expect(historyResult.success).toBe(true);
        ...
        it('T2-S3: getFileHistory handles SVN errors gracefully', async () => {
            ...
            const historyResult = await mockVersionControl.getFileHistory('project/node-123/error.dwg', 10);
    ...
    describe('T3: Full CAD Save → SVN Commit → Version History Chain', () => {
        it('T3-S1: Complete chain - save triggers commit and history is queryable', async () => {
            ...
            const historyResult = await mockVersionControl.getFileHistory('project/node-123/project.dwg', 50);
            expect(historyResult.success).toBe(true);
...
//# sourceMappingURL=cad-save-version.integration.spec.js.map