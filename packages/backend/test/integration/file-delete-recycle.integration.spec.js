...
describe("File Delete & Recycle Integration Tests", () => {
    ...
    describe("Test Case 1: Multiple references - delete one should only decrease count", () => {
        it("should verify that deleting one of multiple files with same hash does not delete the physical file", async () => {
            ...
            // Setup mocks
            databaseService.fileSystemNode.findUnique.mockResolvedValueOnce(file1);
            // When counting references, should find 1 other file with same hash still active
            databaseService.fileSystemNode.count.mockResolvedValueOnce(1);
            // Execute delete
            ...
            expect(databaseService.fileSystemNode.count).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe("Test Case 2: Zero references - delete should move to recycle bin", () => {
        it("should verify that deleting the last reference moves file to recycle bin", async () => {
            ...
            // Setup mocks
            databaseService.fileSystemNode.findUnique.mockResolvedValueOnce(file);
            // When counting references, should find 0 other files with same hash
            databaseService.fileSystemNode.count.mockResolvedValueOnce(0);
            // Execute soft delete
            ...
            expect(databaseService.fileSystemNode.update).toHaveBeenCalledWith(expect.objectContaining({
                where: { id: file.id },
    ...
    describe("Test Case 3: Restore file - reference count should be restored", () => {
        it("should verify that restoring a file restores the reference count", async () => {
            ...
            // Setup mocks
            databaseService.fileSystemNode.findUnique
            ...
            databaseService.fileSystemNode.update.mockResolvedValueOnce(restoredFile);
            // Execute restore
            const result = await fileOperationsService.restoreNode(deletedFile.id, testUserId);
    ...
    describe("Test Case 4: Full workflow - create → multiple references → delete one → delete last → restore", () => {
        it("should verify the complete file delete and restore workflow", async () => {
            ...
            databaseService.fileSystemNode.findUnique.mockResolvedValueOnce({
                ...file1,
                isFolder: false,
                parentId: testProjectId,
                ownerId: testUserId,
            });
            databaseService.fileSystemNode.count.mockResolvedValueOnce(2); // 2 other references
            const delete1Result = await fileOperationsService.deleteNode(file1.id, false);
            ...
            databaseService.fileSystemNode.count.mockResolvedValueOnce(1); // 1 other reference
            const delete2Result = await fileOperationsService.deleteNode(file2.id, false);
            ...
            databaseService.fileSystemNode.count.mockResolvedValueOnce(0); // 0 other references
            const delete3Result = await fileOperationsService.deleteNode(file3.id, false);
            ...
            databaseService.fileSystemNode.update.mockResolvedValueOnce({
                ...deletedFile,
                deletedAt: null,
                fileStatus: "COMPLETED",
            });
            const restoreResult = await fileOperationsService.restoreNode(file3.id, testUserId);
            expect(restoreResult).toBeDefined();
        });
    });
});
//# sourceMappingURL=file-delete-recycle.integration.spec.js.map