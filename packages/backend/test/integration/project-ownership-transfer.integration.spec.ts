...
describe('项目域：转让所有权→原所有者权限失效集成测试', () => {
  ...
  beforeEach(async () => {
    ...
    projectMemberService = moduleFixture.get<ProjectMemberService>(ProjectMemberService);
  });
  ...
  describe('T1: 正常转让所有权流程', () => {
    it('T1-S1: 项目所有者可以将所有权转让给项目成员', async () => {
      ...
      mockDatabaseService.fileSystemNode.findFirst.mockResolvedValue(mockProject);
  ...
  describe('T2: 权限验证场景', () => {
    it('T2-S1: 非项目所有者不能转让项目所有权', async () => {
      ...
      await expect(
        projectMemberService.transferProjectOwnership(
          mockProjectId,
          mockNewOwnerId,
          'non-owner-id'
        )
  ...
  describe('T3: 转让目标验证场景', () => {
    ...
    it('T3-S2: 项目不存在时应抛出异常', async () => {
      mockDatabaseService.fileSystemNode.findFirst.mockResolvedValue(null);
  ...
  describe('T4: 原所有者权限变更验证', () => {
    it('T4-S1: 转让后新所有者获得所有者角色', async () => {
      ...
      await projectMemberService.transferProjectOwnership(
        mockProjectId,
        mockNewOwnerId,
        mockOriginalOwnerId
      );
    ...
    it('T4-S3: 项目所有者ID应正确更新', async () => {
      ...
      mockDatabaseService.fileSystemNode.findFirst.mockResolvedValue(mockProject);
      mockDatabaseService.projectMember.findUnique.mockResolvedValue(mockNewOwnerMember);
      mockDatabaseService.projectRole.findFirst.mockResolvedValue(mockOwnerRole);
      ...
      tx.projectRole.findFirst.mockResolvedValue(mockOwnerRole);
