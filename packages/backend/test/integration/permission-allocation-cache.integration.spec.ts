...
import { PrismaClient, ProjectStatus, UserStatus } from "@cloudcad/db";
...
describe("Permission Allocation → Cache Clearing → Permission Effectiveness Integration", () => {
	...
	async function setupTestUsers() {
		const ownerRegister = await request(app.getHttpServer())
			...
			.send({
				email: ownerEmail,
				username: `owneruser${Date.now().toString().slice(-8)}`,
				password: ownerPassword,
				nickname: "Project Owner",
			});
		ownerId = ownerRegister.body.data.user.id;
		ownerToken = ownerRegister.body.data.accessToken;

		const memberRegister = await request(app.getHttpServer())
			.post("/v1/auth/register")
			.send({
				email: memberEmail,
				username: `memberuser${Date.now().toString().slice(-8)}`,
				password: memberPassword,
				nickname: "Project Member",
			});
		memberId = memberRegister.body.data.user.id;
		memberToken = memberRegister.body.data.accessToken;
	}
	...
	describe("T2: Adding Project Member with Role", () => {
		...
		it("T2-S1: Should add member to project with viewer role", async () => {
			...
			expect(addMemberResponse.body.data.user.id).toBe(memberId);
		});
		...
		it("T2-S3: Should verify member can access project with viewer permissions", async () => {
			...
			expect(permissionsResponse.body.data.permissions).toBeDefined();
		});
	});
	...
	describe("T3: Updating Member Role", () => {
		...
		it("T3-S1: Should update member role from viewer to editor", async () => {
			...
			expect(updateRoleResponse.body.data.projectRoleId).toBe(editorRoleId);
		});
	...
	describe("T4: Removing Member from Project", () => {
		...
		it("T4-S3: Should verify removed member cannot access project", async () => {
			...
			expect(permissionsResponse.body.data.role).toBeNull();
		});
	});
	...
	describe("T5: Permission Denied Testing", () => {
		...
		beforeAll(async () => {
			...
			anotherProjectId = createProject.body.data.id;
		});
	...
	describe("T6: Complete Permission Allocation Chain Integration", () => {
		it("T6-S1: Should complete full permission chain - create project → add member → change role → remove member → verify permissions", async () => {
			...
			await request(app.getHttpServer())
				.delete(
					`/v1/file-system/projects/${chainProjectId}/members/${memberId}`,
				)
				.set("Authorization", `Bearer ${ownerToken}`)
				.expect(200);

			const finalPermissionsResponse = await request(app.getHttpServer())
				.get(`/v1/file-system/projects/${chainProjectId}/permissions`)
				.set("Authorization", `Bearer ${memberToken}`)
				.expect(200);

			expect(finalPermissionsResponse.body.data.permissions).toEqual([]);
			expect(finalPermissionsResponse.body.data.role).toBeNull();
		});
	});
});
