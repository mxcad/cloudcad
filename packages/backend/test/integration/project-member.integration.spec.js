...
describe('项目成员管理集成测试', () => {
    ...
    let testProject;
    let ownerAuthToken;
    let member1AuthToken;
    let member2AuthToken;
    let ownerProjectRole;
    let adminProjectRole;
    let editorProjectRole;
    let viewerProjectRole;
    beforeAll(async () => {
    ...
    describe('测试用例1：添加项目成员', () => {
        ...
        it('添加成员后应该能够在成员列表中看到', async () => {
            ...
            const member1 = response.body.find((m) => m.user && m.user.id === testMember1.id);
            const member2 = response.body.find((m) => m.user && m.user.id === testMember2.id);
    ...
    describe('测试用例4：转让项目所有权（通过服务层）', () => {
        ...
        it('应该能够成功转让所有权给成员1', async () => {
            ...
            const result = await projectMemberService.transferProjectOwnership(testProject.id, testMember1.id, testOwner.id);
...
//# sourceMappingURL=project-member.integration.spec.js.map