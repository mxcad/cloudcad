...
import { log, resolveRequestId, runWithRequest, getLogDir } from '../lib/logger';
import { PROJECT_ROOT } from '../lib/constants';

// 测试专用日志目录（logger 在调用时读取 env，支持运行时覆盖）
process.env.LOG_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'conv-svc-logs-'));
...
describe('logger (零依赖 JSON 日志)', () => {
  it('输出 JSON 单行，含 time/level/service/message', () => {
    ...
    assert.equal(record.service, 'conversion-service');
  ...
  it('resolveRequestId 回传入站合法 id 并写入响应头', () => {
    ...
    const res = { setHeader: (k: string, v: string) => { if (k === 'X-Request-Id') headerValue = v; } };
  ...
  it('日志落盘到 data/logs/<service>/app-YYYY-MM-DD.log', () => {
    ...
    const file = path.join(process.env.LOG_DIR!, 'conversion-service', `app-${dateStr}.log`);
  ...
  it('过期日志文件按保留策略清理', async () => {
    const dir = path.join(process.env.LOG_DIR!, 'conversion-service');
