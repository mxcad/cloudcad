...
describe('WorkerPool', () => {
  ...
  it('should complete a task and store the result', async () => {
    ...
    assert.equal(task.result.newpath, '/out/a.mxweb');
    pool.stop();
  });
  ...
  it('should fire the callback engine on terminal states', async () => {
    ...
    const ok = calls.find((c) => c.taskId === 'cb_ok');
    const fail = calls.find((c) => c.taskId === 'cb_fail');
  ...
  it('should be a no-op when no callback engine is wired', async () => {
    ...
    assert.equal(store.get('no_cb').result.newpath, '/out/a.mxweb');
    pool.stop();
  });
});
