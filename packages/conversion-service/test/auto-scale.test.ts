...
function waitUntil(predicate: () => boolean, timeout = 10000, interval = 20): Promise<void> {
...
function makeSlowRunner(holdMs: number) {
  let concurrent = 0;
  let maxConcurrent = 0;
  const runner = {
...
describe('WorkerPool 自动扩容', () => {
  ...
  it('默认不自动扩容（未显式传 autoScale 时保持 baseline，#431 门禁2）', async () => {
    const pool = new WorkerPool(store, makeSlowRunner(300));
    pool.start();
    ...
    // 默认 autoScale=false：积压 6 个（> baseline*threshold=2*2=4）也不提升并发，
    // 避免 mxcadassembly 孤儿/CPU 打满（8-28 事故）
    await sleep(400);
    assert.equal(pool.pools['1'].max, 2);
    pool.stop();
  });
