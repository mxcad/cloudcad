...
function setAnchorHeight() {
  const idx = Math.min(props.initialHeightIndex, anchors.value.length - 1)
  const h = anchors.value[idx] ?? boundary.value.min
  currentVisible.value = Math.max(h, boundary.value.min)
}

function snapTo(index: number) {
  const idx = Math.min(index, anchors.value.length - 1)
  currentVisible.value = anchors.value[idx] ?? boundary.value.min
  if (!dragging.value) {
    emit('position-change', { y: currentVisible.value })
  }
}

function resetPreservedHeight() {
  currentVisible.value = 0
}
...
watch(() => props.show, (val) => {
  if (val) {
    if (!renderNow.value) {
      ...
      innerShow.value = true
      if (!props.preserveHeightOnReopen || currentVisible.value === 0) {
        setAnchorHeight()
      }
      lockBodyScroll(true)
      emit('open')
...
defineExpose({ snapTo, resetPreservedHeight })
</script>
...
.van-floating-popup {
  ...
  touch-action: none;
  background: var(--van-floating-popup-background);
  color: #fff;
  ...
  :deep(.van-floating-popup__content) {
    ...
    min-height: 0;
    background-color: var(--van-floating-popup-background, var(--bg-primary));
  }
