...
const show = ref(true);
const target = ref<'login' | 'register'>('login');
...
<template>
  ...
  >
    ...
    <div v-else class="login-prompt-body">
      ...
      <p class="login-prompt-title">{{ target === 'register' ? t('正在跳转注册...') : t('正在跳转登录...') }}</p>
      <p class="login-prompt-desc">{{ target === 'register' ? t('请在打开的页面中完成注册') : t('请在打开的页面中完成登录') }}</p>
    </div>
    <template #footer>
      <template v-if="!waiting">
        ...
        <van-button
          ...
          round
          size="large"
          class="login-prompt-register-btn"
          @click="onRegister"
        >
          {{ t('前往注册') }}
        </van-button>
