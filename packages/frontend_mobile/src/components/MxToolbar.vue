...
const translate = (key: string) => {
  // read activeLanguage to create reactive dependency so template re-renders on language change
        ...
        <div class="items">
            <template v-for="(item, index) in items">
                <button class="item" @tap="tap($event, item, index)">
                        ...
                        <span v-if="!isHideItemName" class="name">{{ translate(item.name || "") }}</span>
                </button>
            </template>
        </div>
    </div>
...
.mx-toolbar {
    ...
    --toolbar-name-min-width: 42px;
    --toolbar-zoomed-icon-size: 2;
    --toolbar-item-padding: 5px 3px;
