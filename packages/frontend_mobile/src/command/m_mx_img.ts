...
import { DetailedResult, MxFun, MxType } from 'mxdraw';
import { showToast } from 'vant';
import { addPendingImage } from '@/services/pendingImageService';
    ...
    if (input.files && input.files[0]) {
      ...
      try {
        mxcad.loadImage(url, async (image) => {
          ...
          if (!point1) return;
          const zoomRatio = getRetainDecimalPlaces(
            point1.distanceTo(basePt) / img.width,
            3
          );
...
addCommand('Mx_InsertImageWithUpload', () => {
  MxFun.sendStringToExecute(
    '_InsertImage',
    (data: { url: string; fileName: string; entity: unknown }) => {
