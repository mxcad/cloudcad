...
export type SaveFormat = 'dwg' | 'dxf' | 'mxweb';
...
export function useSaveAs() {
  ...
  const libraryRootId = ref<string | null>(null);
  ...
  async function loadLibraryRoot(type?: LibraryType) {
    const libType = type || libraryType.value;
    try {
      const apiFn = libType === 'drawing'
        ? libraryControllerGetDrawingLibrary
        : libraryControllerGetBlockLibrary;
      const result = await apiFn();
      if (result.error) throw result.error;
      const data = result.data as unknown as { id: string };
      libraryRootId.value = data?.id || null;
    } catch {
      libraryRootId.value = null;
    }
  }
  ...
  return {
    ...
    canSaveToLibrary,
    setCanSaveToLibrary,
    libraryRootId: readonly(libraryRootId),
    init,
    loadProjects,
    loadPersonalSpace,
    loadLibraryRoot,
