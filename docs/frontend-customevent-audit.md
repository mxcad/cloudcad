...
本文档扫描了 apps/frontend/src/ 中所有使用 `window.dispatchEvent` 和 `window.addEventListener` 的位置，用于后续收敛到 `useCadEvents`。
...
**文件**：`apps/frontend/src/services/mxcadManager.ts`
...
**文件**：`apps/frontend/src/contexts/ThemeContext.tsx`

| 行号 | 位置 | detail 数据 | 说明 |
|------|------|-------------|------|
| 230 | `setTheme` | `{ theme: Theme }` | 主题变化 |

**文件**：`apps/frontend/src/pages/CADEditorDirect.tsx`
...
**文件**：`apps/frontend/src/contexts/NotificationContext.tsx`
