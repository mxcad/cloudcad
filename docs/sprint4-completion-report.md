...
**审计范围：** `apps/frontend-vue/` 前端迁移验收 & `apps/backend/src/` 安全审计修复
...
对照 React 源文件（`apps/frontend/src/pages/`）与 Vue 迁移版本（`apps/frontend-vue/src/pages/`），逐页检查以下交互细节：
- 空状态占位提示
- 骨架屏加载样式
- 错误提示措辞和展示位置
...
|---|---------|---------|---------|---------|------|
| 1 | CORS `origin: true` 允许任意来源 | 🟡 中 | `apps/backend/src/main.ts` | 改为明确域名白名单，生产环境读取 `CORS_ORIGINS` / `FRONTEND_URL` 环境变量 | ✅ 已修复 |
| 2 | CSRF 保护缺失，敏感操作无验证 | 🟡 中 | `apps/backend/src/main.ts` `apps/backend/src/app.module.ts` `apps/backend/src/auth/guards/csrf.guard.ts` `apps/backend/src/auth/decorators/csrf-protected.decorator.ts` | 新增 `@CsrfProtected()` 装饰器和全局 `CsrfGuard`，自动验证 `X-CSRF-Token` 请求头 | ✅ 已修复 |
| 3 | 搜索接口 `keyword` 无最大长度限制 | 🟡 中 | `apps/backend/src/file-system/dto/search.dto.ts` | 添加 `@MaxLength(200)` 校验 | ✅ 已修复 |
| 4 | 成员列表接口权限过低（仅需 FILE_OPEN） | 🟡 中 | `apps/backend/src/file-system/file-system.controller.ts` | 将 `@RequireProjectPermission(ProjectPermission.FILE_OPEN)` 提升为 `PROJECT_MEMBER_MANAGE` | ✅ 已修复 |
| 5 | `checkProjectPermission` 的 `permission` 参数无枚举校验 | 🟡 中 | `apps/backend/src/file-system/file-system.controller.ts` | 将 `@Query('permission') permission: string` 改为 `permission: ProjectPermission`，利用 class-validator 自动校验枚举值 | ✅ 已修复 |
...
- `apps/backend/src/auth/decorators/csrf-protected.decorator.ts` — `@CsrfProtected()` 装饰器
- `apps/backend/src/auth/guards/csrf.guard.ts` — 全局 CSRF 验证 Guard
