...
**扫描日期**: 2026-05-03  
**报告人**: Trae  
**扫描范围**: `apps/frontend/src/**/*.ts*`

---

## 1. 概述

本报告对 `apps/frontend/src/` 中所有使用 `window.dispatchEvent()` 或 `window.addEventListener()` 的位置进行了全面扫描和清单整理。这些事件在 Vue 3 迁移中需要收敛到 `useCadEvents`。
...
**文件**: `apps/frontend-vue/src/composables/useCadEvents.ts`
