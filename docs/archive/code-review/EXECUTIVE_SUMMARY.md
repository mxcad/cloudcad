...
| **0** | conversion 模块的测试覆盖率（文件转换核心管线无任何测试。**2026-08-03 已随 `src/conversion/` 模块删除而解决，ADR-0014 Amendment 1**；实际转换路径 `mxcad/conversion/file-conversion.service.ts` 已有独立 spec） |
...
| **P2** | 制定测试补充计划（controller 层 P1；~~conversion 模块 P0~~ 已随模块删除取消） | 计划 0.5 天 | ⏳ 待处理 |
