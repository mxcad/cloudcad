...
description: 图纸版本部署规范 — 图纸版本管理涉及的 SVN 配置完整性验证与部署前后自动检查。触发条件：(1) 修改 packages/mxVersionTool/ 下任意文件；(2) 修改 packages/backend/src/version-control/ 下任意文件；(3) 修改 packages/backend/src/storage-provider/ 下任意文件；(4) 修改 packages/storage-service/ 下任意文件；(5) 变更 MX_REPO_PATH / FILES_DATA_PATH / MX_VERSION_TOOL_PATH 等图纸版本路径配置；(6) 变更文件存储路径规范；(7) 变更部署脚本中版本控制运行时相关；(8) 变更 .env/.env.example 中图纸版本配置变量。
---
