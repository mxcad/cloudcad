...
## Prorated Upgrade — 升级价差折算延期（ADR-0025）

### 问题

用户从低等级 VIP 升级到高等级时，旧 VIP 剩余时间按价差比例折算为额外的新等级天数，而非简单叠加。

### 算法

```typescript
extraDays = Math.floor(remainingDays * oldTierBaseMonthlyPrice / newTierBaseMonthlyPrice);
totalDays = purchasedDays + extraDays;
```

### 实施位置

`BillingService.handlePaymentNotify()` 中调用 `MembershipService.activate()` 之前，先调用 `calculateUpgradeExtraDays()`：

```typescript
// handlePaymentNotify 内部
if (order.vipTier) {
  const durationDays = (order.months ?? 1) * 30;
  const effectiveDays = await this.calculateUpgradeExtraDays(
    tx, order.userId, order.vipTier, durationDays
  );
  await this.membershipService.activate(tx as any, order.userId, order.vipTier.level, effectiveDays);
}
```
...
> 详细决策记录见 `docs/adr/0025-prorated-upgrade-pricing.md`。
