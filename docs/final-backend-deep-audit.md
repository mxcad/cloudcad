...
| **审计日期** | 2025 |
|------------|------|
| **审计范围** | apps/backend/src/ 所有 14 个核心模块 + 辅助模块 |
...
**发现问题：**
- `apps/backend/src/admin/` 目录下缺少 `.spec.ts` 测试文件
...
**发现问题：**
- `apps/backend/src/audit/` 目录下缺少 `.spec.ts` 测试文件
...
**文件清单：**
- `apps/backend/src/auth/auth-facade.service.spec.ts` - 完整的认证服务测试
...
**文件清单：**
- `apps/backend/src/cache-architecture/providers/l3-cache.provider.ts` - L3 缓存实现
- `apps/backend/src/common/schedulers/cache-cleanup.scheduler.ts` - 使用 `@nestjs/schedule` 的定时任务
...
- `apps/backend/src/file-operations/project-crud.service.spec.ts`
...
**文件清单：**
- `apps/backend/src/file-system/file-system.service.spec.ts`
- `apps/backend/src/file-system/file-tree/file-tree.service.spec.ts`
- `apps/backend/src/file-system/file-validation/file-validation.service.spec.ts`
...
**文件清单：**
- `apps/backend/src/mxcad/core/mxcad.controller.spec.ts`
...
**文件清单：**
- `apps/backend/src/policy-engine/policy-engine.module.ts` - 模块定义，使用 forwardRef
- `apps/backend/src/policy-engine/services/policy-engine.service.ts` - 策略引擎服务
- `apps/backend/src/policy-engine/services/policy-config.service.ts` - 策略配置服务
...
**文件清单：**
- `apps/backend/src/users/users.service.ts` - 用户服务实现
...
|---------|-----------|-----------|---------|-----------|
| 1 | 缺少 `users.service.spec.ts` 测试文件 | `apps/backend/src/users/` | - | 添加完整的用户服务测试 |
| 2 | `policy-config.service.ts` 中 `clearCache` 方法未实现 | `apps/backend/src/policy-engine/services/policy-config.service.ts` | 449 | 实现 clearPattern 方法或其他清除缓存的方式 |

### 4.2 中优先级问题

| **序号** | **问题描述** | **文件路径** | **行号** | **建议修复** |
|---------|-----------|-----------|---------|-----------|
| 3 | 缺少 `admin` 模块测试文件 | `apps/backend/src/admin/` | - | 添加测试 |
| 4 | 缺少 `audit` 模块测试文件 | `apps/backend/src/audit/` | - | 添加测试 |
| 5 | 缺少 `cache-architecture` 模块测试文件 | `apps/backend/src/cache-architecture/` | - | 添加测试 |
| 6 | 缺少 `common` 模块完整测试 | `apps/backend/src/common/` | - | 补充测试 |
| 7 | 缺少 `database` 模块测试文件 | `apps/backend/src/database/` | - | 添加测试 |
| 8 | 缺少 `fonts` 模块测试文件 | `apps/backend/src/fonts/` | - | 添加测试 |
| 9 | 缺少 `health` 模块测试文件 | `apps/backend/src/health/` | - | 添加测试 |
| 10 | 缺少 `library` 模块测试文件 | `apps/backend/src/library/` | - | 添加测试 |
| 11 | 缺少 `personal-space` 模块测试文件 | `apps/backend/src/personal-space/` | - | 添加测试 |
| 12 | 缺少 `policy-engine` 模块测试文件 | `apps/backend/src/policy-engine/` | - | 添加测试 |
| 13 | 缺少 `public-file` 模块测试文件 | `apps/backend/src/public-file/` | - | 添加测试 |
| 14 | 缺少 `redis` 模块测试文件 | `apps/backend/src/redis/` | - | 添加测试 |
| 15 | 缺少 `roles` 模块测试文件 | `apps/backend/src/roles/` | - | 添加测试 |
| 16 | 缺少 `runtime-config` 模块测试文件 | `apps/backend/src/runtime-config/` | - | 添加测试 |
| 17 | 缺少 `storage` 模块测试文件 | `apps/backend/src/storage/` | - | 添加测试 |
