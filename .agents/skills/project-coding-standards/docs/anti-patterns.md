...
| `git restore` / `git reset` / `git checkout --` / `git checkout .` / `git checkout <ref> -- <path>` / `git clean -f` / `git revert` / `git stash`（全部形式）/ `git switch -f` / `git checkout <branch>` 等任何可能改变工作区/暂存区/HEAD 状态的操作 | 禁止任何形式的 git 恢复、回退、暂存、切分支操作（钩子 `.opencode/plugins/block-dangerous-git.ts` 硬拦截），需要撤销或临时验证时改用非破坏性方式（手动编辑文件、复制文件到临时目录、提交新改动） |
