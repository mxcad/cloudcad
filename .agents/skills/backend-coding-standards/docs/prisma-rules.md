...
```bash
# 1. 修改 schema.prisma（packages/db/prisma/schema.prisma）
# 2. 生成 migration 脚本
...
schema 单一源在 `packages/db/prisma/schema.prisma`；**所有 Prisma 类型（模型、枚举、`Prisma` namespace、`PrismaClient`）一律从 `@cloudcad/db` 导入，禁止 `import ... from '@prisma/client'`**（ADR-0027）。详见 `docs/type-acquisition.md`。

```typescript
// ✅ 正确：从 @cloudcad/db 取数据层类型
import { Prisma, PrismaClient, $Enums } from '@cloudcad/db';
```
...
修改 schema.prisma（`packages/db/prisma/schema.prisma`）后：
...
- [ ] 数据层类型从 `@cloudcad/db` 导入（无 `@prisma/client` 引用）
- [ ] 关联写操作包裹在事务中
