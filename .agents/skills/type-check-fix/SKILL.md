...
### Step 5 — import 自动整理陷阱

项目**不配置 Biome**（后端曾存在 `biome.json`，因 `organizeImports` 会把 DI 类改为 `import type` 而移除，全仓统一 Prettier）。但任何 `organizeImports` 类工具或 AI 都可能把 NestJS DI 需要的 `import` 自动改为 `import type`：

```typescript
// ❌ organizeImports 类工具或 AI 会自动改成这个（破坏 DI）
