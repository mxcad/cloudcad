...
- **Storage Service → Conversion Service**: 上传事件触发转换；批量下载任务委托
- **Conversion Service → Storage Service**: 转换结果写入 `data/files/`；SVN 提交
- **API Server**: 通过 `packages/mxVersionTool`（SVN CLI 包装器）进行文件版本控制（嵌入式模式下）
