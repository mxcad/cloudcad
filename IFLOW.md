...
| 触发条件 | 子代理 | 额外指令 |
|---------|--------|----------|
| `packages/backend/` 路径相关 | `cloucad-backend-expert` | 先输出「后端开发检查清单」再编码 |
| `packages/frontend/` 路径相关 | `cloucad-frontend-expert` | 先输出「前端开发检查清单」再编码 |
| `prisma/` 或数据库相关 | `cloucad-database-expert` | 先执行「数据库设计审查」再操作 |
| `docker-compose` 或部署相关 | `cloucad-devops-expert` | 必须附带「部署风险评估」 |
| `**/*.spec.ts` 或测试相关 | `cloucad-testing-expert` | 必须执行「测试覆盖率检查」再编码 |
| 安全、认证、权限相关 | `cloucad-security-expert` | 必须执行「安全风险评估」再操作 |
| `file-system/` 或文件操作相关 | `cloucad-file-system-expert` | 必须执行「文件系统架构审查」再操作 |
| 架构设计、技术选型相关 | `cloucad-architecture-expert` | 必须输出「架构设计决策」再设计 |
| 代码提交、PR、质量检查相关 | `cloucad-code-reviewer` | 必须执行「全面代码审查」再合并 |
| 关键词「后端/NestJS/API」 | `cloucad-backend-expert` | 必须给出「API 设计规范」 |
| 关键词「前端/React/组件」 | `cloucad-frontend-expert` | 必须给出「组件设计规范」 |
| 关键词「数据库/Prisma/迁移」 | `cloucad-database-expert` | 必须输出「数据模型设计」 |
| 关键词「部署/Docker/运维」 | `cloucad-devops-expert` | 必须附带「基础设施检查」 |
| 关键词「测试/Jest/覆盖率」 | `cloucad-testing-expert` | 必须给出「测试策略设计」 |
| 关键词「安全/认证/权限/加密」 | `cloucad-security-expert` | 必须执行「安全合规检查」 |
| 关键词「文件/FileSystemNode/CAD」 | `cloucad-file-system-expert` | 必须执行「文件权限检查」再操作 |
| 关键词「架构/微服务/技术选型」 | `cloucad-architecture-expert` | 必须输出「架构演进路线」再决策 |
| 关键词「代码审查/Code Review/PR/质量」 | `cloucad-code-reviewer` | 必须输出「代码审查报告」再决策 |
...
### 1. 主导式协同（核心模式）
- **主导子智能体**: 根据主要任务领域确定
- **辅助子智能体**: 根据任务复杂度和依赖关系调用 1-3 个辅助专家
- **协同流程**: 
  1. 主导子智能体分析需求，制定初步方案
  2. 主导调用相关辅助子智能体进行专业评审
  3. 辅助子智能体返回评审意见和改进建议
  4. 主导子智能体整合所有反馈，输出最终方案

### 2. 并行式协同（复杂任务）
- **适用场景**: 需要多领域同时独立处理的复杂任务
- **协同方式**: 主上下文同时调用多个子智能体并行处理
- **结果整合**: 主导子智能体负责整合各领域结果，确保一致性

### 3. 串行式协同（依赖任务）
- **适用场景**: 有明确前后依赖关系的任务
- **协同方式**: 按依赖顺序依次调用子智能体
- **传递机制**: 前置子智能体的输出作为后置子智能体的输入

### 4. 子智能体间通信协议
```typescript
interface SubAgentMessage {
  from: string;           // 发送方子智能体ID
  to: string;             // 接收方子智能体ID
...
- **任务上下文**: 所有子智能体共享任务上下文
- **进度同步**: 关键节点需要同步进度状态
- **冲突解决**: 不同意见时以主导子智能体决策为准

### 6. 协同示例
```
任务：新增文件上传功能
协同流程：
1. cloucad-backend-expert（主导）分析需求，制定API接口初步方案
2. 主导调用 cloucad-file-system-expert 评审文件存储方案
3. 主导调用 cloucad-security-expert 评审安全检查机制
4. 主导调用 cloucad-database-expert 评审数据存储设计
5. 主导调用 cloucad-testing-expert 评审测试策略
6. cloucad-backend-expert 整合所有反馈，输出完整实现方案
7. 实现完成后，cloucad-code-reviewer 进行全面代码审查
8. 根据审查结果修复问题，直至通过审查
```

### 7. Code Review 协同机制
- **触发时机**: 代码提交、PR 创建、功能完成时自动触发
- **审查流程**: 
  1. 自动化检查（ESLint、测试覆盖率等）
  2. code-reviewer 主导全面审查
  3. 调用相关领域专家进行深度审查
  4. 生成详细审查报告和改进建议
  5. 开发者根据建议修复问题
  6. 重新审查直至通过
- **质量门禁**: 代码必须通过 code-reviewer 审查才能合并
- **持续改进**: 根据审查结果不断优化代码质量
