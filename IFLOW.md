...
#### 10.5.6 API Client 初始化问题

**失误**：在应用启动时直接调用 `getApiClient()`，但 `initApiClient()` 还未完成初始化。

**后果**：抛出错误 `API client not initialized. Call initApiClient() first.`

**规避**：
- 在应用入口（`index.tsx`）使用 `AppInitializer` 组件先调用 `initApiClient()`
- 等待初始化完成后再渲染应用
- 所有 API 调用必须在初始化完成后执行

#### 10.5.7 Axios 与 OpenAPIClientAxios 兼容性问题

**失误**：使用 `axiosConfigDefaults` 配置 `OpenAPIClientAxios` 时遇到 `axios.create is not a function` 错误。

**后果**：API Client 初始化失败，应用无法启动。

**规避**：
- 显式创建 axios 实例并传递给 `OpenAPIClientAxios`
- 使用 `axiosInstance` 参数替代 `axiosConfigDefaults`
- 示例：
```typescript
...
#### 10.5.8 Swagger 路径重复 /api 问题

**失误**：`swagger_json.json` 中的路径已包含 `/api` 前缀，但 `API_BASE_URL` 也包含 `/api`，导致重复。

**后果**：请求 URL 变成 `/api/api/auth/login`，返回 404。

**规避**：
- 创建 axios 实例时去掉 `/api` 后缀：
```typescript
const baseURL = API_BASE_URL.replace(/\/api$/, '');
```
- 因为 swagger 定义中的路径已包含 `/api` 前缀

#### 10.5.9 响应数据空值检查缺失

**失误**：直接访问 API 响应数据的属性，如 `thumbnailResult.data.exists`，未检查 `data` 是否为 `undefined`。

**后果**：`TypeError: Cannot read properties of undefined (reading 'exists')`
...
将旧的 axios 风格 API 服务改造为类型安全的 `getApiClient()` 风格。
...
import type { UpdateNodeDto, OperationMethods } from '../types/api-client';
...
| 场景 | 类型来源 | 示例 |
|------|----------|------|
| **Body DTO** | 从 `api-client.ts` 直接导入 | `import type { UpdateNodeDto } from '../types/api-client'` |
| **Query/Path 参数** | 使用 `Parameters<OperationMethods['...']>[0]` | `type Params = Parameters<OperationMethods['FileSystemController_getNode']>[0]` |
| **Response 类型** | 使用 `Paths.Xxx.Responses.$200` | `type Response = Paths.FileSystemControllerGetNode.Responses.$200` |
...
```typescript
// 正确：直接传递参数对象
getApiClient().FileSystemController_downloadNodeWithFormat({
  nodeId: id,
  format: 'pdf',
  width: '2000',
});
...
- ❌ 禁止使用 `as` 类型断言
- ❌ 禁止使用 `Record<string, any>` 或 `any` 类型
- ❌ 禁止使用旧的 `apiClient`（axios 实例）
- ❌ 禁止手动维护 API 类型（必须从 Swagger 生成）

### 16.5 后端 DTO 规范

**Body DTO**：必须定义独立的 DTO 类，使用 `@ApiProperty()` 装饰器
```typescript
...
**Query DTO**：使用 `@ApiQuery()` 单独定义字段（最佳实践）
```typescript
@ApiQuery({ name: 'format', enum: CadDownloadFormat, required: false })
@ApiQuery({ name: 'width', required: false })
async downloadNodeWithFormat(@Query() query: DownloadNodeQueryDto) { }
```

**注意**：Query DTO 不会生成独立的 schema，前端使用 `Parameters<OperationMethods['...']>[0]` 获取完整参数类型。

### 16.6 类型生成流程

1. 后端定义 DTO 类并使用 `@ApiProperty()` 装饰器
2. 后端 Controller 使用 DTO 作为参数类型
3. 运行 `pnpm generate:types` 生成前端类型
4. 前端从 `types/api-client.ts` 导入类型使用
...
**核心更新**：
- 新增 API Client 初始化最佳实践（第 10.5.6 节）
- 新增 Axios 与 OpenAPIClientAxios 兼容性解决方案（第 10.5.7 节）
- 新增 Swagger 路径 /api 重复问题解决方案（第 10.5.8 节）
- 新增响应数据空值检查规范（第 10.5.9 节）
- 完善常见失误与规避章节，补充前端 API 调用相关问题

**影响文件**：
- `IFLOW.md` - 本文档
- `packages/frontend/services/apiClient.ts` - 初始化逻辑优化
- `packages/frontend/index.tsx` - AppInitializer 组件

### 2026-03-03（v7.1 - API 服务层改造规范）

**核心更新**：
- 新增 API 服务层改造规范章节（第 16 章）
- 明确旧 axios 风格 API 改造为 `getApiClient()` 类型的标准流程
- 定义 Body DTO、Query/Path 参数、Response 类型的使用规则
- 明确禁止事项：`as` 类型断言、`any` 类型、手动维护 API 类型
- 补充后端 DTO 定义规范和类型生成流程

**影响文件**：
- `IFLOW.md` - 本文档
- `packages/frontend/services/filesApi.ts` - 改造示例
- `packages/backend/src/file-system/dto/upload-file.dto.ts` - 新增 DTO 示例
...
_v7.2 | 2026-03-03 | CloudCAD 团队_
_更新：API Client 初始化优化、常见失误补充_
