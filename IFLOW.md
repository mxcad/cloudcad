...
| **本地存储**   | **基于文件系统的本地存储架构，支持智能目录分配和自动清理**      |
...
| **权限分配**   | **前端权限分配组件，支持可视化权限配置和管理**                |
| **项目权限**   | **项目级别权限管理 Hook，支持细粒度项目权限控制**              |
...
│   │   ├── components/
│   │   │   ├── admin/           # 管理员组件
│   │   │   │   ├── GalleryTypeManagement.tsx # 图库分类管理
...
│   │   │   │   ├── ProjectRolesModal.tsx    # 项目角色管理模态框
│   │   │   │   ├── RenameModal.tsx          # 重命名模态框
│   │   │   │   └── SelectFolderModal.tsx    # 选择文件夹模态框
│   │   │   ├── permission/       # 权限组件（新增）
│   │   │   │   └── PermissionAssignment.tsx # 权限分配组件
│   │   │   ├── ui/              # UI 基础组件
...
│   │   │   ├── useProjectPermission.ts # 项目权限管理 Hook（新增）
│   │   │   ├── useProjectManagement.ts # 项目管理 Hook
│   │   │   ├── useExternalReferenceUpload.ts # 外部参照上传 Hook
│   │   │   ├── useMxCadEditor.ts # MxCAD 编辑器 Hook
│   │   │   ├── useMxCadInstance.ts # MxCAD 实例 Hook
...
# 磁盘监控配置
DISK_WARNING_THRESHOLD=21474836480
DISK_CRITICAL_THRESHOLD=10737418240
...
| **本地存储架构** | **基于文件系统的本地存储，支持智能目录分配**        |
| **分片上传**   | **支持大文件分片上传和断点续传**                   |
| **存储管理**   | **统一的存储资源管理，包括目录分配、磁盘监控、文件锁** |
| **自动清理**   | **定期清理过期存储文件，释放磁盘空间**              |
...
| 服务                   | 功能说明                              |
| ---------------------- | ------------------------------------- |
| StorageManager        | 统一存储资源管理，分配节点存储空间    |
| DirectoryAllocator    | 智能目录分配，自动创建和管理目录      |
| LocalStorageProvider  | 本地存储提供者，处理文件读写操作      |
| DiskMonitorService    | 磁盘使用监控，预警磁盘空间不足        |
| FileLockService       | 并发文件操作锁，防止冲突              |
| StorageCleanupService | 定期清理过期存储文件                  |
...
| FileConversionService  | 文件转换服务，转换CAD图纸             |
...
└──────┬──────┘
       ↓
┌─────────────┐
│  6. 创建节点  │ → 在数据库中创建 FileSystemNode
└──────┬──────┘
       ↓
┌─────────────┐
│  7. 转换文件  │ → MxCAD 转换服务转换图纸
└──────┬──────┘
       ↓
┌─────────────┐
│  8. 更新状态  │ → 更新文件状态为 COMPLETED
└─────────────┘
```
...
sequenceDiagram
    ...
    Client->>Backend: 1. 检查分片存在 (checkChunkExist)
    Client->>Backend: 2. 上传分片 (uploadChunk)
    Backend->>Storage: 存储分片到临时目录
    Client->>Backend: 3. 合并请求 (mergeChunks)
    Backend->>Storage: 4. 分配存储空间 (StorageManager)
    Backend->>Storage: 5. 合并分片到存储目录
    Backend->>Database: 6. 创建文件系统节点
    Backend->>MxCAD: 7. 转换文件
    MxCAD->>Backend: 返回转换结果
    Backend->>Storage: 8. 保存转换后的文件
    Backend->>Database: 9. 更新文件状态
    Backend->>Client: 返回成功
```
...
| useProjectPermission Hook    | `packages/frontend/hooks/useProjectPermission.ts`  |
| PermissionAssignment 组件   | `packages/frontend/components/permission/PermissionAssignment.tsx` |
...
| LocalStorageProvider   | `packages/backend/src/storage/local-storage.provider.ts` | 本地存储提供者             |
| StorageService         | `packages/backend/src/storage/storage.service.ts` | 存储服务                   |
...
**PowerShell 命令规范**（Windows 环境）：
- 使用 `dir` 代替 `ls`
- 使用 `copy` 代替 `cp`
- 使用 `type` 代替 `cat`
- 使用 `;` 代替 `&&` 进行命令链
- 使用 PowerShell 原生命令而非 Linux 命令
- 避免使用 `grep`、`awk`、`sed` 等 Linux 工具
...
| 组件/服务                    | 路径                                          |
| --------------------------- | --------------------------------------------- |
| ExternalReferenceModal      | `packages/frontend/components/modals/ExternalReferenceModal.tsx` |
| ExternalReferenceModal.spec.tsx | `packages/frontend/components/modals/ExternalReferenceModal.spec.tsx` |
| useExternalReferenceUpload   | `packages/frontend/hooks/useExternalReferenceUpload.ts` |
| useExternalReferenceUpload.spec.ts | `packages/frontend/hooks/useExternalReferenceUpload.spec.ts` |
| useExternalReferenceUpload.integration.spec.ts | `packages/frontend/hooks/useExternalReferenceUpload.integration.spec.ts` |
| MxCadUploader               | `packages/frontend/components/MxCadUploader.tsx` |
| MxCAD Controller            | `packages/backend/src/mxcad/mxcad.controller.ts` |
| MxCAD Service               | `packages/backend/src/mxcad/mxcad.service.ts` |
...
| `packages/backend/src/health/`                     | 健康检查模块             |
| `packages/backend/src/common/services/`             | 通用服务（存储管理、缓存等）|
| `packages/backend/src/common/README.md`             | 通用模块详细文档         |
| `scripts/generate-frontend-permissions.ts`         | 前端权限常量生成脚本     |

---

## 30. 更新记录

### 2026-02-05（存储架构根本性重构）

**核心重构**：
- **存储架构重构**：从 MinIO S3 存储迁移到本地文件系统存储
- **目录分配机制**：按年月（YYYYMM）自动分配目录，支持子目录扩展（最多100个）
- **存储管理服务**：新增 StorageManager、DirectoryAllocator、LocalStorageProvider 等核心服务
- **磁盘监控**：实时监控磁盘使用情况，支持警告和严重阈值配置
- **文件锁机制**：防止并发文件操作冲突，确保数据一致性
- **自动清理机制**：软删除支持，延迟物理删除（30天后清理）
- **路径安全验证**：防止路径遍历攻击，确保存储安全
- **上传架构优化**：重构文件上传流程，优化分片上传和合并逻辑
- **并发控制**：限流机制控制并发上传数量（最大5个）
- **存储接口定义**：统一 StorageProvider 接口，支持多种存储后端
...
**环境变量更新**：
- 新增 `FILES_DATA_PATH`：本地存储路径
- 新增 `FILES_NODE_LIMIT`：单目录最大节点数（300,000）
- 新增 `STORAGE_CLEANUP_DELAY_DAYS`：存储清理延迟天数（30）
- 新增 `DISK_WARNING_THRESHOLD`：磁盘警告阈值（20GB）
- 新增 `DISK_CRITICAL_THRESHOLD`：磁盘严重阈值（10GB）
- 移除 MinIO 相关配置

**文档质量**：
- 新增本地存储架构章节（10.2）
- 新增文件上传架构章节（10.3）
- 更新存储管理章节（18），反映本地存储架构
- 更新环境变量配置（7.1），移除 MinIO 配置
- 更新服务地址（7.3），移除 MinIO 地址
- 更新分片上传流程图（12.4），反映新架构

---

### 2026-02-05（文档更新与架构完善）

**核心更新**：
- 更新项目结构文档，新增权限组件和标签管理组件
- 更新技术栈版本信息（React 19.2.1、Vite 6.2.0、Zod 4.2.1）
- 完善 FileSystemNode 模型说明，新增 deletedFromStorage 和 deletedByCascade 字段
- 新增审计操作类型（PERMISSION_CHECK、PERMISSION_DENIED）
- 更新前端组件结构，新增 permission 和 admin 子目录
- 完善外部参照上传组件文档，添加测试文件说明
- 更新权限系统章节，新增 useProjectPermission Hook 和 PermissionAssignment 组件
- 补充前端开发最佳实践，添加项目权限和权限分配相关内容
- 更新文件操作规范，补充 Windows PowerShell 命令规范
- 完善文档参考章节，新增脚本和 README 文件说明
...
_v6.6 | 2026-02-05 | CloudCAD 团队_
_更新：存储架构根本性重构、本地存储、智能目录分配、自动清理_