...
- [x] 检测运行环境（Linux vs Windows）
- [x] Linux 环境：直接使用 apt-get 安装组件到指定目录
- [x] Windows 环境：保持现有 Docker 提取逻辑（兼容本地开发）
- [x] 支持 `--output` 参数指定输出目录
- [x] 参考 Dockerfile.node/postgres/redis/svn 的提取逻辑

### 10.2 新建文件

- [x] `runtime/docker/Dockerfile.linux-deploy`
- [x] `runtime/docker/Dockerfile.linux-deploy-verify`
- [x] `scripts/verify-deploy-package.sh`
- [x] `scripts/pack-linux-deploy.js`（可选，简化入口）

### 10.3 测试计划

- [ ] 在 Linux 物理机测试 `extract-linux-runtime.js`
- [x] 在 Docker 容器测试 `extract-linux-runtime.js`
- [x] 测试完整打包流程
- [x] 测试断网验证流程
...
| 平台 | 格式 | 原因 |
|------|------|------|
| Linux | **tar.gz** | Linux 原生支持，无需安装额外解压工具 |
| Windows | 7z | 压缩率更高，Windows 用户通常有 7-Zip |

**Linux 部署包使用 tar.gz 的优势：**
- `tar` 命令所有 Linux 发行版默认包含
- 用户无需安装额外工具即可解压
- 解压命令：`tar -xzf cloudcad-deploy-*.tar.gz`
