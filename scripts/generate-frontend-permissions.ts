...
async function generateFrontendPermissions() {
  try {
    ...
    // 生成权限依赖关系
    const permissionDependencies = generatePermissionDependencies(systemPermissions, projectPermissions);

    // 生成权限分组
    const permissionGroups = generatePermissionGroups(systemPermissions, projectPermissions);
 ...
 * 生成系统权限分组
 */
function generateSystemPermissionGroups(permissions: string[]): string {
 ...
 * 生成项目权限分组
 */
function generateProjectPermissionGroups(permissions: string[]): string {
  const groups: Record<string, { key: string; label: string }[]> = {
    '项目权限': [],
    '文件权限': [],
    'CAD 图纸权限': [],
