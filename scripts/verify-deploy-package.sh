#!/bin/bash
# 梦想网页CAD实时协同平台 部署包验证脚本
# 在干净、断网环境中验证部署包可用性
#
# ⚠️ 已废弃（DEPRECATED）：本脚本已无任何调用点，功能被下列脚本取代，保留仅供历史参考：
#   - runtime/scripts/verify-deploy.js   （部署包内 7 步全绿验证，由 Dockerfile.linux-deploy-verify 调用）
#   - scripts/verify-linux-deploy.js      （开发机侧独立验证，pnpm verify:linux*）
# 新验证请勿引用本脚本。
#
# 使用方式：
...
APP_DIR="/app"
# 生产端口与 runtime/scripts/lib/context.js 一致：后端 3001、前端 3000
BACKEND_PORT=3001
FRONTEND_PORT=3000
