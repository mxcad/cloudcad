/**
 * 梦想网页CAD实时协同平台 Linux Runtime 提取工具
 * 
 * 功能：提取 Linux 运行时组件
 * 
 * 运行环境：
...
function showHelp() {
  console.log(`
${PRODUCT_NAME} Linux Runtime 提取工具
...
function extractPostgresYum(outputPath) {
  ...
  if (fs.existsSync(pgShareDir)) {
    ...
    // 在 share/ 下创建到 share/postgresql/15/* 的符号链接
    // postgres 运行时 get_share_path() 返回 bin/../share/
    // 它会在 share/ 下直接找 timezonesets、timezone 等目录
    // 这些 symlink 让 postgres 无需硬编码的 /usr/share/postgresql/15 路径
  ...
  // 用于拦截 postgres 对 /usr/share/postgresql/15 的硬编码路径访问
  ...
  if (fs.existsSync(redirectC)) {
    ...
    const gccResult = execSync(
      `gcc -shared -fPIC -o ${redirectSo} -ldl ${redirectC}`,
      { stdio: 'pipe' }
    );
...
function extractPostgresApt(outputPath) {
  ...
  for (const pattern of libPaths) {
    try {
      execSync(`cp -L ${pattern} ${libDir}/ 2>/dev/null || true`, { stdio: 'pipe' });
    } catch (e) { /* ignore */ }
  }
  
  // 验证 libpq.so.5 是否存在
  const libpqFiles = fs.readdirSync(libDir).filter(f => f.startsWith('libpq'));
  if (libpqFiles.length > 0) {
    log(`  ✓ 已复制 libpq: ${libpqFiles.join(', ')}`);
  } else {
    log('  ⚠ 警告: 未找到 libpq 库');
  }
...
function main() {
  ...
  log('============================================');
  log(` ${PRODUCT_NAME} Linux Runtime 提取工具`);
  log('============================================');
  log(`输出目录: ${outputDir}`);
  ...
  if (isRuntimeExtracted(outputDir, components)) {
    log('✓ 平台物料已存在（缓存命中），跳过提取');
    log(`  目录: ${outputDir}`);
    // 仍补一下入口脚本（node 提取后生成，保证幂等）
