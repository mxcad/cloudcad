...
// 打包排除规则（同一套配置：scripts/pack-lib/packignore.json）
const {
  isExcluded,
  isKeepEmptyDir,
  isKeepEmptyFile,
...
const INCLUDE_LIST = [
  // 后端源码
  { src: 'packages/backend/src', dest: 'packages/backend/src', isDir: true },
  { src: 'packages/backend/prisma', dest: 'packages/backend/prisma', isDir: true },
  { src: 'packages/backend/prisma.config.ts', dest: 'packages/backend/prisma.config.ts' },
  { src: 'packages/backend/package.json', dest: 'packages/backend/package.json' },
  { src: 'packages/backend/tsconfig.json', dest: 'packages/backend/tsconfig.json' },
  { src: 'packages/backend/tsconfig.build.json', dest: 'packages/backend/tsconfig.build.json' },
  { src: 'packages/backend/nest-cli.json', dest: 'packages/backend/nest-cli.json' },
  { src: 'packages/backend/.env.example', dest: 'packages/backend/.env.example' },

  // db 包（schema 单一源 + prisma 配置 + 源码，容器内构建）
  { src: 'packages/db/prisma', dest: 'packages/db/prisma', isDir: true },
  { src: 'packages/db/prisma.config.ts', dest: 'packages/db/prisma.config.ts' },
  { src: 'packages/db/package.json', dest: 'packages/db/package.json' },
  { src: 'packages/db/src', dest: 'packages/db/src', isDir: true },
  { src: 'packages/db/tsconfig.json', dest: 'packages/db/tsconfig.json' },

  // contracts 包（DI token + 接口，容器内构建）
  ...
  { src: 'packages/frontend/public', dest: 'packages/frontend/public', isDir: true },
  { src: 'packages/frontend/package.json', dest: 'packages/frontend/package.json' },
  { src: 'packages/frontend/tsconfig.json', dest: 'packages/frontend/tsconfig.json' },
  { src: 'packages/frontend/vite.config.ts', dest: 'packages/frontend/vite.config.ts' },
  { src: 'packages/frontend/vitest.config.ts', dest: 'packages/frontend/vitest.config.ts' },
  { src: 'packages/frontend/index.html', dest: 'packages/frontend/index.html' },
  { src: 'packages/frontend/vite-env.d.ts', dest: 'packages/frontend/vite-env.d.ts' },
  { src: 'packages/frontend/types.ts', dest: 'packages/frontend/types.ts' },
  { src: 'packages/frontend/metadata.json', dest: 'packages/frontend/metadata.json' },
 ...
 * 复制目录（递归，排除指定子目录）
 */
function copyDir(src, dest, excludes = [], relPath = '') {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      copyDir(srcPath, destPath, excludes, entryRel);
      ...
      if (fs.statSync(realPath).isDirectory()) {
        copyDir(realPath, destPath, excludes, entryRel);
      } else {
        if (!isExcluded(entryRel) && !isKeepEmptyFile(entryRel)) {
...
function prepareTempDir() {
  ...
  for (const item of INCLUDE_LIST) {
    ...
    if (item.isDir) {
      copyDir(srcPath, destPath, EXCLUDE_IN_DIRS, item.src);
      log(`复制 ${item.src}/`);
    } else {
  ...
  // 创建部署说明
  const readmeContent = `# ${PRODUCT_NAME} v${VERSION} Docker 部署包
...
async function main() {
  ...
  log('============================================');
  log(` ${PRODUCT_NAME} v${VERSION} Docker 部署包打包`);
  log('============================================');
  log(`输出目录: ${OUTPUT_DIR}`);
  log('');
