...
│   ├── commands/             # 自定义命令（feature-dev）
│   └── skills/               # 技能集（code-concise, layered-context, ui-ux-pro-max）
├── svn-repo/                 # SVN 仓库（新增）
├── filesData/                # 文件数据存储
├── temp/                     # 临时文件
└── uploads/                  # 上传文件
...
| 权限系统 | RBAC 权限控制 + 多级缓存 | [permissions.md](documents/backend/core/permissions.md) |
...
```powershell
pnpm dev                               # 启动基础设施 + 后端
pnpm dev:infra                         # 启动基础设施（PostgreSQL, Redis, pgAdmin）
...
### 5.3 前端命令（packages/frontend/cloudcad-manager）
...
基于角色的细粒度权限控制，支持系统权限和项目权限，多级缓存优化
...
详细信息请参考：[README.md](packages/svnVersionTool/README.md)
...
| TagManagement | `/tags` | 标签管理页面 |
| TrashPage | `/trash` | 回收站页面 |
...
_v6.8 | 2026-02-09 | CloudCAD 团队_
_更新：项目整合、SVN 版本控制、iFlow 专用工具_