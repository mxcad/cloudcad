...
describe('T37 - 存储配额满→上传拒绝集成测试', () => {
  let quotaEnforcementService: QuotaEnforcementService;
  let storageQuotaService: StorageQuotaService;
  let storageInfoService: StorageInfoService;
  ...
  beforeEach(async () => {
    ...
    quotaEnforcementService = module.get<QuotaEnforcementService>(QuotaEnforcementService);
    storageQuotaService = module.get<StorageQuotaService>(StorageQuotaService);
    storageInfoService = module.get<StorageInfoService>(StorageInfoService);
  });
  ...
  describe('T37 - 存储配额检查', () => {
    it('T37-S1 - 配额充足时，允许上传', async () => {
      ...
      const result = await quotaEnforcementService.checkUploadQuota(
        mockUserId,
        mockNodeId,
        fileSize
      );
    ...
    it('T37-S2 - 配额不足时，拒绝上传并返回配额信息', async () => {
      ...
      await expect(
        quotaEnforcementService.checkUploadQuota(mockUserId, mockNodeId, fileSize)
    ...
    it('T37-S7 - 检查用户是否已超额使用配额', async () => {
      ...
      const isExceeded = await quotaEnforcementService.isQuotaExceeded(
        mockUserId,
        mockNodeId
      );
    ...
    it('T37-S8 - 获取配额超额详情和建议', async () => {
      ...
      const details = await quotaEnforcementService.getQuotaExceededDetails(
        mockUserId,
        mockNodeId
      );
    ...
    it('T37-S11 - 更新节点存储配额', async () => {
      ...
      const newQuotaGB = 50;
      const updatedNode = await storageQuotaService.updateNodeStorageQuota(
        mockNodeId,
        newQuotaGB
      );
    ...
    it('T37-S12 - 验证配额值不能为负数', async () => {
      await expect(
        storageQuotaService.updateNodeStorageQuota(mockNodeId, -1)
      ).rejects.toThrow('存储配额不能为负数');
    });
  });
});
