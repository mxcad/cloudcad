...
describe('File Delete & Recycle Integration Tests', () => {
  ...
  beforeEach(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: 'VERSION_CONTROL_SERVICE', useValue: mockVersionControlService },
        { provide: StorageInfoService, useValue: mockStorageInfoService },
        { provide: FileTreeService, useValue: mockFileTreeService },
      ],
    }).compile();

    fileOperationsService = moduleFixture.get<FileOperationsService>(FileOperationsService);
  ...
  describe('Test Case 1: Multiple references - delete one should only decrease count', () => {
    it('should verify that deleting one of multiple files with same hash does not delete the physical file', async () => {
      ...
      (databaseService.fileSystemNode.count as jest.Mock)
  ...
  describe('Test Case 4: Full workflow - create → multiple references → delete one → delete last → restore', () => {
    it('should verify the complete file delete and restore workflow', async () => {
      ...
      const file1 = { id: 'wf-file1', name: 'wf1.dwg', fileHash, deletedAt: null };
      const file2 = { id: 'wf-file2', name: 'wf2.dwg', fileHash, deletedAt: null };
      const file3 = { id: 'wf-file3', name: 'wf3.dwg', fileHash, deletedAt: null };

      // Step 1: Delete first file - should have 2 other references
      (databaseService.fileSystemNode.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...file1, isFolder: false, parentId: testProjectId, ownerId: testUserId });
      (databaseService.fileSystemNode.count as jest.Mock)
        .mockResolvedValueOnce(2); // 2 other references
      ...
      jest.clearAllMocks();
      (databaseService.fileSystemNode.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...file2, isFolder: false, parentId: testProjectId, ownerId: testUserId });
      (databaseService.fileSystemNode.count as jest.Mock)
        .mockResolvedValueOnce(1); // 1 other reference
      ...
      jest.clearAllMocks();
      (databaseService.fileSystemNode.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...file3, isFolder: false, parentId: testProjectId, ownerId: testUserId });
      (databaseService.fileSystemNode.count as jest.Mock)
        .mockResolvedValueOnce(0); // 0 other references
      ...
      (databaseService.fileSystemNode.update as jest.Mock)
        .mockResolvedValueOnce({ ...deletedFile, deletedAt: null, fileStatus: 'COMPLETED' });
