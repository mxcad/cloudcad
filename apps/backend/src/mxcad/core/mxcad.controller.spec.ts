...
describe('MxCadController', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: FileSystemPermissionService, useValue: mockFileSystemPermission },
  ...
  describe('checkChunkExist', () => {
    it('should return exists=true when chunk exists', async () => {
      mockMxCadService.checkChunkExist.mockResolvedValue({ ret: 'chunkAlreadyExist' });
      const result = await controller.checkChunkExist(
        { chunk: 0, fileHash: 'abc', size: 100, chunks: 2, filename: 'f.dwg' } as any,
    ...
    it('should return exists=false when chunk does not exist', async () => {
      mockMxCadService.checkChunkExist.mockResolvedValue({ ret: 'chunkNoExist' });
      const result = await controller.checkChunkExist(
        { chunk: 5, fileHash: 'abc', size: 100, chunks: 10, filename: 'f.dwg' } as any,
        { user: { id: 'u1', roleId: 'USER' } } as any,
      );
  ...
  describe('checkFileExist', () => {
    it('should return exists=true when file exists', async () => {
      mockMxCadService.checkFileExist.mockResolvedValue({ ret: 'fileAlreadyExist', nodeId: 'n1' });
      ...
      mockPrisma.fileSystemNode.findUnique.mockResolvedValue({ id: 'root-1', isRoot: true, projectId: null });
      mockJwtService.verify.mockReturnValue({ sub: 'u1' });
      mockPrisma.user.findUnique.mockResolvedValue({ id: 'u1', status: 'ACTIVE', roleId: 'USER' });
      ...
      const result = await controller.checkFileExist(
        ...
        { headers: { authorization: 'Bearer token' }, body: { nodeId: 'root-1' }, query: {} } as any,
      );
      expect(result.exists).toBe(true);
    });
  });
  ...
  describe('checkDuplicateFile', () => {
    ...
    it('should return isDuplicate=false for new file', async () => {
      mockMxCadService.checkDuplicateFile.mockResolvedValue({ isDuplicate: false });
      const result = await controller.checkDuplicateFile(
  ...
  describe('checkExternalReference', () => {
    it('should return exists boolean', async () => {
      ...
      const result = await controller.checkExternalReference('node-1', { fileName: 'ref.dwg' });
      expect(result.exists).toBe(true);
    });
  ...
  describe('saveMxwebToNode', () => {
    ...
    it('should throw on save failure', async () => {
      ...
      await expect(
        controller.saveMxwebToNode('node-1', {} as any, '', {} as any),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('uploadThumbnail', () => {
    it('should upload thumbnail and return success', async () => {
      mockMxCadService.getFileSystemNodeByNodeId.mockResolvedValue({
        id: 'n1',
        path: '2026/n1/f.mxweb',
      });
      ...
      await controller.uploadThumbnail(
        'n1',
        { path: '/tmp/test.png', originalname: 'test.png', filename: 'test.png' } as any,
        res as any,
      );
      expect(res.json).toHaveBeenCalledWith(
    ...
    it('should return error when no file provided', async () => {
      const res = { json: jest.fn().mockReturnThis(), status: jest.fn().mockReturnThis() };
      await controller.uploadThumbnail('n1', null as any, res as any);
      expect(res.json).toHaveBeenCalledWith(
  ...
  // ==================== getNonCadFile ====================
  describe('getNonCadFile', () => {
    it('should reject paths with traversal attempts', async () => {
      const res = { json: jest.fn().mockReturnThis(), status: jest.fn().mockReturnThis() };
      await controller.getNonCadFile('../../etc/passwd', res as any);
      expect(res.json).toHaveBeenCalledWith(
        expect.objectContaining({ code: -1 }),
      );
    });
    ...
    it('should return file stream for valid path', async () => {
      ...
      await controller.getNonCadFile('valid/path.dwg', res as any);
