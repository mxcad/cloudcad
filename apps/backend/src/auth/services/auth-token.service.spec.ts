...
describe('AuthTokenService', () => {
  ...
  describe('generateTokens', () => {
    ...
    it('should generate tokens successfully with default expirations', async () => {
      mockConfigService.get.mockImplementation((key) => {
        ...
        if (key === 'jwt.expiresIn') return undefined;
        if (key === 'jwt.refreshExpiresIn') return undefined;
        return 'test-value';
      });
      ...
      mockJwtService.verify.mockReturnValue({ exp: Math.floor(Date.now() / 1000) + 86400, sub: 'user123', type: 'refresh' });
    ...
    it('should generate tokens with configured expirations', async () => {
      mockConfigService.get.mockImplementation((key) => {
        ...
        if (key === 'jwt.refreshExpiresIn') return '14d';
        return 'test-value';
      });
  ...
  describe('refreshToken', () => {
    it('should throw when token type not refresh', async () => {
      mockJwtService.verify.mockReturnValue({ sub: 'user123', type: 'access', exp: Math.floor(Date.now() / 1000) + 86400 });
    ...
    it('should throw when refresh token invalid', async () => {
      mockJwtService.verify.mockReturnValue({ sub: 'user123', type: 'refresh', exp: Math.floor(Date.now() / 1000) + 86400 });
      mockConfigService.get.mockReturnValue('test-secret');
      mockPrisma.refreshToken.findFirst.mockResolvedValue(null);
    ...
    it('should throw when user not active', async () => {
      ...
      mockConfigService.get.mockReturnValue('test-secret');
      mockPrisma.refreshToken.findFirst.mockResolvedValue({ token: 'valid' });
      mockPrisma.user.findUnique.mockResolvedValue({ id: 'user123', status: 'SUSPENDED' });
