	...
	describe("forgotPassword", () => {
		...
		describe("with phone", () => {
			it("should throw when sms disabled", async () => {
				...
				expect(result.message).toContain("均未启用");
			});
			...
			it("should throw when user not found", async () => {
				...
				await expect(
					service.forgotPassword(undefined, "13812345678"),
				).rejects.toThrow(UnauthorizedException);
			});
			...
			it("should send verification sms when valid", async () => {
				...
				).toHaveBeenCalledWith("13812345678");
				expect(result.message).toContain("密码重置验证码已发送");
			});
		});
	});
	...
	describe("resetPassword", () => {
		describe("with email", () => {
			...
			it("should reset password successfully", async () => {
				...
				expect(
					mockAuthTokenService.deleteAllRefreshTokens,
				).toHaveBeenCalledWith("user123");
				expect(
					mockTokenBlacklistService.removeUserFromBlacklist,
				).toHaveBeenCalledWith("user123");
		...
		describe("with phone", () => {
			it("should throw when verification code invalid", async () => {
				...
				await expect(
					service.resetPassword(undefined, "13812345678", "123456", "newpass"),
				).rejects.toThrow(UnauthorizedException);
			});
