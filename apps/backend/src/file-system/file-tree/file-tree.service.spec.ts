...
describe('FileTreeService', () => {
  ...
  describe('createFileNode', () => {
    ...
    it('should create a file node successfully', async () => {
      ...
      mockPrisma.fileSystemNode.findUnique
        .mockResolvedValueOnce({ id: 'parent-1', isFolder: true, isRoot: false, projectId: 'proj-1' }) // parent check
    ...
    it('should generate unique name when file name conflicts', async () => {
      ...
      mockPrisma.$transaction.mockImplementation(async (cb: Function) => {
        const tx = {
          fileSystemNode: {
            findMany: mockPrisma.fileSystemNode.findMany,
            findUnique: mockPrisma.fileSystemNode.findUnique,
            create: jest.fn().mockResolvedValue({ id: 'node-2', name: 'test (2).dwg' }),
            update: jest.fn(),
          },
        };
        return cb(tx);
      ...
      mockPrisma.fileSystemNode.findUnique
        .mockResolvedValueOnce({ id: 'parent-1', isFolder: true, isRoot: false, projectId: 'proj-1' })
  ...
  describe('getNode', () => {
    ...
    it('should throw NotFoundException when not found', async () => {
      mockPrisma.fileSystemNode.findUnique.mockResolvedValue(null);
      await expect(service.getNode('missing')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('getRootNode', () => {
    ...
    it('should return project root for nested node', async () => {
      ...
      mockPrisma.fileSystemNode.findUnique
        .mockResolvedValueOnce({ id: 'child-1', isRoot: false, projectId: null, parentId: 'parent-1' })
        .mockResolvedValueOnce({ id: 'parent-1', isRoot: false, projectId: 'root-1', parentId: null })
        .mockResolvedValueOnce({ id: 'root-1', isRoot: true, projectId: null, parentId: null });
  ...
  describe('getProjectId', () => {
    ...
    it('should traverse up parents to find root', async () => {
      mockPrisma.fileSystemNode.findUnique
        .mockResolvedValueOnce({ id: 'child-1', isRoot: false, projectId: null, parentId: 'parent-1' })
        .mockResolvedValueOnce({ id: 'parent-1', isRoot: false, projectId: null, parentId: 'root-1' })
        .mockResolvedValueOnce({ id: 'root-1', isRoot: true, projectId: null, parentId: null });
      expect(await service.getProjectId('child-1')).toBe('child-1');
    });
  ...
  describe('getLibraryKey', () => {
    ...
    it('should resolve libraryKey from root for nested nodes', async () => {
      mockPrisma.fileSystemNode.findUnique
        .mockResolvedValueOnce({ id: 'child-1', isRoot: false, projectId: 'lib-1', libraryKey: null })
